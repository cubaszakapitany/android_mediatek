...
/* wanglj, DATE20131225, NOTE, Bug For_7811_kernel_source START */
#if defined(TINNO_ANDROID_S7811A) && !defined(BUILD_UBOOT) && !defined(BUILD_LK)
static int g_ps_status = 0;
static volatile int starting_work = 0;
static struct delayed_work  g_backlight_delay_work;

int mtkfb_set_ps_state(int ps_status)
{
	if ( g_ps_status != ps_status ){
		printk("[FB Driver] set g_ps_status %d --> %d\n", g_ps_status, ps_status);
		g_ps_status = ps_status;
	}
}
...
{
	extern wait_queue_head_t lcm_flush_wait;
	extern volatile int flush_flag;
       printk("[FB Driver] <g_ps_status=%d>set flush_flag = 1\n", g_ps_status);
	flush_flag = 1;
	wake_up_interruptible(&lcm_flush_wait);
	starting_work = 0;
}

#endif
/* wanglj, Bug For_7811_kernel_source END */
