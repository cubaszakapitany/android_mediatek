...
static long ion_sys_ioctl(struct ion_client *client, unsigned int cmd, unsigned long arg, int from_kernel)
{
	...
	if(ion_debugger)
	{
		...
		if(!from_kernel && (Param.record_param.backtrace_num > 0))
		{
			...
			{
				if((Param.record_param.mapping_record[i].name!= NULL) && (Param.record_param.mapping_record[i].size > 0) )
				{
					...
					if(string_len > 0)
					{
						...
						{
							Param.record_param.mapping_record[i].name = NULL;
							Param.record_param.mapping_record[i].size = 0;
