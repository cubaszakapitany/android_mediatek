...
//add by xiaochun.cai begin
#ifdef RE_CHARGE_TEMPERATURE    //add for re-charge
int g_Batt_OverTemperature = 0;
extern PMU_STATUS BAT_PreChargeModeAction(void);
#endif

//add by xiaochun.cai end
...
static void mt_battery_CheckBatteryStatus(void)
{
   ...
   #ifdef RE_CHARGE_TEMPERATURE    //add for re-charge
    if (BMT_status.temperature <= RE_CHARGE_TEMPERATURE && BMT_status.temperature >=RE_CHARGE_TEMPERATURE_LOW && (g_Batt_OverTemperature == 1))
    {
        BAT_PreChargeModeAction();
        BMT_status.bat_charging_state = CHR_PRE;
        g_Batt_OverTemperature = 0;
    }
   #endif
}
...
//Eric add< power off if charger disconnect on POWER_OFF charge mode.
//Only for the bug: the phone does not shut down immediately  if pull out charger when system is initializing
        if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
        {
             struct timespec time1;
             time1 = current_kernel_time();  //there is always a charger disconnect interrupt when power on, although the charger is connected. Eric.
             if(time1.tv_sec >0) {mt_power_off();}
        } else //>Eric add
             mt_usb_disconnect();
    }
}
...
void do_chrdet_int_task(void)
{
    if(g_bat_init_flag == KAL_TRUE)
    {
        if( upmu_is_chr_det() == KAL_TRUE )
        {
            battery_xlog_printk(BAT_LOG_CRTI, "111[do_chrdet_int_task] charger exist!g_boot_mode:%d\n", g_boot_mode);
    		BMT_status.charger_exist = KAL_TRUE;
		    wake_lock(&battery_suspend_lock);
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
    ...
    do
    {
        ...
        wait_event_interruptible(charger_hv_detect_waiter, (charger_hv_detect_flag == KAL_TRUE));
       	...
       	if ((upmu_is_chr_det() == KAL_TRUE))
        {
            //check_battery_exist();//modified by mickal.ma
        }
