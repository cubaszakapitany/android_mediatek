...
extern kal_uint8 g_tp_charger_flag;
#endif 
...
#else
	else if(BMT_status.temperature <= MIN_CHARGE_TEMPERATURE && BMT_status.charger_exist== KAL_TRUE)//modified by mickal.ma
#endif
    {
...
void BAT_thread(void)
{
	...
	//BEGIN <charging> <DATE20130927> <add charging flag> zhangxiaofei
    #if defined(PROJECT_L4550)
    g_tp_charger_flag = BMT_status.charger_exist;
    #endif
...
static ssize_t current_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int len = 0;
    char desc[32];
    bool cmd_current_unlimited = false;
