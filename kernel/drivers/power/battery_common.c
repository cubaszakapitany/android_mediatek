...
extern int g_powerkey_pressed;//guomingyi
////////////////////////////////////////////////////////////////////////////////
...
//<guomingyi><20140609><For ftm charging led display start>
#define led_red (0)
#define led_green (1)
#define led_off (0)
#define led_on (255)
extern mt65xx_leds_set_onoff(int type, int on);
//<guomingyi><20140609><For ftm charging led display end>
...
{
	battery_xlog_printk(BAT_LOG_CRTI, "mt_battery_update_EM\n");	
...
battery_xlog_printk(BAT_LOG_CRTI, "[Battery]###battery_update\n");
...
static void mt_battery_notify_VBatTemp_check(void)
{
    ...
    {
	if(BMT_status.temperature <= -30){
             	g_BatteryNotifyCode &= ~(0x0002);
	     	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] <<<<<<<<<<<<<<NOT HAVE BATTERY!!>>>>>>>>>>>>>\n");
         }
	else if(BMT_status.temperature <= -28){
...
static void mt_battery_thermal_check(void)
{
	if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        //ignore default rule
#else    
		if(BMT_status.temperature >= POWER_OFF_TEMPERATURE)
        {
            #if defined(CONFIG_POWER_EXT)
...
#else
	{
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] mt_battery_update_status..\n");
...
//<guomingyi><20140609><For ftm charging led display start>  
	battery_xlog_printk(BAT_LOG_CRTI, "###--->>>AC_CHARGER_CURRENT:%d\n", AC_CHARGER_CURRENT);
	if((g_bat_init_flag == KAL_TRUE) && (g_boot_mode == FACTORY_BOOT)){
		if(upmu_is_chr_det()){
			battery_xlog_printk(BAT_LOG_CRTI, "###--->>>turn on led\n");
			if(BMT_status.bat_full){
				mt65xx_leds_set_onoff(led_green, led_on);
				mt65xx_leds_set_onoff(led_red, led_off);
			}
			else{
				mt65xx_leds_set_onoff(led_red, led_on);
				mt65xx_leds_set_onoff(led_green, led_off);
			}
		}
		else{
				battery_xlog_printk(BAT_LOG_CRTI, "###--->>>turn off led\n");
				mt65xx_leds_set_onoff(led_red, led_off);
				mt65xx_leds_set_onoff(led_green, led_off);
		}
	}
//<guomingyi><20140609><For ftm charging led display end>
            ...
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Charger plug in/out, Call battery_meter_reset. (%d), upmu_is_chr_det():%d\n", BMT_status.UI_SOC, upmu_is_chr_det());
        }
...
static int battery_suspend(struct device *device)
{
	...
	g_powerkey_pressed =0;//guomingyi add
    return 0;
}

extern void bat_update(void);//guomigyi add
...
static int battery_resume(struct device *device)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "g_powerkey_pressed:%d, (is_pcm_timer_trigger||bat_spm_timeout):%d,\n", g_powerkey_pressed,(is_pcm_timer_trigger ||bat_spm_timeout));
	if((is_pcm_timer_trigger == KAL_TRUE) || bat_spm_timeout || (g_powerkey_pressed == 1)) //guomingyi add
	{	
		battery_xlog_printk(BAT_LOG_CRTI, "battery_resume --->BAT_thread\n");
		mutex_lock(&bat_mutex);
		bat_update(); //guomingyi add
		BAT_thread();
		mutex_unlock(&bat_mutex);
	}
	else
