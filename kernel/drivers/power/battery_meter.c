...
int BattThermistorConverTemp(int Res)
{
    ...
    int RES1=0,RES2=0;
    int TBatt_Value=-200,TMP1=0,TMP2=0;
    // Jake.L, DATE20130710, Auto fit table size, DATE20130710-01 START
    int table_size=16;
    ...
    // Jake.L, DATE20130710-01 END
    else
    {
        RES1=Batt_Temperature_Table[0].TemperatureR;
        TMP1=Batt_Temperature_Table[0].BatteryTemp;

        // Jake.L, DATE20130710, Auto fit table size, DATE20130710-01 LINE
        for(i=0;i<=table_size/*16*/;i++)
        {
            if(Res>=Batt_Temperature_Table[i].TemperatureR)
            {
                RES2=Batt_Temperature_Table[i].TemperatureR;
...
void bat_update(void)
{
	...
	oam_car_1 = 0;
	oam_car_2 = 0;
	bm_print(BM_LOG_CRTI, "###bat_update--->>hw_ocv_after_sleep=%d, oam_d0=%d\n", hw_ocv_after_sleep, oam_d0);
}
