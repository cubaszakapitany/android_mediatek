...
static kal_uint32 charging_full_check(void)
{
	...
	if ( status == KAL_TRUE) {
		...
		if (g_full_check_count >= FULL_CHECK_TIMES) {
			//<Begin> <Add bat voltage> <20131112> <mickal.ma>
			#if defined(PROJECT_L4550)
			if(g_temp_status != TEMP_POS_45_TO_POS_60 && BMT_status.bat_vol < 4300)//bat_vol low 4.3V, charging not full
					return KAL_FALSE;
			#endif
			//<End> <Add bat voltage> <20131112> <mickal.ma>
			return KAL_TRUE;
		}
		else
			return KAL_FALSE;
