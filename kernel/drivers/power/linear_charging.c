...
PMU_STATUS BAT_PreChargeModeAction(void)
{
    ...
    battery_xlog_printk(BAT_LOG_CRTI,
		"#111[BATTERY] Pre-CC mode charge, timer=%ld on %ld    upmu_is_chr_det():%d\n\r", 
		BMT_status.PRE_charging_time, BMT_status.total_charging_time,  upmu_is_chr_det());    
...
//guomingyi add start.
	...
	if(upmu_is_chr_det()){
		if(g_boot_mode == FACTORY_BOOT){
			 battery_xlog_printk(BAT_LOG_CRTI, "#BMT_status.bat_full:%d\n", BMT_status.bat_full);
