...
// QHQ RT Monitor    
	...
	{
		//arg: printf("  -s wdt [1/2/3/4]: 1(WMS), 2(PMS), 3(SF), 4(Display)\n");lhd
...
static void external_exception(const char *assert_type, const int *log, int log_size, const int *phy, int phy_size, const char *detail, const int db_opt)
{
    int *ee_log = NULL;
    unsigned long flags = 0;
