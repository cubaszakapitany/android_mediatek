...
void aee_disable_api(void) {
	if (g_aee_api) {
		xlog_printk(ANDROID_LOG_INFO, AEK_LOG_TAG, "disable aee kernel api");
		g_aee_api = NULL;
	}
}
...
void aed_md_exception_api(const int *log, int log_size, const int *phy, int phy_size, const char* detail, const int db_opt)
{
	...
	if (g_aee_api)
	{
		if (g_aee_api->md_exception) {
			g_aee_api->md_exception("modem", log, log_size, phy, phy_size, detail, db_opt);
		} else {
...
void aed_combo_exception_api(const int *log, int log_size, const int *phy, int phy_size, const char* detail, const int db_opt)
{
	...
	if (g_aee_api)
	{
		if (g_aee_api->combo_exception) {
			g_aee_api->combo_exception("combo", log, log_size, phy, phy_size, detail, db_opt);
		} else {
