...
static int ipanic_mmprofile(void *data, unsigned char *buffer, size_t sz_buf) {
	...
	static unsigned int mmprofile_dump_size = 0;
	unsigned int pbuf = 0;
	...
	if (mmprofile_dump_size == 0) {
		...
		if (mmprofile_dump_size == 0 || mmprofile_dump_size > IPANIC_MMPROFILE_LIMIT) {
			IPANIC_E("%s: INVALID MMProfile size[%x]", __func__, mmprofile_dump_size);
			return -3;
		}
	}
...
static const char IPANIC_DT_STR[][16] = {"Undefined", "kernel log", "main log", "system log", "radio log"};
...
inline int ipanic_func_write(fn_next next, void *data, int off, int total, int encrypt) {
	...
	IPANIC_V("%s: off[%x], encrypt[%d]\n", __func__, off, encrypt);
	...
	do {
		...
		if ((off - start + size) > total) {
			IPANIC_E("%s: data oversize(%x>%x@%x)\n", __func__, off - start + size, total, start);
			errno = -EFBIG;
			break;
		}
		if (encrypt) 
...
int ipanic_next_write(fn_next next, void *data, int off, int total, int encrypt) {
...
void ipanic_data_to_sd(int dt, void *data) {
	...
	if (next == NULL) {
		...
		errno = ipanic_next_write(next, data, dheader->offset, dheader->total, dheader->encrypt);
	}
	if (IS_ERR(ERR_PTR(errno))) {
		IPANIC_W("%s: dump %s failed[%d]\n", __func__, dheader->name, errno);
		if (errno == -EFBIG)
...
loff_t ipanic_offset_mini_rdump(void) {
	struct ipanic_header *ipanic_hdr = ipanic_header();
	return ipanic_hdr->data_hdr[IPANIC_DT_MINI_RDUMP].offset;
}

void ipanic_header_update_minirdump(size_t size) {
	struct ipanic_header *ipanic_hdr = ipanic_header();
...
struct ipanic_header* ipanic_header_from_sd(void) {
	...
	if(IS_ERR_OR_NULL((void*)header)) {
		IPANIC_D("%s: read header failed[%d]\n", __func__, header);
		header = NULL;
	} else if(header->magic != AEE_IPANIC_MAGIC) {
		IPANIC_D("%s: no ipanic data[%x]\n", __func__, header->magic);
		header = NULL;
		ipanic_erase();
	} else {
		...
		IPANIC_D("%s: ipanic data available^v^%s^v^\n", __func__, str);
	}
	return header;
}
...
struct aee_oops* ipanic_oops_from_sd(void) {
	...
	if (oops == NULL) {
		IPANIC_E("%s: can not allocate buffer\n", __FUNCTION__);
		return NULL;
	}
	...
	for (i = IPANIC_DT_HEADER + 1; i < IPANIC_DT_RESERVED31; i++) {
		...
		if (data) {
			...
			case IPANIC_DT_MMPROFILE:
				...
				break;
			default:
				IPANIC_I("%s: [%d] NOT USED.\n", __FUNCTION__, i);
			}
		} else {
			IPANIC_W("%s: read %s failed, %x@%x\n", __FUNCTION__,
				 dheader->name, dheader->used, dheader->offset);
		}
	}
	return oops;		
...
int ipanic(struct notifier_block *this, unsigned long event, void *ptr) {
	...
	IPANIC_D("%s: ipanic done^_^", __func__);
	for (dt = IPANIC_DT_HEADER + 1; dt < IPANIC_DT_RESERVED31; dt++) {
		dheader = &ipanic_hdr->data_hdr[dt];
		if (dheader->valid) {
			IPANIC_D("%s[%x@%x],", dheader->name, dheader->used, dheader->offset);
		}
	}
	IPANIC_D("^_^\n");
...
void ipanic_recursive_ke(struct pt_regs *regs, struct pt_regs *excp_regs, int cpu) {
	...
	__mrdump_mini_create_oops_dump(AEE_REBOOT_MODE_NESTED_EXCEPTION, excp_regs, "Nested Panic");
	...
	if (ipanic_dt_active(IPANIC_DT_RAM_DUMP)) {
		...
		__mrdump_create_oops_dump(AEE_REBOOT_MODE_NESTED_EXCEPTION, excp_regs, "Nested Panic");
	}
}
...
struct ipanic_header* ipanic_header(void) {
	...
	BUG_ON(ipanic_msdc_info(iheader));
...
int __init aee_ipanic_init(void)
{
	...
	ipanic_msdc_init();
	IPANIC_I("ipanic: startup, partition assgined %s\n", AEE_IPANIC_PLABEL);
	return 0;
}
