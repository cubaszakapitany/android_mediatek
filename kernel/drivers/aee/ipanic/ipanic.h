...
#define IPANIC_LOG_LEVEL ANDROID_LOG_INFO
#define LOG(prio, tag, fmt, ...)			\
	if (prio >= IPANIC_LOG_LEVEL) {				\
		if (aee_in_nested_panic())			\
			aee_nested_printf(fmt, ##__VA_ARGS__);	\
		else						\
			xlog_printk(prio, tag, fmt, ##__VA_ARGS__);\
	}
		
#define IPANIC_V(x...)	LOG(ANDROID_LOG_VERBOSE, IPANIC_LOG_TAG, ## x)
#define IPANIC_D(x...)	LOG(ANDROID_LOG_DEBUG, IPANIC_LOG_TAG, ## x)
#define IPANIC_I(x...)	LOG(ANDROID_LOG_INFO, IPANIC_LOG_TAG, ## x)
#define IPANIC_W(x...)	LOG(ANDROID_LOG_WARN, IPANIC_LOG_TAG, ## x)
#define IPANIC_E(x...)	LOG(ANDROID_LOG_ERROR, IPANIC_LOG_TAG, ## x)
