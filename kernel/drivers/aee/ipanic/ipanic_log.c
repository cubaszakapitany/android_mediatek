...
#define __LOG_BUF_LEN (1 << CONFIG_LOG_BUF_SHIFT)
	...
	dumper->list.prev = next.value ? (struct list_head*)next.value : (struct list_head*)(log_end - __LOG_BUF_LEN);
	dumper->list.next = (struct list_head*)log_end;
	next.value = log_end;
	IPANIC_D("kernel log region: [%x,%x]", (int)dumper->list.prev, (int)dumper->list.next);
#else
	...
	IPANIC_D("kernel log region: [%x:%x,%x:%x]", dumper->cur_idx, dumper->cur_seq, dumper->next_idx, dumper->next_seq);
#endif
}
