...
#if defined CONFIG_ARCH_MT6572
static int backlight_set_pwm_0(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
	struct pwm_spec_config pwm_setting;
...
static int backlight_set_pwm(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
	...
	pwm_setting.PWM_MODE_FIFO_REGS.IDLE_VALUE = 0;
	pwm_setting.PWM_MODE_FIFO_REGS.GUARD_VALUE = 0;
	pwm_setting.PWM_MODE_FIFO_REGS.STOP_BITPOS_VALUE = 63;		// 31;
	pwm_setting.PWM_MODE_FIFO_REGS.GDURATION = 0;			// (pwm_setting.PWM_MODE_FIFO_REGS.HDURATION+1)*32 - 1;
	pwm_setting.PWM_MODE_FIFO_REGS.WAVE_NUM = 0;
...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK2)
	{
		...
		{       
            ...
            upmu_set_isink_ch2_step(0x2); // 4mA
            upmu_set_isink_sfstr2_tc(0x0); // 0.5us
            upmu_set_isink_sfstr2_en(0x0); // Disable soft start
			upmu_set_rg_isink2_double_en(0x0); // Disable double current
			upmu_set_isink_phase2_dly_en(0x0); // Disable phase delay
...
*/
    ...
    printk("mt65xx_leds_set_cust: #6572 set brightness, name:%s, mode:%d, level:%d\n", 
		cust->name, cust->mode, level);
...
static void mt65xx_led_work(struct work_struct *work)
{
	...
	LEDS_DEBUG("#mt65xx_led_work [LED]%s:%d\n", led_data->cust.name, led_data->level);
...
#define led_off (0)
#define led_on (255)
void mt65xx_leds_set_onoff(int type, int on){
	LEDS_DEBUG("#mt65xx_leds_set_onoff on: %d, type:%d\n", on, type);
	
	if(on == 0){
		mt65xx_leds_brightness_set(type, led_off);
	}
	else{
		mt65xx_leds_brightness_set(type, led_on);
	}
}
//guomingyi add end.
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	int ret, rc;
	struct cust_mt65xx_led *cust_led_list = get_cust_led_list();
	printk("#mt65xx_leds_probe [LED]%s\n", __func__);
...
static int __init mt65xx_leds_init(void)
{
	...
	LEDS_DEBUG("#mt65xx_leds_init [LED]%s\n", __func__);
