    ...
    )
{
    ...
    for (j=0; j<CFG_BATCH_MAX_MSCAN; j++) {
        ...
        for (i=0; i<pBr->ucScanCount; i++)
        {
            ...
            kalMemCopy(ssid, prEntry->aucSSID, prEntry->ucSSIDLen);
            ssid[prEntry->ucSSIDLen] = '\0';
            nsize2 = kalSnprintf(text2, TMP_TEXT_LEN_L, "ssid=%s\n",
                ssid);

            freq = batchChannelNum2Freq(prEntry->ucFreq);
            nsize3 = kalSnprintf(text3, TMP_TEXT_LEN_L, "freq=%d\nlevel=%d\ndist=%d\ndistSd=%d\n====\n", 
                freq,
                prEntry->cRssi,
                prEntry->u4Dist,
                prEntry->u4Distsd);
    ...
    )
{
    ...
    if(i4CmdFound == 0) {
        i4CmdFound = 1;
        if (strnicmp(pcCommand, CMD_BATCH_SET, strlen(CMD_BATCH_SET)) == 0) {
            kalIoctl(prGlueInfo,
            ...
            #if 0
            DBGLOG(SCN, TRACE, ("Batch Scan Results, scan count = %u\n", rEventBatchResult.ucScanCount));
            for (i=0; i<rEventBatchResult.ucScanCount; i++)
            {
                prEntry = &rEventBatchResult.arBatchResult[i];
                DBGLOG(SCN, TRACE, ("Entry %u\n", i));
                DBGLOG(SCN, TRACE, ("    BSSID = "MACSTR"\n", MAC2STR(prEntry->aucBssid)));
                DBGLOG(SCN, TRACE, ("    SSID = %s\n", prEntry->aucSSID));
                DBGLOG(SCN, TRACE, ("    SSID len = %u\n", prEntry->ucSSIDLen));
                DBGLOG(SCN, TRACE, ("    RSSI = %d\n", prEntry->cRssi));
                DBGLOG(SCN, TRACE, ("    Freq = %u\n", prEntry->ucFreq));
            }
            #endif
