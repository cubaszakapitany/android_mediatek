    ...
    case WLAN_STATUS_MEDIA_CONNECT:
        ...
        if(prGlueInfo->fgIsRegistered == TRUE) {
            ...
            if(bss == NULL) {
                ...
                if(prBssDesc != NULL) {
					UINT_8 *pucIEBuf = prBssDesc->aucIEBuf;
					UINT_8 ucSSIDLen;
					UINT_16 u2Offset;
					BOOLEAN fgNeedRelaceSSID = FALSE;
					IE_FOR_EACH(pucIEBuf, prBssDesc->u2IELength, u2Offset) {
						if (IE_ID(pucIEBuf) == ELEM_ID_SSID) {
							ucSSIDLen = IE_LEN(pucIEBuf);
							pucIEBuf += 2; /* move pucIEBuf to the head of ssid content */
							if (ucSSIDLen != 0) {
								UINT_8 i = 0;
								u2Offset = 0;
								for (; i<ucSSIDLen; i++) {
								...
								if (!u2Offset)
									fgNeedRelaceSSID = TRUE;
							} else
								fgNeedRelaceSSID = TRUE;
							break;
						}
					}
					/* if ssid is hidden in IEBuf of prBssDesc, then replace it with real ssid 
						this is to solve the connect fail issue for hidden ssid */
					if (prBssDesc->ucSSIDLen && fgNeedRelaceSSID) {			
						/* if real ssid len is different with ssid in IEBuf, then memmove to reserve space for ssid */
						DBGLOG(INIT, INFO, ("replace ssid\n"));
						//SW4_TRACE_LOGDUMP8(prBssDesc->aucIEBuf, prBssDesc->u2IELength);
						if (prBssDesc->ucSSIDLen != ucSSIDLen) {
							memmove(pucIEBuf+prBssDesc->ucSSIDLen, pucIEBuf+ucSSIDLen, 
								prBssDesc->u2IELength-(pucIEBuf-prBssDesc->aucIEBuf)-ucSSIDLen);
							prBssDesc->u2IELength = prBssDesc->u2IELength+prBssDesc->ucSSIDLen-ucSSIDLen;
							IE_LEN((pucIEBuf-2)) = prBssDesc->ucSSIDLen;
						}
						kalMemCopy(pucIEBuf, prBssDesc->aucSSID, prBssDesc->ucSSIDLen);
						//SW4_TRACE_LOGDUMP8(prBssDesc->aucIEBuf, prBssDesc->u2IELength);
					}
