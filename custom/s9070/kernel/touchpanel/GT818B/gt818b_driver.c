...
void tpd_reset_fuc(struct i2c_client *client)
{
   ...
   //power off, need confirm with SA
    hwPowerDown(TINNO_CUSTOMER_CTP_POWER,  "TP");
 //   hwPowerDown(MT65XX_POWER_LDO_VGP,  "TP");
	msleep(20);
	
   //power on, need confirm with SA
    hwPowerOn(TINNO_CUSTOMER_CTP_POWER, VOL_2800, "TP");
	...
	else
	{
		...
		else
		{
			...
			else if(key == 2)
			{
			 ...
			 //edit by Magnum 2012-11-12 just for adjust,should be optimized
			// tpd_down(TPD_CUST_KEY_X3, TPD_CUST_KEY_Y,12800,1);
				}
			else if(key == 4)
			{
			 //   TPD_DEBUG("Magnum return key....\n");
			    printk("Magnum return key....\n");
				tpd_down(TPD_CUST_KEY_X3, TPD_CUST_KEY_Y,12800,1);
			} 
		}
	}		
...
static int __init tpd_driver_init(void)
{
    ...
    i2c_register_board_info(TINNO_CUSTOMER_I2C_NO, &i2c_tpd, 1);
...
//fs_initcall_sync(tpd_driver_init);
//subsys_initcall(tpd_driver_init);
