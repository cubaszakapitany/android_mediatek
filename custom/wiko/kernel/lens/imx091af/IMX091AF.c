...
static bool g_s_bIsReleased = FALSE;    // LINE <><20140110><more delay to avoid hit> Jiangde++    
...
inline static int moveIMX091AF(unsigned long a_u4Position)
{
    ...
    // BEGIN <><20140110><more delay to avoid hit> Jiangde++   
    if (TRUE == g_s_bIsReleased)
    {
        spin_lock(&g_IMX091AF_SpinLock);
        g_s_bIsReleased = FALSE;
        spin_unlock(&g_IMX091AF_SpinLock);
        
        if (a_u4Position >= g_u4IMX091AF_MACRO / 4)
        {
            IMX091AFDB("HJDDbg, [IMX091AF] g_s_bIsReleased=TRUE, [curr]=%d, Move to [target]=%d\n", g_u4CurrPosition, a_u4Position);
            s4IMX091AF_WriteReg(a_u4Position / 8 * 4);
            s4IMX091AF_WriteReg(a_u4Position / 8 * 5);
            s4IMX091AF_WriteReg(a_u4Position / 8 * 6);
            msleep(10);
            s4IMX091AF_WriteReg(a_u4Position / 8 * 7);
            msleep(10);
            s4IMX091AF_WriteReg(a_u4Position / 8 * 8);
            msleep(10);
        }   
    }
    // BEGIN <><20140110><more delay to avoid hit> Jiangde++ 
...
static int IMX091AF_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if (g_s4IMX091AF_Opened)
    {
        IMX091AFDB("HJDDbg, [IMX091AF] feee IMX091AF_Release 1 \n");
        g_sr = 5;
        s4IMX091AF_WriteReg(300);   // LINE <><20140109><more delay to avoid hit> Jiangde++
        msleep(33);                 // LINE <><20140109><more delay to avoid hit> Jiangde++        
        s4IMX091AF_WriteReg(200);
        msleep(10);
        s4IMX091AF_WriteReg(100);
        msleep(10);
        IMX091AFDB("HJDDbg, [IMX091AF] after delay, IMX091AF_Release 2 \n");
            	            	    	    
        spin_lock(&g_IMX091AF_SpinLock);
        g_s4IMX091AF_Opened = 0;
        g_s_bIsReleased = TRUE;     // LINE <><20140110><more delay to avoid hit> Jiangde++    
        spin_unlock(&g_IMX091AF_SpinLock);
