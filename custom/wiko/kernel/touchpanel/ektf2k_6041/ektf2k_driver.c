...
//#include "OLV10_20140317_2KK_dijing_4011_v03.i"
};
static uint8_t file_fw_data_shenyue[] = {
#include "S401x_shen_OL_V10xx_V1011.i"
};
static uint8_t file_fw_data_huaruichuan[] = {
#include "S401x_hua_OL_V10xx_V1010.i"
};
static uint8_t file_fw_data_yeji[] = {
#include "S4011_ye_OL_V10xx_V1007.i"            
};
static uint8_t* file_fw_data = NULL;

//FW ID CHECK ----start by baojun.fu
#define FW_ID_DIJING					0x2925
#define FW_ID_SHENYUE					0x2926
#define FW_ID_HUARUICHUAN				0x2927
#define FW_ID_YEJI					0x4011
enum
{
         PageSize           = 132,
         PageNum            = 249,
...
//{ iap-ram
		case IOCTL_IAPRAM:
			if (copy_from_user(file_iapRam_data, (const void*)arg, 17*132))
			{
...
*/
        ...
        RECOVERY=0x80;
			...
			FW_ID =  buf_recv[5] << 8 | buf_recv[4];
			printkElan("[elan] FW_ID = %x\r\n", FW_ID);
        rc = i2c_master_recv(client, buf_recv, 8);
...
static int write_check_fwid_to_rom(struct i2c_client *client, uint8_t *cmd, size_t size)
{
	...
	printkElan("[elan] check cmd: %02x, %02x, %02x, %02x\n", cmd[0], cmd[1], cmd[2], cmd[3]);
    ...
    if( buf_recv[2] != cmd[2])//not fit for all FW ID then write it
    {
...
static void changeNumToChar(char *buf, int num){
	int i = 0, j = 0;
	int n = num;
	char tmp[50] = {0};
...
static int __fw_packet_handler(struct i2c_client *client)
{
	...
	uint8_t cmd_check_fwid[] = { 0x54, 0XD2, 0xFF,0xFF };	/* Get Check FWID *///LINE<elan tp><add dijing vendor><date20131108>yinhuiyong
...
// Bootcode version
         ...
         
		{
			...
			else if(FW_ID == FW_ID_SHENYUE){
				sprintf(tpd_desc, "%s","SHENYUE");
			}
			...
			else if(FW_ID == FW_ID_DIJING)
			{
				...
				char buf[50] = {"Unknow1"};
				changeNumToChar(buf, FW_ID);
				sprintf(tpd_desc, "%s", buf);
			}
       
			//printkElan("[elan] tpd_desc_show = %d\n",FW_VERSION);
			tpd_fw_version = FW_VERSION;
		}
         printkElan( "mtk-tpd:[elan] %s: firmware version: 0x%4.4x, firmware ID: 0x%4.4x\n",
                            __func__, FW_VERSION, FW_ID);
...
// iap-ram
	if (buf[0] == 0x55) 
	{
		//BEGIN<elan tp><add dijing vendor><date20131108>yinhuiyong
		//rc = __fw_packet_handler(client);
		//if (rc < 0){
		//   printkElan("mtk-tpd:[elan] %s, fw_packet_handler fail, rc = %d", __func__, rc);
		//}
		...
		//}
		//END<elan tp><add dijing vendor><date20131108>yinhuiyong
...
static int update_fw_handler(void *unused)
{
         ...
         while(probe_flage == 0)
         {
         	printkElan("[elan] wait probe_flage\n");
         	msleep(30);
		 }
...
/* FW ID & FW VER*/
    ...
    if(FW_ID == New_FW_ID)//RECOGNISE TP 
	{
		        ...
		        if (New_FW_VER > FW_VERSION) {
		            printkElan("FW_Version is different!");
		            Update_FW_One(private_ts->client, RECOVERY);
		    } else if(FW_VERSION == 0) {                       
		            printkElan("FW_Version is not read ok!");
		            RECOVERY=0x80;                  
		            Update_FW_One(private_ts->client, RECOVERY);           
		    }
	}
    ...
    //Update_FW_One(private_ts->client, RECOVERY);
//---------add by baojun.fu for iap finish		
	printkElan("[elan] iap_finish_flag----------set flag\n");
        
    power_lock=0;           
    work_lock=0;
    //enable_irq(CUST_EINT_TOUCH_PANEL_NUM);
		//RESET ic to read new resolution--2013Äê10ÔÂ23ÈÕ14:34:59
		mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);//mask INT
		printkElan("ektf2k reset afer IAP FINISH !\n");
    	mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ZERO);
    	mdelay(10);
    	mt_set_gpio_out( GPIO_CTP_RST_PIN, GPIO_OUT_ONE );
		msleep(300);
...
// End Firmware Update 
         ...
         if(1)
         {
		if((FW_ID == FW_ID_HUARUICHUAN)||(FW_ID == 0x14f0)) {
		     printkElan(KERN_ERR "[elan] %s: TP is FW_ID_HUARUICHUAN, FW_ID: 0x%4.4x \n", __func__, FW_ID);
		     file_fw_data = file_fw_data_huaruichuan;
		}
		else if((FW_ID == FW_ID_SHENYUE) || (FW_ID == 0x4010)){
			file_fw_data = file_fw_data_shenyue;
			 printkElan(KERN_ERR "[elan] %s: TP is FW_ID_SHENYUE, FW_ID: 0x%4.4x \n", __func__, FW_ID);
		}
		else if ((FW_ID == FW_ID_YEJI)) {
		     printkElan(KERN_ERR "[elan] %s: TP is FW_ID_YEJI, FW_ID: 0x%4.4x \n", __func__, FW_ID);
		     file_fw_data = file_fw_data_yeji;
		}
		else if (FW_ID == FW_ID_DIJING) {
		     printkElan(KERN_ERR "[elan] %s: TP is FW_ID_DIJING, FW_ID: 0x%4.4x \n", __func__, FW_ID);
		     file_fw_data = file_fw_data_dijing;
		}
		else if (FW_ID == 0xd404) {
		     printkElan(KERN_ERR "[elan] %s: TP is FW_ID_DIJING, FW_ID: 0x%4.4x \n", __func__, FW_ID);
		     file_fw_data = file_fw_data_dijing;
		}
		else {
		     printkElan(KERN_ERR "[elan] TP ID is error: no support!\n");
		     //return 1;//TP ID is error
		}
...
{	
	...
	if((have_interrupts == 1)||(1==PROXIMITY)){
	...
	else
    {
        ...
        {
            por_cnt = 0;
			printkElan("[elan esd] %s: i'm alive failed need reset!\n", __func__);
            //reset here
			mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
            mt_set_gpio_dir(GPIO_CTP_RST_PIN, 1);
            mt_set_gpio_out(GPIO_CTP_RST_PIN, 0);
        ...
        else
        {
            ...
            if (res != sizeof(cmd)){
                printkElan("[elan esd] %s: i2c_master_send failed reset now\n", __func__);
                //reset here
                mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
                mt_set_gpio_dir(GPIO_CTP_RST_PIN, 1);
                mt_set_gpio_out(GPIO_CTP_RST_PIN, 0);
            ...
            {
                printkElan("[elan esd] %s: i2c_master_send successful\n", __func__);
                
                msleep(20);
                
                if(have_interrupts == 1){
                    printkElan("[elan esd] %s: i2c_master_send successful, had response\n", __func__);
                }
                else
                {
                    printkElan("[elan esd] %s: i2c_master_send successful, no response need reset\n", __func__);
                    //reset here
                    mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
                    mt_set_gpio_dir(GPIO_CTP_RST_PIN, 1);
                    mt_set_gpio_out(GPIO_CTP_RST_PIN, 0);
...
static int tpd_suspend(struct i2c_client *client, pm_message_t message)
{
	...
	if (work_lock == 1)//if update FW stop suspend 2014/1/3
	{
	 ...
	 {
	     PROXIMITY_SLEEP = 1;
         printkElan("mtk-tpd:[elan] TP enter into sleep mode  ps return!\n");
		 return ;
	 }
...
static int tpd_resume(struct i2c_client *client)
{
 ...
 if( PROXIMITY== 1)//PROXIMITY_SLEEP
 {
     ...
     {
	     PROXIMITY_SLEEP = 0;
         printkElan("mtk-tpd:[elan]tpd_resume TPD wake up ps return!\n");
         return ;
     }
 } 
#endif //TP_PROXIMITY_SENSOR_NEW
...
#ifdef TPD_HAVE_BUTTON
	printkElan("tpd_local_init FW_ID:%d\n", FW_ID);
