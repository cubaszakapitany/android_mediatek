...
static int ltr501_ps_enable(int gainrange)
{
	...
	error = ltr501_i2c_write_reg(LTR501_PS_LED, 0x7b); //0x63//0x7b
	if(error<0)
    	{
	...
	APS_LOG("#####ltr501_ps_enable , obj->hw->polling_mode_ps:%d\n",obj->hw->polling_mode_ps);
	...
	/*for interrup work mode support -- by liaoxl.lenovo 12.08.2011*/
		...
		{		
			...
			ps_state = ltr501_i2c_read_reg(LTR501_INTERRUPT_PERSIST);
 			APS_LOG("#####ltr501_ps_enable ...OK!, LTR501_INTERRUPT_PERSIST:%d\n",ps_state);
	...
	ps_state = ltr501_i2c_read_reg(LTR501_PS_CONTR);
 	APS_LOG("ltr501_ps_enable ...OK!, ps_enable_state:%d\n",ps_state);
...
static int ltr501_ps_read(void)
{
    ...
    APS_DBG("ps_rawdata = %d %x %x\n", psdata, psval_hi, psval_lo);
...
static int ltr501_get_ps_value(struct ltr501_priv *obj, u16 ps)
{
	...
	APS_LOG("###ltr501_get_ps_value:ps:%d\n,obj->ps_thd_val_high=%d,obj->ps_thd_val_low=%d \n",
		ps, obj->ps_thd_val_high, obj->ps_thd_val_low);
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_ALS_RAW_DATA:    
            ...
            case ALSPS_IOCTL_PS_SET_CALI:
                ...
                if(!g_ps_cali_flag)
                {
                    g_ps_base_value = 134; // set default base value
                }
...
static int ltr501_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	APS_LOG("ltr501_i2c_probe:value_high=%x,value_low=%x! \n",obj->ps_thd_val_high,obj->ps_thd_val_low);
