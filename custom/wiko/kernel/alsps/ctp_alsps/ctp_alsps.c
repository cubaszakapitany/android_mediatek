...
static int ctp_power_down( int enable);
...
int ctp_read_data( u8 *data)
{
	...
	if((g_tpd_drv)&&(g_tpd_drv->tpd_get_ps!=NULL))
	{
...
{
    u8 regdata=0;
    if((g_tpd_drv)&&(g_tpd_drv->tpd_convert!=NULL))
    {
    APS_LOG("ctp_power_down!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    	if(enable)
        	g_tpd_drv->tpd_convert(0,false);  
		else
			g_tpd_drv->tpd_convert(1,false);
    }
	else
		APS_LOG("ctp_power_down err !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	APS_LOG("##### PS SENSOR_OPERATE %x %x %x!!\n", self, buff_in, buff_out);
