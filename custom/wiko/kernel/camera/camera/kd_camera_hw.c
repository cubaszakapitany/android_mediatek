...
//       if(pinSetIdx == 0) {   //v0.3
        ...
        mdelay(10);
        //DVDD
        if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5648_MIPI_RAW,currSensorName)) ||currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5649_MIPI_RAW,currSensorName))){
		...
		if(mt_set_gpio_mode(GPIO_CAMERA_LDO_D1050V_EN_PIN, GPIO_CAMERA_LDO_EN_PIN_M_GPIO)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN mode failed!! \n");}
		if (mt_set_gpio_pull_enable(GPIO_CAMERA_LDO_D1050V_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[[CAMERA SENSOR] Set CAMERA_POWER_PULL_PIN DISABLE ! \n"); }
		if(mt_set_gpio_dir(GPIO_CAMERA_LDO_D1050V_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN dir failed!! \n");}
		if(mt_set_gpio_out(GPIO_CAMERA_LDO_D1050V_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN failed!! \n");}
	}
	else{
...
/*		if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX135_MIPI_RAW,currSensorName))){
	 ...
	 else
	 {
	   ...
	   //     PK_DBG("Set CAMERA_POWER OFF _1.05vDVDD_PIN\n");
		...
		if (mt_set_gpio_pull_enable(GPIO_CAMERA_LDO_D1050V_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[CAMERA SENSOR] Set GPIO94 PULL DISABLE ! \n"); }
  		if(mt_set_gpio_mode(GPIO_CAMERA_LDO_D1050V_EN_PIN, GPIO_CAMERA_LDO_EN_PIN_M_GPIO)){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
  		if(mt_set_gpio_dir(GPIO_CAMERA_LDO_D1050V_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
  		if(mt_set_gpio_out(GPIO_CAMERA_LDO_D1050V_EN_PIN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
	}
	   else
	{
		if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, mode_name)) {
