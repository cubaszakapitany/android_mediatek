...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
    ...
    PK_DBG("kdCISModulePowerOn: 4EC 8AA### currSensorName:%s, SensorIdx:%d\n", currSensorName, SensorIdx);
...
//main sensor: (hi257 , sp0a19, sp2519),  sub sensor: sp0a18.
	if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2519_YUV, currSensorName)) 
		&& (DUAL_CAMERA_SUB_SENSOR == SensorIdx)){
		return 0;
	}
	...
	if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP0A19_YUV, currSensorName)) 
		&& (DUAL_CAMERA_SUB_SENSOR == SensorIdx)){
		return 0;
	}
	if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP0A18_YUV, currSensorName)) 
		&& (DUAL_CAMERA_MAIN_SENSOR == SensorIdx)){
		return 0;
	}
	...
	if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_HI257_YUV, currSensorName)) 
		|| (0 == strcmp(SENSOR_DRVNAME_SP2519_YUV, currSensorName)))
		&& (DUAL_CAMERA_MAIN_SENSOR == SensorIdx)) 
       {
	   	...
	   	||(0 == strcmp(SENSOR_DRVNAME_SP0A18_YUV,currSensorName))))
       {  
		   PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in--- SP0A19_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
          ...
          if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))
          {
	   	  ...
	   	  if(currSensorName && ((0 == strcmp(SENSOR_DRVNAME_SP0A19_YUV,currSensorName)) 
	   		||(0 == strcmp(SENSOR_DRVNAME_SP0A18_YUV,currSensorName))))
	   	  {
		  	//spa019 no dvdd power
	   	  	PK_DBG("[CAMERA SENSOR] SP0A19_YUV no digital power\n");
	   	  }
		  else
		  {
	          if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_SUB_D, VOL_1800,mode_name)) //VOL_1200
	          {
	             PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
	             //return -EIO;
	             goto _kdCISModulePowerOn_exit_;
	   ...
	   {
	       if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_SUB_D, mode_name)) {
