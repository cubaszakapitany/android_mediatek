...
#define GC0329_OP_CODE_END		0x03		/* End of initial setting. */
	...
	u8 GC0329_Initialize_from_T_Flash()
	{
		...
		fp = filp_open("/storage/sdcard0/gc0329_sd.dat", O_RDONLY , 0); 
		if (IS_ERR(fp)) { 
			printk("create file error\n"); 
			return 2;//-1; 
		} 
		else
		printk("GC0329_Initialize_from_T_Flash Open File Success\n");
		...
		for (j=0; j<i; j++)
		{
			...
			{
				GC0329_write_cmos_sensor((kal_uint8)GC0329_Init_Reg[j].init_reg, (kal_uint8)GC0329_Init_Reg[j].init_val);
			}
			else
			{
				printk("REG ERROR!\n");
...
void GC0329NightMode(kal_bool bEnable)
{
	...
	else 
	{
		...
		GC0329_write_cmos_sensor(0xfe, 0x00);
		GC0329GammaSelect(GC0329_RGB_Gamma_m6);//m4 betty
...
void GC0329_Sensor_Init(void)
{
	...
	GC0329_write_cmos_sensor(0x73, 0x80);//channel R gain 90 betty
	GC0329_write_cmos_sensor(0x74, 0x80);//channel G1 gain
	GC0329_write_cmos_sensor(0x75, 0x80);//channel G2 gain
	GC0329_write_cmos_sensor(0x76, 0x94);//channel B gain

	GC0329_write_cmos_sensor(0x42, 0x00);
	GC0329_write_cmos_sensor(0x77, 0x50);//57 betty
	GC0329_write_cmos_sensor(0x78, 0x4d);
	GC0329_write_cmos_sensor(0x79, 0x45);

	GC0329_write_cmos_sensor(0x03, 0x01);//add betty
	GC0329_write_cmos_sensor(0x04, 0x80);
	...
	GC0329_write_cmos_sensor(0x29, 0x00); //80 betty
	GC0329_write_cmos_sensor(0x32, 0x00);//04 betty
	GC0329_write_cmos_sensor(0x33, 0x20);
	...
	GC0329_write_cmos_sensor(0x41, 0x24);//44 betty
	...
	////////////////////INTPEE////////////////////
	GC0329_write_cmos_sensor(0x90, 0x00);//add betty
	...
	GC0329_write_cmos_sensor(0xa2, 0x11);//ff betty
	GC0329_write_cmos_sensor(0xa4, 0x30);//50 betty
	GC0329_write_cmos_sensor(0xa5, 0x31);//31 betty
	...
	GC0329_write_cmos_sensor(0xb3, 0x44);//44  betty
	GC0329_write_cmos_sensor(0xb4, 0x00);//fd betty
	GC0329_write_cmos_sensor(0xb5, 0x05);//02 betty
	GC0329_write_cmos_sensor(0xb6, 0xfa);
	GC0329_write_cmos_sensor(0xb7, 0x48);//48 betty
	...
	////////////////////YCP////////////////////
	GC0329_SET_PAGE0;
	GC0329_write_cmos_sensor(0xd1, 0x30);//34 betty
	GC0329_write_cmos_sensor(0xd2, 0x30);
	GC0329_write_cmos_sensor(0xd3, 0x3d);
	GC0329_write_cmos_sensor(0xd5, 0x03);
	...
	GC0329_write_cmos_sensor(0x13, 0x50);//50 betty
	GC0329_write_cmos_sensor(0x17, 0x88);
	GC0329_write_cmos_sensor(0x21, 0xe8);//b0 betty
	GC0329_write_cmos_sensor(0x22, 0x48);
	GC0329_write_cmos_sensor(0x3c, 0xe0);
	GC0329_write_cmos_sensor(0x3d, 0x80);
	GC0329_write_cmos_sensor(0x3e, 0x30); 
	...
	GC0329_write_cmos_sensor(0x5c, 0x37);//34 betty
	GC0329_write_cmos_sensor(0x5d, 0x73);
	GC0329_write_cmos_sensor(0x5e, 0x01);
	...
	GC0329_write_cmos_sensor(0x64, 0x68);
	GC0329_write_cmos_sensor(0x65, 0x98);
	GC0329_write_cmos_sensor(0x66, 0xfa);
	GC0329_write_cmos_sensor(0x67, 0x68);//70 betty
	GC0329_write_cmos_sensor(0x68, 0x58);
	GC0329_write_cmos_sensor(0x69, 0x80); //85 jaambo
	GC0329_write_cmos_sensor(0x6a, 0x40);
	GC0329_write_cmos_sensor(0x6b, 0x39);
	GC0329_write_cmos_sensor(0x6c, 0x20);//18 betty
	GC0329_write_cmos_sensor(0x6d, 0x40);//28 betty
	GC0329_write_cmos_sensor(0x6e, 0x41);
	GC0329_write_cmos_sensor(0x70, 0x10);//02 betty
	GC0329_write_cmos_sensor(0x71, 0x00);
	GC0329_write_cmos_sensor(0x72, 0x30);
	...
	////////////////////ABS////////////////////
	GC0329_write_cmos_sensor(0x9c, 0x00); //02 betty
	...
	GC0329_write_cmos_sensor(0x44, 0xa2);
	GC0329GammaSelect(GC0329_RGB_Gamma_m6);//0:use default m4 betty
...
UINT32 GC0329Open(void)
{
    ...
    if(fromsd == 1)//¡§o??¡è???¡§?SD?¡§¡é¡§¡§?//gepeiwei   120903
        GC0329_Initialize_from_T_Flash();//??¡§?SD??¡ì?¡§¡é¡§¡§?|¨¬????¡§¡ãao?£¤¡§oy
    else
    {
        GC0329_Sensor_Init();
        GC0329_Write_More_Registers();
        ...
        MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->PreviewDelayFrame = 4;//00 betty
...
BOOL GC0329_set_param_wb(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AWB_MODE_DAYLIGHT: //sunny
			GC0329_awb_enable(KAL_FALSE);
			GC0329_write_cmos_sensor(0x77, 0x90); 
			GC0329_write_cmos_sensor(0x78, 0x80);   
			GC0329_write_cmos_sensor(0x79, 0x40);			
		break;
		...
		case AWB_MODE_INCANDESCENT: //office
			GC0329_awb_enable(KAL_FALSE);
			GC0329_write_cmos_sensor(0x77, 0x40);       
			GC0329_write_cmos_sensor(0x78, 0x68);
			GC0329_write_cmos_sensor(0x79, 0xa0);       
		break;
...
BOOL GC0329_set_param_exposure(UINT16 para)
{
	...
	value_luma = (GC0329_NIGHT_MODE?0x2b:0x03);
	value_Y = (GC0329_NIGHT_MODE?0x68:0x50);//50 betty
        ...
        UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	{/*MAX LEN is 31 char*/
		*pFeaturePara = '\0';
	        strcat(pFeaturePara, SENSOR_DRVNAME_GC0329_YUV );
	        strcat(pFeaturePara, "_" );
	        strcat(pFeaturePara, vendor );
