...
MINT32 i = 0;
    ...
    for ( i = (KDIMGSENSOR_MAX_INVOKE_DRIVERS-1) ; i >= KDIMGSENSOR_INVOKE_DRIVER_0 ; i-- ) {
        ...
        PK_DBG("[kd_sensorlist] kd_MultiSensorOpen: g_bEnableDriver[%d]=%d, g_pInvokeSensorFunc[%d]=%x\n", 
                i, g_bEnableDriver[i], i, g_pInvokeSensorFunc[i]);
		
        if ( g_bEnableDriver[i] && g_pInvokeSensorFunc[i] ) {

            PK_DBG("[kd_sensorlist] kd_MultiSensorOpen: [%d] socketidx=%d %d, SensorNameStr=%s %s\n", i,
...
int kdSetDriver(unsigned int* pDrvIndex)
{
    ...
    for ( i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
		PK_DBG("[kd_sensorlist]: ###pSensorList :%s, func:%s\n", pSensorList[i].drvname, pSensorList[i].SensorInit);  
		
        spin_lock(&kdsensor_drv_lock);
        g_bEnableDriver[i] = FALSE;
        g_invokeSocketIdx[i] = 
		(CAMERA_DUAL_CAMERA_SENSOR_ENUM)((pDrvIndex[i] & KDIMGSENSOR_DUAL_MASK_MSB)>>KDIMGSENSOR_DUAL_SHIFT);
	spin_unlock(&kdsensor_drv_lock);
        drvIdx[i] = (pDrvIndex[i] & KDIMGSENSOR_DUAL_MASK_LSB);

        //

PK_DBG("[kd_sensorlist]: ###g_invokeSocketIdx[%d]:%d\n", i, g_invokeSocketIdx[i]);
PK_DBG("[kd_sensorlist]: ###drvIdx[%d]:%d\n", i, drvIdx[i]);

		
        if ( DUAL_CAMERA_NONE_SENSOR == g_invokeSocketIdx[i] ) { 
		printk("kdSetDriver:####DUAL_CAMERA_NONE_SENSOR %d, sensor:%s\n", i, pSensorList[i].drvname);
		continue; 
	}
...
#ifdef CUSTOM_KERNEL_FLASHLIGHT
...
{
    printk("camera infomation1 : %s\n", (char*)g_invokeSensorNameStr[0]);

    if(g_invokeSensorNameStr[1] != NULL){
	printk("camera infomation2 : %s\n", (char*)g_invokeSensorNameStr[1]);
    }
