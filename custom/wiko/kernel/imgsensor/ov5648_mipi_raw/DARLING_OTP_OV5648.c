...
bool otp_update_wb_ov5648(unsigned short golden_rg, unsigned short golden_bg),
...
{   
    ...
    TRACE("otp_check_wb_group.flag:0x%x\n", flag);
...
signed char check_darlingMID(signed char index)
	{
...
*/
		...
		for (index=0; index<3; index++)
		{
			if (otp_check_wb_group(index) == 2)
			{
				...
				TRACE("check_daringMID sucessed %d", index);		
