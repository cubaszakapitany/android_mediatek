...
static void OV5648MIPI_Preview_Setting(void)
{
    ...
    OV5648MIPI_write_cmos_sensor(0x3815, 0x31); // y inc
    OV5648MIPI_write_cmos_sensor(0x3817, 0x00); // hsync start
    OV5648MIPI_write_cmos_sensor(0x3820, 0x0a); // flip off, v bin off
    OV5648MIPI_write_cmos_sensor(0x3821, 0x05); // mirror on, h bin on
...
static void OV5648MIPI_Capture_Setting(void)
{
    ...
    OV5648MIPI_write_cmos_sensor(0x3817, 0x00); // hsync start
    OV5648MIPI_write_cmos_sensor(0x3820, 0x42); // flip off, v bin off
    OV5648MIPI_write_cmos_sensor(0x3821, 0x04); // mirror on, v bin off
...
UINT32 OV5648MIPIOpen(void)
{
    ...
    otp_update_wb_ov5648(0x177,0x15d);//darling
...
UINT32 OV5648GetSensorID(UINT32 *sensorID) 
{
    ...
    *sensorID=((OV5648MIPI_read_cmos_sensor(0x300A) << 8) | OV5648MIPI_read_cmos_sensor(0x300B));   
    ...
    if (*sensorID != OV5648_SENSOR_ID) {
     ...
     if( 0==check_darlingMID(0) ){
     	   SENSORDB("check_darlingMIDFailed\n");  
          *sensorID = 0xFFFFFFFF;
	    return ERROR_SENSOR_CONNECT_FAIL;	  
     }
     else{
	    *sensorID = OV5648_SENSOR_ID;    
     }
    ...
    SENSORDB("Darling Sensor ID: 0x%x ", *sensorID);
                             ...
                             UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            ...
            ((PNVRAM_SENSOR_DATA_STRUCT)pFeaturePara)->SensorId = OV5648_SENSOR_ID;
	...
	case SENSOR_FEATURE_GET_SENSOR_NAME:
		{
			*pFeaturePara = '\0';
			...
			strcat(pFeaturePara, OV5648MIPI_sensor.vendor);
