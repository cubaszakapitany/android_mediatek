...
//    uint arr[384]={0};
    ...
    arr=(uint *)kmalloc(384*4, GFP_KERNEL);
    if (arr == NULL)
    {
        printk("[Sunny] read_otp_lenc malloc memory fail\n");
        return 1;
    }
...
int update_otp_wb(void)
{
    ...
    current_otp=(struct otp_struct *)kmalloc(sizeof(struct otp_struct), GFP_KERNEL);
    if (current_otp == NULL)
    {
        printk("[Sunny] read_otp_lenc malloc memory fail\n");
        return 1;
...
int get_sensor_customer_id()
{
   ...
   camera_id_volt/=1000;          //volt uV to mV
    ...
    printk("[get_sensor_imx135_customer_id]sunny res:%d, adcVol:%d mV\n", res, adcVol);
    if ((600 < adcVol) && (adcVol < 1200))			//truly ID volt is 0.9V. shunyu is 0V
    {
	    customer_id=2;
    }
    else if(camera_id_volt< 300)
