...
void IMX179MIPI_write_shutter(kal_uint16 shutter)
{
	...
	unsigned long flags;	
	SENSORDB("[IMX179MIPI]enter IMX179MIPI_write_shutter function shutter =%d\n",shutter); 
...
{
	kal_uint8 iI;
    SENSORDB("[IMX179MIPI]enter IMX179MIPIGain2Reg function iGain =%d\n",iGain);
...
{   
    kal_uint8 iReg;
	SENSORDB("[IMX179MIPI]enter IMX179MIPI_SetGain function iGain=%d\n",iGain);
...
//	     }
    ...
    strcpy(IMX179MIPI_sensor.vendor, "Sunny");   
...
void IMX179MIPI_SetShutter(kal_uint16 iShutter)
{
	...
	IMX179MIPI_sensor.video_shutter = iShutter;
	spin_unlock_irqrestore(&imx179_drv_lock,flags);
...
{
    SENSORDB("[IMX179MIPI] enter IMX179MIPISetTestPatternMode function bEnable=%d\n", bEnable);
    
    if(bEnable) // enable test patten output
	{   
	...
	else // disable test patten output
	{   
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	IMX179MIPISetFlipMirror(IMAGE_HV_MIRROR);
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
		...
		{
			*pFeaturePara = '\0';
			strcat(pFeaturePara, SENSOR_DRVNAME_IMX179_MIPI_RAW);
			strcat(pFeaturePara, "_" );
			strcat(pFeaturePara, IMX179MIPI_sensor.vendor);
