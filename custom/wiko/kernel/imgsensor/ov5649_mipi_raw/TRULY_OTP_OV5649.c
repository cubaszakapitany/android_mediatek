...
#include "ov5649_Sensor.h"
#include "ov5649_Camera_Sensor_para.h"
#include "ov5649_CameraCustomized.h"
*/
...
//extern kal_uint16 OV5649_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
//extern kal_uint16 OV5649_read_cmos_sensor(kal_uint32 addr);


#undef TRACE
//#define TRACE(fmt, args...) printk(KERN_INFO "ov5649_OTP.c: " fmt, ## args)
#define TRACE(fmt, arg...) printk("[OV5649MIPIRaw_OTP]%s: " fmt "\n", __FUNCTION__ ,##arg)//LINE <> <DATE20130923> <ov5649 OTP log> wupingzhou
...
//#define OV5649_write_cmos_sensor(kal_uint32 addr, kal_uint32 para) OV5649MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
//#define OV5649_read_cmos_sensor(kal_uint32 addr) OV5649MIPI_read_cmos_sensor(kal_uint32 addr)

#define OV5649_write_cmos_sensor(addr,para) OV5649MIPI_write_cmos_sensor(addr,para)
#define OV5649_read_cmos_sensor(addr) OV5649MIPI_read_cmos_sensor(addr)
...
{
	OV5649_write_cmos_sensor(OTP_LOAD_ADDR, 0x01);
...
{
	OV5649_write_cmos_sensor(OTP_LOAD_ADDR, 0x00);
}

void otp_read(unsigned short otp_addr, unsigned char* otp_data)
{
	otp_read_enable();
	*otp_data = OV5649_read_cmos_sensor(otp_addr);
	otp_read_disable();
}
...
void otp_clear(void)
{
	...
	for (i=0; i<16; i++) 
	{
		OV5649_write_cmos_sensor(OTP_DATA_ADDR+i, 0x00);
	}
}
...
{   
    	...
    	if (index > 2)
	{
   	...
   	{
   		// select bank 0
   		OV5649_write_cmos_sensor(OTP_BANK_ADDR, 0xc0);
   		OV5649_write_cmos_sensor(OTP_BANK, 0x00);
   		OV5649_write_cmos_sensor(OTP_END_ADDR, 0x0f);	
   	}
   	else /*index==2*/
   	{
   		// select bank 1
   		OV5649_write_cmos_sensor(OTP_BANK_ADDR, 0xc0);
   		OV5649_write_cmos_sensor(OTP_BANK, 0x10);
   		OV5649_write_cmos_sensor(OTP_END_ADDR, 0x1f);	
   		otp_addr = 0x3D07;
   	}
...
bool otp_apply_wb(unsigned short golden_rg, unsigned short golden_bg)
{
	...
	if (ratio_r != 1)
	{
		...
		OV5649_write_cmos_sensor(GAIN_RH_ADDR, gain_r >> 8);
		OV5649_write_cmos_sensor(GAIN_RL_ADDR, gain_r & 0x00ff);
	}

	if (ratio_g != 1)
	{
		gain_g = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_g);
		OV5649_write_cmos_sensor(GAIN_GH_ADDR, gain_g >> 8);
		OV5649_write_cmos_sensor(GAIN_GL_ADDR, gain_g & 0x00ff);
	}

	if (ratio_b != 1)
	{
		gain_b = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_b);
		OV5649_write_cmos_sensor(GAIN_BH_ADDR, gain_b >> 8);
		OV5649_write_cmos_sensor(GAIN_BL_ADDR, gain_b & 0x00ff);
	}
...
bool otp_apply_wb(unsigned short golden_rg, unsigned short golden_bg)
{
	...
	TRACE("------golden_rg=0x%x, golden_bg=0x%x\n", golden_rg, golden_bg);//LINE <> <DATE20130923> <ov5649 OTP log> wupingzhou
	TRACE("------current rg_ratio=0x%x, bg_ratio=0x%x\n", rg_ratio, bg_ratio);//LINE <> <DATE20130923> <ov5649 OTP log> wupingzhou
...
*/
 		//TRACE("the value of 0x0103 register is  0x%x",OV5649_read_cmos_sensor(0x0103));		
		//mdelay(1);
		//TRACE("the value of 0x0100 register is  0x%x",OV5649_read_cmos_sensor(0x0100));		
		//mdelay(1);

		//enable to read the otp infomation
		OV5649_write_cmos_sensor(0x0103, 0x01);
		OV5649_write_cmos_sensor(0x0100, 0x01);
		mdelay(10);
		...
		for (index=0; index<3; index++)
		{
			if (otp_check_wb_group(index) == 2)
			{
				...
				OV5649_write_cmos_sensor(0x0103, 0x00);
				OV5649_write_cmos_sensor(0x0100, 0x00);
				mdelay(10);
				return 1;
			}
