...
kal_bool OV5649MIPIDuringTestPattern = KAL_FALSE;
#define OV5649MIPI_TEST_PATTERN_CHECKSUM (0xc24ec08d)
...
void OV5649MIPISetMaxFrameRate(UINT16 u2FrameRate);
...
void OV5649MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
...
void OV5649MIPI_Set_Mirror_Flip(kal_uint8 image_mirror)
{
	...
	{
		case IMAGE_NORMAL:
		    OV5649MIPI_write_cmos_sensor(0x3820,((OV5649MIPI_read_cmos_sensor(0x3820) & 0xF9) | 0x00));
		    OV5649MIPI_write_cmos_sensor(0x3821,((OV5649MIPI_read_cmos_sensor(0x3821) & 0xF9) | 0x06));
		    break;
		case IMAGE_H_MIRROR:
		    OV5649MIPI_write_cmos_sensor(0x3820,((OV5649MIPI_read_cmos_sensor(0x3820) & 0xF9) | 0x00));
		    OV5649MIPI_write_cmos_sensor(0x3821,((OV5649MIPI_read_cmos_sensor(0x3821) & 0xF9) | 0x00));
		    break;
		case IMAGE_V_MIRROR:
		    OV5649MIPI_write_cmos_sensor(0x3820,((OV5649MIPI_read_cmos_sensor(0x3820) & 0xF9) | 0x06));
		    OV5649MIPI_write_cmos_sensor(0x3821,((OV5649MIPI_read_cmos_sensor(0x3821) & 0xF9) | 0x06));		
		    break;
...
static void OV5649MIPI_Sensor_Init(void)
{
    ...
    */
    ...
    // fix EV3 issue
    OV5649MIPI_write_cmos_sensor(0x3704, 0xc0); // analog control
    ...
    OV5649MIPI_write_cmos_sensor(0x4800, 0x14); // MIPI line sync enable
...
static void OV5649MIPI_Preview_Setting(void)
{
    ...
    OV5649MIPI_write_cmos_sensor(0x0100, 0x00); //Stream Off 
    ...
    //OV5649MIPI_write_cmos_sensor(0x380e, 0x03); // vts = 992
    //OV5649MIPI_write_cmos_sensor(0x380f, 0xE0); // vts
    OV5649MIPI_write_cmos_sensor(0x380c, ((OV5649MIPI_PV_PERIOD_PIXEL_NUMS >> 8) & 0xFF)); // hts = 2816
    OV5649MIPI_write_cmos_sensor(0x380d, (OV5649MIPI_PV_PERIOD_PIXEL_NUMS & 0xFF));        // hts
    OV5649MIPI_write_cmos_sensor(0x380e, ((OV5649MIPI_PV_PERIOD_LINE_NUMS >> 8) & 0xFF));  // vts = 992
    OV5649MIPI_write_cmos_sensor(0x380f, (OV5649MIPI_PV_PERIOD_LINE_NUMS & 0xFF));         // vts
    ...
    OV5649MIPI_write_cmos_sensor(0x3815, 0x31); // y inc
    OV5649MIPI_write_cmos_sensor(0x3817, 0x00); // hsync start
    OV5649MIPI_write_cmos_sensor(0x3820, 0x0a); // flip off, v bin off
    OV5649MIPI_write_cmos_sensor(0x3821, 0x05); // mirror on, h bin on
    OV5649MIPI_write_cmos_sensor(0x4004, 0x02); // black line number
    //OV5649MIPI_write_cmos_sensor(0x4005, 0x1a); // blc normal freeze
    OV5649MIPI_write_cmos_sensor(0x4005, 0x18); // blc normal freeze
    OV5649MIPI_write_cmos_sensor(0x4050, 0x6e); // blc normal freeze
...
static void OV5649MIPI_Capture_Setting(void)
{
    ...
    OV5649MIPI_write_cmos_sensor(0x380c, ((OV5649MIPI_FULL_PERIOD_PIXEL_NUMS >> 8) & 0xFF)); // hts = 2816
    OV5649MIPI_write_cmos_sensor(0x380d, (OV5649MIPI_FULL_PERIOD_PIXEL_NUMS & 0xFF));        // hts
    OV5649MIPI_write_cmos_sensor(0x380e, ((OV5649MIPI_FULL_PERIOD_LINE_NUMS >> 8) & 0xFF));  // vts = 1984
    OV5649MIPI_write_cmos_sensor(0x380f, (OV5649MIPI_FULL_PERIOD_LINE_NUMS & 0xFF));         // vts
    ...
    OV5649MIPI_write_cmos_sensor(0x3817, 0x00); // hsync start
    OV5649MIPI_write_cmos_sensor(0x3820, 0x42); // flip off, v bin off
    OV5649MIPI_write_cmos_sensor(0x3821, 0x04); // mirror on, v bin off
...
if (sensor_id != OV5648_SENSOR_ID){
    ...
    otp_update_wb(0x14b, 0x142);

    spin_lock(&ov5649mipi_drv_lock);
    OV5649MIPIDuringTestPattern = KAL_FALSE;    
...
UINT32 OV5649GetSensorID(UINT32 *sensorID) 
{
    ...
    *sensorID=((OV5649MIPI_read_cmos_sensor(0x300A) << 8) | OV5649MIPI_read_cmos_sensor(0x300B));   
    ...
    if (*sensorID != OV5648_SENSOR_ID) {      
        // if Sensor ID is not correct, Must set *sensorID to 0xFFFFFFFF 
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }

     if( 0==check_trulyMID(0) ){
          SENSORDB("check_trulyMIDFailed\n");
	   *sensorID = 0xFFFFFFFF;
	   return ERROR_SENSOR_CONNECT_FAIL;
     }
     else{
          *sensorID = OV5649_SENSOR_ID;  
     }

     spin_lock(&ov5649mipi_drv_lock);
     strcpy(OV5649MIPI_sensor.vendor, "Truly");  
     spin_unlock(&ov5649mipi_drv_lock);
	
     SENSORDB("Truly Sensor ID: 0x%x ", *sensorID);
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    OV5649MIPI_sensor.video_mode = KAL_FALSE;
    OV5649MIPI_sensor.line_length = OV5649MIPI_FULL_PERIOD_PIXEL_NUMS;
    OV5649MIPI_sensor.frame_length = OV5649MIPI_FULL_PERIOD_LINE_NUMS;     
    spin_unlock(&ov5649mipi_drv_lock);
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if(OV5649MIPI_SENSOR_MODE_CAPTURE != OV5649MIPI_sensor.ov5649mipi_sensor_mode)
    {
        OV5649MIPI_Capture_Setting();
    }
...
UINT32 OV5649MIPIGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
    ...
    pSensorResolution->SensorVideoWidth = OV5649MIPI_IMAGE_SENSOR_PV_WIDTH;
    pSensorResolution->SensorVideoHeight = OV5649MIPI_IMAGE_SENSOR_PV_HEIGHT;		
...
UINT32 OV5649MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) 
{
    kal_uint32 pclk;
	kal_int16 dummyLine;
    ...
    switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			pclk = OV5649MIPI_PREVIEW_CLK;
			lineLength = OV5649MIPI_PV_PERIOD_PIXEL_NUMS;
			frameHeight = (10 * pclk)/frameRate/lineLength;
			dummyLine = frameHeight - OV5649MIPI_PV_PERIOD_LINE_NUMS;
			OV5649MIPI_Set_Dummy(0, dummyLine);			
			break;			
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			pclk = OV5649MIPI_VIDEO_CLK;
			lineLength = OV5649MIPI_VIDEO_PERIOD_PIXEL_NUMS;
			frameHeight = (10 * pclk)/frameRate/lineLength;
			dummyLine = frameHeight - OV5649MIPI_VIDEO_PERIOD_LINE_NUMS;
			OV5649MIPI_Set_Dummy(0, dummyLine);			
			break;
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_ZSD:			
			pclk = OV5649MIPI_CAPTURE_CLK;
			lineLength = OV5649MIPI_FULL_PERIOD_PIXEL_NUMS;
			frameHeight = (10 * pclk)/frameRate/lineLength;
			dummyLine = frameHeight - OV5649MIPI_FULL_PERIOD_LINE_NUMS;
			OV5649MIPI_Set_Dummy(0, dummyLine);			
			break;		
...
UINT32 OV5649MIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
...
UINT32 OV5649MIPI_SetTestPatternMode(kal_bool bEnable)
{
	...
	{
	    spin_lock(&ov5649mipi_drv_lock);
        OV5649MIPIDuringTestPattern = KAL_TRUE;
        spin_unlock(&ov5649mipi_drv_lock);

        // 0x503D[8]: 1 enable,  0 disable
        // 0x503D[1:0]; 00 Color bar, 01 Random Data, 10 Square
        OV5649MIPI_write_cmos_sensor(0x503D, 0x80);
	}
	else        
	{
	    spin_lock(&ov5649mipi_drv_lock);
        ...
        // 0x503D[8]: 1 enable,  0 disable
        // 0x503D[1:0]; 00 Color bar, 01 Random Data, 10 Square
        OV5649MIPI_write_cmos_sensor(0x503D, 0x00);
	}
                             ...
                             UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            ...
            ((PNVRAM_SENSOR_DATA_STRUCT)pFeaturePara)->SensorId = OV5649_SENSOR_ID;
		...
		case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
			OV5649MIPISetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, *(pFeatureData32+1));
			break;
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
			OV5649MIPIGetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, (MUINT32 *)(*(pFeatureData32+1)));
			break;
        case SENSOR_FEATURE_SET_TEST_PATTERN:
            OV5649MIPI_SetTestPatternMode((BOOL)*pFeatureData16);
            break;
        case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE://for factory mode auto testing             
            *pFeatureReturnPara32= OV5649MIPI_TEST_PATTERN_CHECKSUM;
            *pFeatureParaLen=4;                             
             break;     
	case SENSOR_FEATURE_GET_SENSOR_NAME:
		{
			*pFeaturePara = '\0';
			strcat(pFeaturePara, SENSOR_DRVNAME_OV5648_MIPI_RAW);       
			strcat(pFeaturePara, "_" );
			strcat(pFeaturePara, OV5649MIPI_sensor.vendor);
