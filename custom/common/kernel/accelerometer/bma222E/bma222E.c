...
static int bma222e_init_flag =-1; // 0<==>OK -1 <==> fail
...
static int bma222e_enable_nodata(int en)
{
	...
	{
		printk("bma222e_enable_nodata fail!\n");
		return -1;
	}

    printk("bma222e_enable_nodata OK!\n");
	return 0;
}
...
static int bma222_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = bma222_create_attr(&bma222e_init_info.platform_diver_addr->driver)))
	{
...
static int bma222_i2c_remove(struct i2c_client *client)
{
	...
	if((err = bma222_delete_attr(&bma222e_init_info.platform_diver_addr->driver)))
	{
