...
static struct i2c_board_info __initdata i2c_mpu6050c_acc={ I2C_BOARD_INFO("MPU6050C_ACC", MPU6050C_ACC_I2C_SLAVE_ADDR>>2)};  
...
static int g_recv_reg;
static struct i2c_client *g_client;
static  atomic_t g_after_resume;
...
static int MPU6050C_SetDataBandWidth_Accel(struct i2c_client *client, u8 BandWidth)
{
	...
	memset(databuf, 0, sizeof(u8)*2);	 
	databuf[0] = MPU6050C_REG_ACC_CONFIG2;    
...
static int MPU6050C_ACC_SetLpAccelOdr(struct i2c_client *client, int sample_rate)
{
	...
	memset(databuf, 0, sizeof(u8)*2);	 
	databuf[0] = MPU6050C_ACC_REG_LP_ACCEL_ODR;    
	databuf[1] = sample_rate;

	res = i2c_master_send(client, databuf, 0x2);

	if(res <= 0)
	{
		printk("[mpu6050] send data fail\n");
		return MPU6050C_ACC_ERR_I2C;
	}
...
//MPU6050C_ACC_REG_ACCE_INT_ENABLE
static int MPU6050C_ACC_Int_Enable(struct i2c_client *client, int sample_rate)
{
	...
	memset(databuf, 0, sizeof(u8)*2);	 
	databuf[0] = MPU6050C_ACC_REG_ACCE_INT_ENABLE;    
	databuf[1] = sample_rate;
...
static int MPU6050C_REG_SetByPass(struct i2c_client *client)
{
	u8 databuf[2] = {0}; 
	u8 tempbuf;
	int smplrt_div = 0;
	...
	if(hwmsen_read_byte(client, MPU6050C_ACC_REG_INT_CONFIG, databuf))
	{
		GSE_ERR("read bypass format register err!\n");
		return MPU6050C_ACC_ERR_I2C;
	}
	else
	{
		GSE_LOG("read  bypass register: 0x%x\n", databuf[0]);
	}
	
	tempbuf = databuf[0];
	
	databuf[0] = MPU6050C_ACC_REG_INT_CONFIG;
	databuf[1] = tempbuf |0x02;
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		GSE_ERR("write bypass register err!\n");
		return MPU6050C_ACC_ERR_I2C;
	}
	return MPU6050C_ACC_SUCCESS;    
}
...
static ssize_t show_recv(struct device_driver *ddri, char *buf)
{
    if(!g_client)
    {
...
/*----------------------------------------------------------------------------*/
static ssize_t store_recv(struct device_driver *ddri, const char *buf, size_t count)
{
    int addr;
    u8 dat;
    u8 databuf[2] = {0};
...
hwmsen_read_block(g_client, addr, &dat, sizeof(dat)), dat);
...
static ssize_t show_send(struct device_driver *ddri, char *buf)
{
    return 0;
}


static ssize_t store_send(struct device_driver *ddri, const char *buf, size_t count)
{
    int addr, cmd;
    u8 dat;
    u8 databuf[2] = {0};
    ...
    printk("send(%02X, %02X) = %d\n", addr, cmd,
        i2c_master_send(g_client, databuf, 0x2));
...
static DRIVER_ATTR(recv,        S_IWUSR |  S_IRUGO, show_recv,        store_recv);
static DRIVER_ATTR(send,       S_IWUSR |   S_IRUGO, show_send,        store_send);
...
//	msleep(100); //wanghai
	...
	res = MPU6050C_REG_SetByPass(client);
	if(res != MPU6050C_ACC_SUCCESS)
	{
		return res;
	}
	...
	res=MPU6050C_ACC_SetLpAccelOdr(client,9);
	if(res != MPU6050C_ACC_SUCCESS)
	{
		GSE_ERR("MPU6050C_ACC_SetLpAccelOdr failed!\n");
       
		return res;
	}
	
	
	res = MPU6050C_SetDataBandWidth_Accel(client,0x0a);
	if(res != MPU6050C_ACC_SUCCESS) 
	{
		GSE_ERR("MPU6050C_ACC_SetDataBandWidth failed!\n");
		return res;
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{
				value = *(int *)buff_in;
				GSE_LOG("mpu6050c_acc_acc_operate:value = %d\n",value);
...
static int mpu6050c_acc_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_client = client; //wanghai
	
	memset(obj, 0, sizeof(struct mpu6050c_acc_i2c_data));

	obj->hw = get_cust_acc_hw();
	err = hwmsen_get_convert(obj->hw->direction, &obj->cvt);     
	client->addr = MPU6050C_ACC_I2C_SLAVE_ADDR>>1;
	if(err)
	{
	...
	obj->enable = 0;
	obj->reso = &mpu6050c_data_resolution[0];
