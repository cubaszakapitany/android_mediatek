...
{
    PRINTK("Sound_Speaker_Turnon channel = %d,gsk_on=%d\n",channel,gsk_on);
    int spkmode=0;
	//if(gsk_on)
		//return;
	...
	switch (channel) {
	    case Channel_Stereo:			
	        printk("Sound_Speaker_Turnon Channel_Stereo \n");
		// mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
	        ...
	        break;
	    case Channel_Right:			
	        printk("Sound_Speaker_Turnon Channel_Right \n");
		// mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
               mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ONE); // high
	        break;
	    case Channel_Left:			
	        printk("Sound_Speaker_Turnon Channel_Left \n");
    	//	mt_set_gpio_dir(GPIO_EXTERNAL_AMPLIFIER_PIN,GPIO_DIR_OUT); // output
    		mt_set_gpio_out(GPIO_EXTERNAL_AMPLIFIER_PIN,GPIO_OUT_ONE); // high	
	        break;
    ...
    gsk_on = true;
    PRINTK("Sound_Speaker_Turnon gsk_on = %d\n",gsk_on);
}

void Sound_Speaker_Turnoff(int channel)
{
    PRINTK("Sound_Speaker_Turnoff channel = %d,gsk_on=%d\n",channel,gsk_on);
    int spkmode=0;
	//if(!gsk_on)
		//return;
	switch (channel) {
    ...
    PRINTK("Sound_Speaker_Turnoff gsk_on = %d\n",gsk_on);
}

void Sound_Speaker_Turnon_Left(int channel)
{
    PRINTK("Sound_Speaker_Turnon_Left channel = %d\n",channel);
	//if(gsk_on)
		//return;
    Sound_Speaker_Turnoff_Left(channel);	
    mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ONE); // high
...
void Sound_Speaker_Turnon_Right(int channel)
{
    PRINTK("Sound_Speaker_Turnon_Right channel = %d\n",channel);
	//if(gsk_on)
		//return;
    //mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
    //mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ONE); // high
...
void Sound_Speaker_Turnoff_Left(int channel)
{
    PRINTK("Sound_Speaker_Turnoff_Left channel = %d\n",channel);
	//if(!gsk_on)
		//return;
    mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ZERO); // high
...
void Sound_Speaker_Turnoff_Right(int channel)
{
    PRINTK("Sound_Speaker_Turnoff_Right channel = %d\n",channel);
	//if(!gsk_on)
		//return;
    //mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
    //mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ZERO); // high
