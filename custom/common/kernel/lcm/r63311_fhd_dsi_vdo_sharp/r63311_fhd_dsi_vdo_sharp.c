...
#else
  if(TRUE != hwPowerOn(MT65XX_POWER_LDO_VGP3, VOL_1800,"lcdGP3"))
	{
...
//	unsigned int data_array[16];
  ...
  {
  	LK_INIT=0;
	  if(TRUE != hwPowerOn(MT65XX_POWER_LDO_VGP3, VOL_1800,"lcdGP3"))                   //add referance count
		{
			printk("%s, Fail to enable digital power\n", __func__);
		}
	}
...
#else
	if(TRUE != hwPowerDown(MT65XX_POWER_LDO_VGP3,"lcdGP3"))
	{
