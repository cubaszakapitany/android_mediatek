...
//extern int tpd_get_ps(void);
//extern void tpd_convert(char mode,bool force);
...
extern s32 ekt_tpd_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
                   void *buff_out, s32 size_out, s32 *actualout);
extern int tpd_ps_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout);
extern bool tpd_get_IC();
extern bool tpd_get_ft6x06_IC();
/*----------------------------------------------------------------------------*/
...
int ctp_read_data( u8 *data)
{
	...
	if(g_tpd_drv)
	{
	    *data = g_tpd_drv->tpd_get_ps();
    }
	printk("ctp_read_data:  %x\n", *data);	
/*	
...
//Ivan		 atomic_set(&obj->ps_deb_on, 1);
   ...
   */
		 printk("first enalbe ps set pending interrupt %d\n",obj->pending_intr);
	}
	else//disable ps
	{
		 if(false == obj->als_enable)
		 {
		   printk("PS(0): disable both !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		 }
...
//Ivan added
	if (obj->ps_enable != 1 && ctp_power_flag == 1)
	{
	    ...
	    printk("ctp_eint_work All sensor OFF\n");
	    return;
	}
//Ivan added
	if (ctp_power_flag == 1)
	...
	printk("ctp_eint_work &obj->pending_intr =%d\n",obj->pending_intr);
...
//Ivan
			printk("### ALSPS_SET_PS_MODE enable = %d\n",enable);
...
//Ivan
			printk("### ALSPS_GET_PS_MODE enable = %d\n",enable);
...
//Ivan
			printk("### ALSPS_GET_PS_DATA data = %d\n",dat);
...
//Ivan
			printk("### ALSPS_GET_PS_RAW_DATA data = %d\n",dat);
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{				
				value = *(int *)buff_in;
//Ivan
				printk("##### Msg2133 PS SENSOR_ENABLE value = %d\n",value);
		...
		case SENSOR_GET_DATA:
			//printk("fwq get ps data !!!!!!\n");
			printk("##### PS SENSOR_GET_DATA data++\n");
...
/*
				...
				{
				    msleep(50);
				    printk("##### PS SENSOR_GET_DATA data Sensor suspend\n");
				}
*/
#endif
				...
				{
					err = -1;;
					printk("##### PS SENSOR_GET_DATA ctp_read_data ERROR\n");
				...
				else
				{
				    ...
				    {
				      ctp_read_data(&obj->ps);
//				      printk("get sensor ctp_get_ps_value error data = %d!\n",obj->ps);
				      msleep(50);
				    }
					...
					sensor_data->value_divide = 1;
					sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
					//printk("fwq get ps data =%d\n",sensor_data->values[0]);
				    
					
				}				
//Ivan
			    printk("##### PS SENSOR_GET_DATA data = %d\n", sensor_data->values[0]);
 ...
 static int ps_event_handler(void *unused)
 {
	...
	//edit by Magnum 2012-7-27
        ...
        hwm_sensor_data sensor_data;
		 ...
		 //get raw data
		  //printk("fwq ps INT\n");
		  //printk("Magnum read ps data  = %d \n",ctp_get_ps());
		  if(g_tpd_drv)
		  {  
			  sensor_data.values[0] = g_tpd_drv->tpd_get_ps(); 
	      }
		  sensor_data.value_divide = 1;
		  sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
		  //let up layer to know
		  printk("ctp_simulate_ps read ps data  = %d \n",sensor_data.values[0]);
		  if(err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data))
		  {
			printk("call hwmsen_get_interrupt_data fail = %d\n", err);
		  }
...
//Ivan remove??
	...
	{
		printk("ctp_device register failed\n");
	...
	obj_ps.polling = 0;
	if(tpd_get_ft6x06_IC())
	{
	printk("=====tpd_ps_operate======");
	obj_ps.sensor_operate = tpd_ps_operate;//tpd_ps_operate;//ektf2k_ps_operate;//ctp_ps_operate;//Line<add elan tp><date20130816><>yinhuiyong
	}
	else if(tpd_get_IC())
	{
	  printk("=====ekt_tpd_ps_operate=====");
	 obj_ps.sensor_operate = ekt_tpd_ps_operate;
	}
	else
	{
