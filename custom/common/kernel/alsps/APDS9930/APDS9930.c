...
bool g_bsunps =0;//0 is normal sun 1 is out door sun
...
static void APDS9930_ps_cali_tp_check(void);
static void APDS9930_ps_cali_set_threshold(struct APDS9930_priv *obj) ;
static void APDS9930_ps_cali_start(struct APDS9930_priv *obj);
int APDS9930_i2c_master_operate(struct i2c_client *client, const char *buf, int count, int i2c_flag);
u16 value_ps_high,value_ps_low;
...
static void APDS9930_ps_cali_start(struct APDS9930_priv *obj)
{
    ...
    if(NULL == obj->client)
    {
        APS_ERR("obj->client == NULL\n"); 
        return;
    }

    // read ps
    for(vl_index = 0; vl_index < 11; vl_index++)
    {
...
static void APDS9930_ps_cali_set_threshold(struct APDS9930_priv *obj)
{
    ...
    APS_LOG("APDS9930_ps_cali_set_threshold:g_ps_base_value=%x, hw->ps_threshold_high=%x,hw->ps_threshold_low=%x \n",
        g_ps_base_value, hw->ps_threshold_high, hw->ps_threshold_low);
    
    value_ps_high= g_ps_base_value + hw->ps_threshold_high;
    value_ps_low= g_ps_base_value + hw->ps_threshold_low;


    if( g_ps_base_value < 0x70)
    {
	value_ps_high = g_ps_base_value + 0X1C0;
	value_ps_low  = g_ps_base_value + 0X180;
    }
    else //if( g_ps_base_value < 0x140)
    {
        value_ps_high = g_ps_base_value + 0X270;
	value_ps_low  = g_ps_base_value + 0X1C0;
    }
    if(value_ps_high >0X3FF)
   {
     value_ps_high = 0X380;
     value_ps_low  = 0X3F0;
   }
    
	atomic_set(&obj->ps_thd_val_high, value_ps_high);
	atomic_set(&obj->ps_thd_val_low,  value_ps_low);
	...
	APS_LOG("APDS9930_ps_cali_set_threshold:value_ps_high=%x,value_ps_low=%x! \n",value_ps_high,value_ps_low);
}
...
static int LuxCalculation(struct i2c_client *client, int ch0data, int ch1data)
{
	...
	luxValue = ((IAC*APDS9930_GA*APDS9930_DF)/100)*4/((17408/100)*16);
...
{	 
	...
	*/
    ...
    // Jake.L, DATE20140526, add for ALPS01579409, DATE20140526-01 LINE
    mutex_lock(&APDS9930_mutex);
...
// Jake.L, DATE20140529, ALPS01579409, DATE20140529-01 LINE
static int APDS9930_check_als_value(struct APDS9930_priv *obj, u16 als)
{
	int invalid = 0;
...
APS_LOG("XXXXXXXXXXXXXXXXXXXXXXXXXXXAPDS9930_get_ps_value g_bsunps  = %d,ps==%d,==atomic_read(&obj->ps_thd_val_high)==%d",g_bsunps,ps,ps_cali.close);
			if((ps >ps_cali.close))
			{
				val = 0;  /*close*/
				val_temp = 0;
	...
	else
	{
			if(g_bsunps)
			{
  			   atomic_set(&obj->ps_thd_val_high, 0x350);
	                   atomic_set(&obj->ps_thd_val_low,  0X330);
                        }
APS_LOG("==========APDS9930_get_ps_value g_bsunps  = %d,ps==%d,==atomic_read(&obj->ps_thd_val_high)==%d",g_bsunps,ps,atomic_read(&obj->ps_thd_val_high));
			...
			else if((ps  < atomic_read(&obj->ps_thd_val_low)))
			{
				...
				intr_flag_value = 0;
			       ...
			       val = val_temp;	
printk("==========APDS9930_get_ps_value value_ps_high==%d",value_ps_high);
printk("==========APDS9930_get_ps_value value_ps_low==%d",value_ps_low);
        atomic_set(&obj->ps_thd_val_high, value_ps_high);
	atomic_set(&obj->ps_thd_val_low,  value_ps_low);
...
*/ 
   printk("==========APDS9930_get_ps_value val %d",val);
   return val;	
}
...
static void APDS9930_eint_work(struct work_struct *work)
{
	...
	else
	{
		...
		printk("APDS9930_eint_work rawdata ps=%d als_ch0=%d!\n",obj->ps,obj->als);
		//APS_LOG("APDS9930 int top half time = %lld\n", int_top_time);
		printk("APDS9930_get_ps_value(obj, obj->ps)= %d  --cyy\n", APDS9930_get_ps_value(obj, obj->ps));

		if(obj->als > 20000)
		{
			printk("APDS9930_eint_work ALS too large may under lighting als_ch0=%d!.g_bsunps=%d\n",obj->als,g_bsunps);
                        g_bsunps = 1;
        		//return;
		}
		else
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_TEST_RESULT:
			...
			case ALSPS_IOCTL_PS_SET_CALI:
				...
				APDS9930_ps_cali_set_threshold(obj);
				break;
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				if(obj->als == 0)
				{
					...
					sensor_data->values[0] = APDS9930_check_als_value(obj, obj->als);
					temp_als = sensor_data->values[0];
				}
				APS_DBG("ALS SENSOR_GET_DATA: %d \n", sensor_data->values[0]);
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
			}
			break;
