...
//#define TP_REPLACE_ALSPS // add for build error
/////////////////////////////////////////////////////////
#define I2C_NUM 1
...
#define TPD_KEYS                { KEY_MENU, KEY_HOMEPAGE/*KEY_HOME*/, KEY_BACK}
...
int elan_tp_replace_ps_mod_on(void);
int elan_tp_replace_ps_mod_off(void);
int  elan_tp_replace_ps_enable(int enable);
u16  elan_get_tp_replace_ps_value(void);
int elan_tp_replace_ps_state= 0;
int elan_tp_replace_ps_close = 0;
#endif
                                        ...
                                        button_state = KEY_BACK;
                                               ...
                                               case ELAN_KEY_HOME:
                                                        MTK_TP_DEBUG("KEY home 1\n");
                                                                                    #ifndef LCT_VIRTUAL_KEY
                                                                                                                                                                                                    input_report_key(idev, /*KEY_HOMEPAGE*/KEY_HOME, 1);
                                                                                    #else
                                                                                    input_report_key(idev, BTN_TOUCH, 1);
                                                                                    input_report_abs(idev, ABS_MT_TOUCH_MAJOR, 8);
...
int elan_tp_replace_ps_mod_on(void)
{
	...
	char buf_on[] = {0x54, 0xC1, 0x00, 0x01};
       elan_tp_replace_ps_close = 0;
	res = i2c_master_send(i2c_client, buf_on, sizeof(buf_on));
	if (res != sizeof(buf_on)) {
		printk("[elan] turn on face mod faild\n");
		elan_tp_replace_ps_state=0;
		return 0;
	} else {
	       elan_tp_replace_ps_state=1;
		printk("[elan] turn on face mod ok\n");
		return 1;
	}
}
...
int elan_tp_replace_ps_mod_off(void)
{
	...
	if (ret != sizeof(buf_off)) {	
		elan_tp_replace_ps_state=1;
		printk("[elan] turn off face mod faild\n");
		return 0;
	} else {
	       elan_tp_replace_ps_state=0;
		printk("[elan] turn off face mod ok\n");
		return 1;
	}
#else
	mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
...
if (enable)
	{
	if(1==elan_tp_replace_ps_mod_on())
		{
		printk("[elan]open the ps mod successful\n");
	    return 1;
		}
	else
		{
		printk("[elan]open the ps mod fail\n");
	    return 0;
		}
	...
	{
	if(1==elan_tp_replace_ps_mod_off())
		{
		printk("[elan]close the ps mod successful\n");
	    return 1;
		}
	else
		{
		printk("[elan]close the ps mod fail\n");
	    return 0;
		}
	}
}
...
u16  elan_get_tp_replace_ps_value(void)
	{
	if(1==elan_tp_replace_ps_close)
		{
		printk("[elan]elan_get_tp_replace_ps_value 500\n");
		return 500;
		}
	else
		{
		printk("[elan]elan_get_tp_replace_ps_value 100\n");
		return 100;
		}
