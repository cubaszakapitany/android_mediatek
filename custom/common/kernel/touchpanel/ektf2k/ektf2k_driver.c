...
//#define MTK_ELAN_DEBUG
         ...
         #define MTK_TP_DEBUG(fmt, args ...) printk(fmt,##args)
#else
...
#include "Tinno_4040_hrc_Ver1004.i"
};
static uint8_t file_fw_data_dijing[] = {
#include "Tinno_4040_dijin_Ver1005.i"
};
static uint8_t* file_fw_data = NULL;

//FW ID CHECK ----start by baojun.fu
#define FW_ID_HRC				0x4040
#define FW_ID_DIJING			0xd404
...
static int write_check_fwid_to_rom(struct i2c_client *client, uint8_t *cmd, size_t size)
{
    ...
    if( (major != 0x40) && (major != 0xD4) )//not fit for all FW ID then write it
    {
...
/* FW ID & FW VER*/
    ...
    //Update_FW_One(private_ts->client, RECOVERY);
//---------add by baojun.fu for iap finish		
	printk("[elan] iap_finish_flag----------set flag\n");
    ...
    //enable_irq(CUST_EINT_TOUCH_PANEL_NUM);
		...
		printk("ektf2k reset afer IAP FINISH !\n");
    	mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ZERO);
    	mdelay(10);
    	mt_set_gpio_out( GPIO_CTP_RST_PIN, GPIO_OUT_ONE );
		msleep(300);
...
// End Firmware Update 
         ...
         {
             if(FW_ID == FW_ID_HRC) {
                     printk(KERN_ERR "[elan] %s: TP is FW_ID_HRC, FW_ID: 0x%4.4x \n", __func__, FW_ID);
                     file_fw_data = file_fw_data_hrc;
             }else if (FW_ID == FW_ID_DIJING) {
