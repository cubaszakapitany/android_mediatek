...
//---------add by baojun.fu for iap finish		
uint8_t iap_finish_flag = 0;

/*The newest firmware, if update must be changed here*/
static uint8_t file_fw_data_yeji[] = {
//#include "YEJI_EKTF3248_S8321A_11_V0.01_tpFactory.i"
#include "YEJI_EKTF3248_S8321A_15_V0.01_tpFactory.i"
};
static uint8_t file_fw_data_jiemian[] = {
//#include "JTOUCH_EKTF3248_S8321A_11_V0.01_tpFactory.i"
#include "JTOUCH_EKTF3248_S8321A_31_V0.01_tpFactory.i"
};
static uint8_t* file_fw_data = NULL;

//FW ID CHECK ----start by baojun.fu
// 3000K Solution
#define FW_ID_YEJI		0x1169
#define FW_ID_JIEMIAN	0x116A

#define ELAN_TPID_INFO "/data/local/TPID_info"  //TPID LogFile Path
int TP_ID = 0x00;
int WriteTPInfo(char *path);
int ReadTPInfo(char *path);
...
#define HALL_CHECK
#ifdef HALL_CHECK
/********start add hall check function by baojun.fu@elanic.com.cn***********/
...
void elan_hall_check_callback(unsigned long data)
{
        printk("[elan]:%s change mode enter %d %d\n", __func__, curr_hall_state,prev_hall_state);
        // 根据Hall状态， 设置TP进入相应模式
        curr_hall_state = mt_get_gpio_in(GPIO_HALL_1_PIN);//gpio_get_value(139);
...
void elan_hall_check_work_func(void)
{
        ...
        //printk("[elan]:%s change mode enter! %d \n", __func__, curr_hall_state);
        prev_hall_state = curr_hall_state;
        if(curr_hall_state)
        {
                //normal mode
                if (i2c_master_send(private_ts->client, cmd_normal, sizeof(cmd_normal)) != sizeof(cmd_normal))
                {
                    //MTK_TP_DEBUG("[elan]:%s send normal mode cmd failed!\n", __func__);
                }
                else
                {
                    //MTK_TP_DEBUG("[elan]:%s send normal mode cmd OK!\n", __func__);
                }
        }
        else
        {
                //half mode 
                if (i2c_master_send(private_ts->client, cmd_half, sizeof(cmd_half)) != sizeof(cmd_half))
                {
                    //MTK_TP_DEBUG("[elan]:%s send half mode cmd failed!\n", __func__);
                }
                else
                {
                    //MTK_TP_DEBUG("[elan]:%s send half mode cmd OK!\n", __func__);
                }
        }
}

/******** end  add hall check function by baojun.fu@elanic.com.cn***********/
...
int elan_iap_open(struct inode *inode, struct file *filp){ 
      ...
      if (private_ts == NULL)  
	  		MTK_TP_DEBUG("private_ts is NULL~~~");
...
//{ iap-ram
		...
		case IOCTL_IAPRAM_CHECKSUM:
			MTK_TP_DEBUG("[elan] IOCTL_IAPRAM_CHECKSUM,%d\n",(int*)arg);
			//len = 1;
			if (copy_from_user(buf, (const void*)arg, 4))
			{
				MTK_TP_DEBUG("[elan] copy_from_user error\n");
				return -EFAULT;
			}
			MTK_TP_DEBUG("[elan] IOCTL_SET_COMMAND:%x:%x:%x:%x\n",
...
//      {
        ...
        {        
                 ...
                 if (checkCnt >= 5)
                 {
                          MTK_TP_DEBUG("[ELAN] ERROR: CheckIapMode %d times fails!\n", IAPRESTART);
                          return E_FD;
                 }
                 else
                 {
                          MTK_TP_DEBUG("[ELAN] CheckIapMode retry %dth times! And restart IAP~~~\n\n", checkCnt);
                          goto IAP_RESTART;
                 }
        }
        else
                 MTK_TP_DEBUG("[ELAN]  CheckIapMode ok!\n");
#endif
//      } else
...
static int elan_ktf2k_touch_sysfs_init(void)
{
         ...
         if (android_touch_kobj == NULL) {
                   MTK_TP_DEBUG(KERN_ERR "[elan]%s: subsystem_register failed\n", __func__);
                   ret = -ENOMEM;
                   return ret;
         }
         ret = sysfs_create_file(android_touch_kobj, &dev_attr_gpio.attr);
         if (ret) {
                   MTK_TP_DEBUG(KERN_ERR "[elan]%s: sysfs_create_file failed\n", __func__);
                   return ret;
         }
         ret = sysfs_create_file(android_touch_kobj, &dev_attr_vendor.attr);
         if (ret) {
                   MTK_TP_DEBUG(KERN_ERR "[elan]%s: sysfs_create_group failed\n", __func__);
                   return ret;
         }
         return 0 ;
}
...
*/
         ...
         {
        RECOVERY=0x80;
						TP_ID =  buf_recv[5] << 8 | buf_recv[4];
						MTK_TP_DEBUG("[elan] TP_ID = %x\r\n", TP_ID);

        rc = i2c_master_recv(client, buf_recv, 8);

        		MTK_TP_DEBUG("mtk-tpd:[elan] %s: Bootcode Verson %2x:%2X:%2x:%2x:%2x:%2X:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3], buf_recv[4], buf_recv[5], buf_recv[6], buf_recv[7]);
        return RECOVERY; 
         }
...
static int write_check_fwid_to_rom(struct i2c_client *client, uint8_t *cmd, size_t size)
{
    ...
    minor = buf_recv[3];
    //FW_ID_YEJI		0x1169
	//FW_ID_JIEMIAN		0x116A

    if( major != 0X11 )//FW ID 0x11XX
    {
...
// Firmware ID
         ...
         /*****************************************************************************************/
         cmd_check_fwid[2] = major;															// write check fwid info
         cmd_check_fwid[3] = minor;
         if(FW_VERSION >= 0x5510)//old FW do not support the new cmd--baojun.fu
         {
         rc = write_check_fwid_to_rom(client, cmd_check_fwid , 4);  // write check fwid info
         } 														// write check fwid info

         /*****************************************************************************************/
         FW_ID = major << 8 | minor;/*Get Check FWID */
         //FW ID check ------by baojun.fu
         TP_ID = FW_ID;
//		 WriteTPInfo(ELAN_TPID_INFO);
#endif
...
static int elan_ktf2k_ts_setup(struct i2c_client *client)
{
         ...
         if (rc != 0x80){
             ...
             {
                   rc = 0x80;
                   MTK_TP_DEBUG("mtk-tpd:[elan] FW_VERSION = %d, last FW update fail\n", FW_VERSION);
             }
         }
  ...
  if(!pReadData){
	MTK_TP_DEBUG("mtk-tpd:[elan] dma_alloc_coherent failed!\n");
        ...
        MTK_TP_DEBUG("[elan_debug] %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7],buf[8], buf[9], buf[10], buf[11], buf[12], buf[13], buf[14], buf[15],buf[16], buf[17]);
         ...
         if ((buf[0] == MTK_FINGERS_PKT) || (buf[0] == FIVE_FINGERS_PKT)) {                      //for mtk five finger
                   ...
                   MTK_TP_DEBUG("[elan_recv] %x %x %x %x %x %x %x %x\n", buf[24], buf[25], buf[26], buf[27], buf[28], buf[29], buf[30], buf[31]);
                    ...
                    rc = i2c_master_recv(client, buf+ 32, 2); //yixuhong 20130821 8 > 2
                   if (rc != 2)
...
//mdelay(500);
	...
	// Send Dummy Byte	
	MTK_TP_DEBUG("[ELAN] send one byte data:%x,%x",client->addr,data);
	res = i2c_master_send(client, &data,  sizeof(data));
	if( res!=sizeof(data) )
	{
		MTK_TP_DEBUG("[ELAN] dummy error code = %d\n",res);
...
#endif
		MTK_TP_DEBUG("[ELAN] IAP_RAM successfully!\n");
	return 0;
}
...
static int elan_ktf2k_ts_iapRam_continue(struct i2c_client *client)
{
	...
	MTK_TP_DEBUG("[elan]: %s %02x, %02x, %02x, %02x\n",__func__, cmd[0], cmd[1], cmd[2], cmd[3]);
	if ((i2c_master_send(client, cmd, sizeof(cmd))) != sizeof(cmd)) {
		MTK_TP_DEBUG("[elan] %s: i2c_master_send failed\n", __func__);
		return -EINVAL;
	}
...
/*	rc = elan_ktf2k_ts_poll(client);
	...
	if(buf_recv[0]== 0xcc && buf_recv[1]== 0xcc && buf_recv[2]==0xcc && buf_recv[3]==0xcc)
	{
		...
		{
			MTK_TP_DEBUG("[elan] checksum is right\n");
			return 1;
		}
		else
		{
			MTK_TP_DEBUG("[elan] checksum is Error, Need to be updated\n");	
			return 0;
		}
	}
/*
...
static void elan_ktf2k_ts_report_data(struct i2c_client *client, uint8_t *buf)
{
	...
	if (buf[0] == 0x66) 
	{
		curr_hall_state = 1;//add for reset IC to wake up 2013/11/29
		prev_hall_state = 1;
	}
	if (buf[0] == 0x55) 
	{
		//report finger up event 2014/4/21
		input_report_key(idev, BTN_TOUCH, 0);
		input_mt_sync(idev);
		input_sync(idev);
		return;// hello packet no more opration 2014/4/21
	}
#if 0
// iap-ram
	else if (buf[0] == 0xcc) 
	{
		...
		MTK_TP_DEBUG("[elan] %s: via rom data %2x:%2x:%2x:%2x\n", __func__, file_iapRam_data[2241], file_iapRam_data[2240], file_iapRam_data[2239], file_iapRam_data[2238]);
		if(buf[0]== 0xcc && buf[1]== 0xcc && buf[2]==0xcc && buf[3]==0xcc)
		{
			...
			else
			{
				...
				MTK_TP_DEBUG("[elan] %s Enter into normal report system\n", __func__);
				elan_ktf2k_ts_iapRam_continue(i2c_client);
				msleep(500);
			}
		}
...
//iap-ram
	...
	else if(buf[0] == 0xFA)
	{
	   if(buf[2] == 0xAA && PROXIMITY == 1 ) // close to
		 {
			 MTK_TP_DEBUG("TP_PROXIMITY_SENSOR VVV by cheehwa\n");
			 PROXIMITY_STATE = 0;
			 MTK_TP_DEBUG("tpd_touchinfo PROXIMITY_STATE %d\n",PROXIMITY_STATE );
		 }
		 else if(buf[2] == 0x55 && PROXIMITY == 1 ) // leave
		 {
			MTK_TP_DEBUG("TP_PROXIMITY_SENSOR XXX by cheehwa\n");
			 PROXIMITY_STATE = 1;
			MTK_TP_DEBUG("tpd_touchinfo PROXIMITY_STATE %d\n",PROXIMITY_STATE );
		 }				 
	} 
         ...
         switch (buf[0]) {
                ...
                //input_sync(idev);     
                        ...
                        if(X_RESOLUTION > 0 && Y_RESOLUTION > 0)
                        {
                            x = ( x * LCM_X_MAX )/X_RESOLUTION;
                            y = ( y * LCM_Y_MAX )/Y_RESOLUTION;
                            MTK_TP_DEBUG("[elan resolution] %s, x res=%d, y res=%d\n",__func__, X_RESOLUTION , Y_RESOLUTION);
                        }
                        else
                        {
                            x = ( x * LCM_X_MAX )/ELAN_X_MAX;
                            y = ( y * LCM_Y_MAX )/ELAN_Y_MAX;
                            MTK_TP_DEBUG("[elan resolution] %s, x res=%d, y res=%d\n",__func__, ELAN_X_MAX , ELAN_Y_MAX);
                        }
                                                        #endif                 
...
static int update_fw_handler(void *unused)
{
	 ...
	 int retval = TPD_OK;
         ...
         while(probe_flage == 0)
         {
         	MTK_TP_DEBUG("[elan] wait probe_flage\n");
         	msleep(30);
		 }
					...
					MTK_TP_DEBUG(" FW_VERSION=0x%x,   New_FW_VER=0x%x \n",  FW_VERSION  , New_FW_VER); 
					 ...
					 /* for firmware auto-upgrade*/        
						if (New_FW_VER > (FW_VERSION)) {
							MTK_TP_DEBUG("FW_Version is different!");
							Update_FW_One(private_ts->client, RECOVERY);
					} else if(FW_VERSION == 0) {                       
							MTK_TP_DEBUG("FW_Version is not read ok!");
							RECOVERY=0x80;                  
							Update_FW_One(private_ts->client, RECOVERY);           
					}
		...
		//RESET ic to read new resolution--2013年10月23日14:34:59
		MTK_TP_DEBUG("ektf2k reset afer IAP FINISH !\n");
    	mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ZERO);
			msleep(20);
		//change POWER DOWN flow  2014/5/16
		#ifdef TPD_POWER_SOURCE_CUSTOM
			hwPowerDown(TPD_POWER_SOURCE_CUSTOM, "TP");
		#else
			hwPowerDown(MT6323_POWER_LDO_VGP1, "TP");//yixuhong 20130821 modify,for ctp power up
		#endif
			msleep(20);
		//change POWER DOWN flow   2014/5/16
		#ifdef TPD_POWER_SOURCE_CUSTOM
		...
		#else
			hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_2800, "TP");//yixuhong 20130821 modify,for ctp power up
		#endif
			msleep(20);
		//change POWER ON flow  2014/5/16
    	mt_set_gpio_out( GPIO_CTP_RST_PIN, GPIO_OUT_ONE );
		msleep(150);
	
	    fw_err = elan_ktf2k_ts_setup(private_ts->client);
	    if (fw_err < 0) {
	        MTK_TP_DEBUG(KERN_INFO "[elan] FW INFO read failed !\n");
	    }
	MTK_TP_DEBUG("[elan] iap_finish_flag----------set flag\n");
 //---------add in probe----------
#ifdef ESD_CHECK	
...
//         #endif
       //  client->addr |= I2C_ENEXT_FLAG;
         ...
         MTK_TP_DEBUG("mtk-tpd:[elan] %s:client , TPD_DEVICE = ektf2k\n",__func__);
         ...
         //MTK_TP_DEBUG("mtk-tpd:[elan]%x=IOCTL_I2C_INT\n",IOCTL_I2C_INT);
         //MTK_TP_DEBUG("mtk-tpd:[elan]%x=IOCTL_IAP_MODE_LOCK\n",IOCTL_IAP_MODE_LOCK);
         //MTK_TP_DEBUG("mtk-tpd:[elan]%x=IOCTL_IAP_MODE_UNLOCK\n",IOCTL_IAP_MODE_UNLOCK);

#if 1
         i2c_client = client;
         private_ts = &ts;
         private_ts->client = client;
	//MTK_TP_DEBUG("[elan] ELAN enter tpd_probe_ ,the i2c addr=0x%x", client->addr);
	//MTK_TP_DEBUG("GPIO43 =%d,GPIO_CTP_EINT_PIN =%d,GPIO_DIR_IN=%d,CUST_EINT_TOUCH_PANEL_NUM=%d",GPIO43,GPIO_CTP_EINT_PIN,GPIO_DIR_IN,CUST_EINT_TOUCH_PANEL_NUM);
#endif
/*
...
*/
   // MTK_TP_DEBUG("ELAN enter tpd_probe_ ,the i2c addr=0x%x", client->addr);
/*
...
*/
         ...
         MTK_TP_DEBUG("[elan] RESET RESOLUTION\n");
...
/*
  ...
  */       
// Setup Interrupt Pin
  mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);//mask INT
  mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
    mt_set_gpio_dir(GPIO_CTP_RST_PIN, 1);
    mt_set_gpio_out(GPIO_CTP_RST_PIN, 0);
	msleep(20);
//change POWER ON flow after RST pull LOW 2014/5/16
#ifdef TPD_POWER_SOURCE_CUSTOM
    hwPowerOn(TPD_POWER_SOURCE_CUSTOM, VOL_2800, "TP");
#else
	hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_2800, "TP");//yixuhong 20130821 modify,for ctp power up
...
//#endif
	mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);//mask INT
         ...
         {
    //FW ID CHECK ----start by baojun.fu
    //         ReadTPInfo(ELAN_TPID_INFO);
             if(TP_ID == FW_ID_YEJI) {
                     MTK_TP_DEBUG(KERN_ERR "[elan] %s: TP is FW_ID_YEJI, FW_ID: 0x%4.4x \n", __func__, TP_ID);
                     file_fw_data = file_fw_data_yeji;
             }else if (TP_ID == FW_ID_JIEMIAN) {
                     MTK_TP_DEBUG(KERN_ERR "[elan] %s: TP is FW_ID_JIEMIAN, FW_ID: 0x%4.4x \n", __func__, TP_ID);
                     file_fw_data = file_fw_data_jiemian;
             }
             else {
                     MTK_TP_DEBUG(KERN_ERR "[elan] TP ID is error: no support!\n");
                     //return 1;//TP ID is error ; DO not return error but not to upgrade 2014/5/16
             }                  
    //FW ID CHECK ----end   by baojun.fu
             mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
...
void ReadTPVendor(char * fw_vendor_name)
{
	if(TP_ID == FW_ID_YEJI)
		strcpy(fw_vendor_name,"YEJI");
	else
		strcpy(fw_vendor_name,"JIEMIAN");
}
//FW ID CHECK ----start by baojun.fu
int ReadTPInfo(char *path)
{
	...
	if(IS_ERR(filp))
		MTK_TP_DEBUG("[ELAN] File Open Error:%s\n", path);             
	else
		MTK_TP_DEBUG("[ELAN] File open successfully~~~\n");
		
	if(!filp->f_op) {       
			MTK_TP_DEBUG("[ELAN] File Operation Method Error!!\n");       
			return 0;       
	}
	
	//read
	filp->f_op->read(filp, (char *)rbuf, sizeof(rbuf), &filp->f_pos); 
	
	sscanf(rbuf, "%04x", &TP_ID);
	MTK_TP_DEBUG("[ELAN] TP_ID = 0x%04x\r\n", TP_ID);
	
	MTK_TP_DEBUG("[ELAN] Read data is [0x%04x]\n", TP_ID);
...
int WriteTPInfo(char *path)
{
	...
	if(filp == NULL)
		filp = filp_open(path,O_RDWR|O_CREAT,0777);
	...
	sprintf(wbuf, "%04x",  TP_ID);
	
	//write
	filp->f_op->write(filp, (char *)wbuf, sizeof(wbuf), &filp->f_pos);
...
{	
	...
	if(have_interrupts == 1){
		...
		if ((++por_cnt) >= 2)
        {
            ...
            //reset here
	    hwPowerDown(MT6323_POWER_LDO_VGP1, "TP");//yixuhong add 20131127,power down when esd checked
	    mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
            mt_set_gpio_dir(GPIO_CTP_RST_PIN, 1);
            mt_set_gpio_out(GPIO_CTP_RST_PIN, 0);
            ...
            // for enable/reset pin
	    hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_2800, "TP");//yixuhong add 20131127,power on when esd checked
            mt_set_gpio_mode(GPIO_CTP_RST_PIN, 0);
            mt_set_gpio_dir(GPIO_CTP_RST_PIN, 1);
            mt_set_gpio_out(GPIO_CTP_RST_PIN, 1);
            msleep(10);
...
static int tpd_resume(struct i2c_client *client)
{
	...
	if (work_lock == 1)//if update FW stop resume 2014/1/3
	{
		return -1;
	}
...
// james    
	...
	if(SW_REBOOT == bootMode)
		bootMode = NORMAL_BOOT;
// end james
    MTK_TP_DEBUG("[elan]: I2C Touchscreen Driver init\n");

    if(i2c_add_driver(&tpd_i2c_driver) != 0)
    {
        MTK_TP_DEBUG("[elan]: unable to add i2c driver.\n");
        return -1;
    }
...
static struct tpd_driver_t tpd_device_driver =
{
    ...
    .tpd_have_button = 0,
#endif
	.tpd_get_fw_version = ReadTPVer,
    .tpd_get_fw_vendor_name = ReadTPVendor,
};

static int __init tpd_driver_init(void)
{
         MTK_TP_DEBUG("mtk-tpd ekft2k touch panel driver init\n");
...
static int __init tpd_driver_init(void)
{
    MTK_TP_DEBUG("[elan]: Driver Verison mtk0003 for MTK65xx serial\n");
    MTK_TP_DEBUG("MediaTek ekft2k touch panel driver init\n");
	  //i2c_register_board_info(0, &ekft2k_i2c_tpd, 1) 
    if(tpd_driver_add(&tpd_device_driver) < 0)
    {
        MTK_TP_DEBUG("[elan]: %s driver failed\n", __func__);
    }
