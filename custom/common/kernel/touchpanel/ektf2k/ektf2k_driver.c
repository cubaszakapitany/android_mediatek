...
*/
         if(buf_recv[0]==0x55 && buf_recv[1]==0x55)//握手数据包
         {
...
static int write_check_fwid_to_rom(struct i2c_client *client, uint8_t *cmd, size_t size)
{
    ...
    if( buf_recv[2] != cmd[2] )//not fit for  FW ID then write it
    {
                ...
                //input_sync(idev);     
                        ...
                        else
                        {
                            ...
                            y = ( y * LCM_Y_MAX )/ELAN_Y_MAX;
                                                        ...
                                                        //y = ( y * LCM_Y_MAX )/ELAN_Y_MAX;
                                                        #ifdef ELAN_DEBUG
                                                        MTK_TP_DEBUG("mtk-tpd:[elan_debug] %s, x=%d, y=%d\n",__func__, x , y);
                                                        #endif
                                                        //x = LCM_X_MAX-x;        
                                                        //y = Y_RESOLUTION-y;                           
                                                        if (!((x>=LCM_X_MAX) || (y>=LCM_Y_MAX))) 
                                                        {   
																MTK_TP_DEBUG("mtk-tpd:[elan_debug] %s, Start input report event!\n",__func__);
                                                                 input_report_key(idev, BTN_TOUCH, 1);
                                                                 input_report_abs(idev, ABS_MT_TRACKING_ID, i);
                                                                 input_report_abs(idev, ABS_MT_TOUCH_MAJOR, 8);
                                                                 input_report_abs(idev, ABS_MT_POSITION_X, x);
         ...
         if(1)
         {
            ...
            printk(KERN_ERR "[elzan] %s: TP , FW_ID: 0x%4.4x \n", __func__, FW_ID);
             if(FW_ID == FW_ID_HUARUICHUAN) {
                     ...
                     printk(KERN_ERR "[elan] %s: TP is FW_ID_YEJI, FW_ID: 0x%4.4x \n", __func__, FW_ID);
