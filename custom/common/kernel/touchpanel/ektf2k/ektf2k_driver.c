...
#if  0//IAP_PORTION                         //upgrade  FW DMA mode
...
//} iap-ram
#define IOCTL_ELAN_New_FW_ID  		_IOR(ELAN_IOCTLID,  22, int)
#define IOCTL_ELAN_New_FW_VER  				_IOR(ELAN_IOCTLID,  23, int)
...
int CTP_Face_Mode_State(void);
int CTP_Face_Mode_Switch(bool onoff_state);
static  int Get_Ctp_Face_Mode(void);
 s32 ekt_tpd_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
	...
	uint8_t buf[64]={0};
		...
		//	#endif
				   ...
				   case IOCTL_ELAN_New_FW_VER:
					  ...
					  printk("IOCTL_ELAN_New_FW_VER");
					break;
				   case IOCTL_ELAN_New_FW_ID:			   	
...
PAGE_REWRITE:
		...
		res = WritePage(szBuff, PageSize,8);
#endif
...
//      ts->fw_ver = major << 8 | minor;
 ...
 /*  {
	...
	sprintf(tpd_desc, "%s-%s(0x%x)","HUARUICHUAN", tpd_device_driver.tpd_device_name,
        FW_VERSION);
...
// iap-ram
	...
	if (buf[0] == 0x55) 
	{
        ...
        if (PROXIMITY == 1)
        {
			...
			printk("mtk-tpd:[elan] 55 55 55 55--PS ON---\n", __func__);
        }
	   ...
	   if(buf[2] == 0xAA && PROXIMITY == 1 ) // close to
		 {
			 ...
			 PROXIMITY_STATE = 0;
			 printk("tpd_touchinfo PROXIMITY_STATE %d\n",PROXIMITY_STATE );
		 }
...
//      int button_state = 0;
         ...
         int index = 0;
         int i =0;
         printk("mtk-tpd[elan] interrupt touch_event_handler\n");
...
bool tpd_get_IC()
{
	printk("============tpd_get_IC %d==============",g_bEktIC);
	return  g_bEktIC;
}
EXPORT_SYMBOL(tpd_get_IC);
  		...
  		//add by yaohua.li 
{
		...
		uint8_t buf_recv[8] = {0};
         ...
         rc = elan_ktf2k_ts_get_data(client, cmd_id, buf_recv, 4);
         printk("======returntpd_probe=============[elan] %s: RE-K Packet %2x:%2X:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3]);
		 if (rc < 0){
		 
			 printk("====================returntpd_probe==============");
            goto read_id_error;
         }
}
// Setup Interrupt Pin
                   ...
                   void *buff_out, s32 size_out, s32 *actualout)
{
    ...
    {
        case SENSOR_DELAY:
        	printk("hdx tpd_ps_operate SENSOR_DELAY\n");
        ...
        case SENSOR_ENABLE:
        	 printk("hdx tpd_ps_operate SENSOR_ENABLE");
            if ((buff_in == NULL) || (size_in < sizeof(int)))
            {
        ...
        case SENSOR_GET_DATA:
        	 printk("hdx tpd_ps_operate SENSOR_GET_DATA\n");
...
*/
char* get_ekt_fw_name(void)
{
	char buf[32]= {"HUARUICHUANG"};
	return "HUARUICHUANG";
}
int get_fw_ekt_version(void)
{
	return FW_VERSION&0x00F;// flag;
}
char* get_ekt_TpName( void )
{
   char tpd_desc_info[50];
   sprintf(tpd_desc_info, "%s-S3520AP-ekft2k-0x%x",get_ekt_fw_name(),get_fw_ekt_version());
...
static struct tpd_driver_t tpd_device_driver =
{
    ...
    .suspend = tpd_suspend,
    .resume = tpd_resume,
    .tpd_get_ps = CTP_Face_Mode_State,
    .tpd_convert = CTP_Face_Mode_Switch,
    .tpd_TpName = get_ekt_TpName,
#ifdef TPD_HAVE_BUTTON
    .tpd_have_button = 1,
#else
    .tpd_have_button = 0,
