...
static u8 gtp_bak_ref_proc(struct i2c_client *client, u8 mode)
{
    ...
    if((RECOVERY_BOOT == get_boot_mode())||(FACTORY_BOOT == get_boot_mode()) || (gtp_ref_retries >= GTP_CHK_FS_MNT_MAX))
    {
    ...
    do
    {
		...
		if(pre_key&&(key_value==0))
		{
			...
			{	
				if(pre_key==1)
					...
					tpd_button(190, 910, 0);
				else
					tpd_button(310, 910, 0);
			}
			else
				tpd_up(0, 0, 0);
			pre_key = 0;
        ...
        if (key_value || pre_key)
        {
            for (i = 0; i < GTP_MAX_KEY_NUM; i++)
            {
                ...
                #if 1
                input_y = 910;
                if(touch_key_array[i]==KEY_MENU)
                	input_x = 70;
				else if(touch_key_array[i]==KEY_HOMEPAGE)
					input_x = 190;
				else
					input_x = 310;
					
				if (FACTORY_BOOT == get_boot_mode()|| RECOVERY_BOOT == get_boot_mode())
                	tpd_button(input_x, input_y, key_value & (0x01 << i));
				else if(key_value & (0x01 << i))
					tpd_down(input_x, input_y, 0, 0);
				else
					tpd_up(0, 0, 0);
				#endif
...
{
#if GTP_COMPATIBLE_MODE
    if ((CHIP_TYPE_GT9F == gtp_chip_type) && (RECOVERY_BOOT != get_boot_mode())&&(FACTORY_BOOT != get_boot_mode()))
    {
