...
//begin add by zhangmanlin 20140227

static unsigned char read_sensor_id( struct i2c_client *client )
{
    //unsigned char id;
    u8 sensor_id = 0;
    s32 ret = 0;
    ...
    GTP_INFO("get_fwbuff lishengli666  : sensor_id =  %d",
             sensor_id);  
	#if 0
    if (SUCCESS == ret)
    {
        if(sensor_id == 0)
            gtp_fw = gtp_dijing_FW;
        else if(sensor_id == 1)
            gtp_fw = gtp_shenyue_FW;
        else
            gtp_fw = gtp_default_FW;
    }
    else
        gtp_fw = gtp_default_FW;
#endif
...
//end add by zhangmanlin 20140227 
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head *fw_head, char *path)
{
    ...
    s32 ret = 0;
    u8 sensor_id =0;
//add by zhangmanlin on 20140227
#if GTP_UPDTAE_FL_SENSOR_ID    
    #if 0
    if (NULL != path)
    {
		...
		#endif
    GTP_INFO("update by firmware.h with each sensor id!!!!!");
    msleep(50);
    
    ret = i2c_read_bytes(client, GTP_REG_SENSOR_ID, &sensor_id, 1); ///gtp_i2c_read_dbl_check(client,GTP_REG_SENSOR_ID,&sensor_id,1);
    
    if (ret < 0)
    {
    		GTP_ERROR("Failed to get sensor_id, set default firmware");
	    	GTP_FW_point = gtp_default_FW_fl; 
    }
    else
    {
	    GTP_INFO("<<<fl update file>>>,sensor id is: %d",sensor_id);
	    
	    if(sensor_id == 0)
	        GTP_FW_point = gtp_fw_fl_sensor0;
	    else if(sensor_id == 1)
	        GTP_FW_point = gtp_fw_fl_sensor1;
	    else if(sensor_id == 2)
	        GTP_FW_point = gtp_fw_fl_sensor2;
	    else 
	        GTP_FW_point = gtp_default_FW_fl; 
    }
    memcpy(gtp_default_FW_fl,GTP_FW_point,sizeof(gtp_default_FW_fl));
 #endif   
//add by zhangmanlin on 20140227   end //
