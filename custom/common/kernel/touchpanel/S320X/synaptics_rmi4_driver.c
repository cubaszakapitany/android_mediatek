...
#include "SynapticsImage_BM8811.h"
#include "SynapticsImage_JTOUCH8811.h"
...
{	
	...
	//if (status & ts->fn1a_mask) { /* key */
		...
		if (button)
		{
			...
			for (i = 0; i < MAX_KEY_NUM; i++)
			{
			    button_index = button & (0x01 << i);
                        if (FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
                        {
                            if(button_index==1 || button_index==2 || button_index==4)
                            {
                                input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
                                id = i;
                            }
                        ...
                        else
                        {
                            if(button_index==1){
                            	//input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
                            	tpd_down(130,1345,50);
                            	id = i;
                            }
                            if(button_index==2){
                            	//input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
                            	tpd_down(360,1345,50);
                            	id = i;
                            }
                            if(button_index==4){
                            	//input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
                            	tpd_down(580,1345,50);
                            	id = i;
                            }
                        }
			}
...
static int tpd_clear_interrupt(struct i2c_client *client)
{
	int retval = 0;
	u8 status = 0;
        CTP_DBG("ts->f01.data_base + 1=%x",ts->f01.data_base + 1);
...
//	queue_delayed_work(det_workqueue,&det_work,msecs_to_jiffies(EXP_FN_DET_INTERVAL));
    ...
    //LINE <touch panel> <DATE2013821> <for diff vendor upgrade> zhangxiaofei
    product_id = synaptics_get_vendor_info();
    //CTP_DBG("zx: product_id = %s \n",product_id);
    printk("zx: product_id = %s \n",product_id);
    #if 1
    if(product_id)
    {
        #if 1
        if( 0 == memcmp(product_id, "BIEL", 4))
        {
            CTP_DBG("BIEL TP ready to upgrade \n");
            synaptics_fw_updater(SynapticsImage);
        }
        else if( 0 == memcmp(product_id, "BM8811", 6))
        {
            CTP_DBG("BM8811 ready to upgrade BM8811 \n");
            synaptics_fw_updater(SynapticsImage_BM8811);
        }
        else if( 0 == memcmp(product_id, "JTOUCH8811", 10))
        {
            CTP_DBG("JTOUCH ready to upgrade BM8811 \n");
            synaptics_fw_updater(SynapticsImage_JTOUCH8811);
        }
        else
        {
            CTP_DBG("no match TP: \n", product_id);
