...
//BEGIN <tp> <DATE20130608> <tp proximity> zhangxiaofei
#if defined TPD_PROXIMITY 
    start_reg = 0x00;
	memset(buffer, 0xFF, FTS_PROTOCOL_LEN);
	mutex_lock(&g_pts->mutex);
	ret = i2c_master_send(g_pts->client, &start_reg, 1);
	ret = i2c_master_recv(g_pts->client, &buffer[0], 7);// read register
	proximity_status = buffer[1];
	TPD_PROXIMITY_DBG("buffer[0] = 0x%02X\n", buffer[0]);
	TPD_PROXIMITY_DBG("buffer[1] = 0x%02X,  proximity_status = 0x%02X \n", buffer[1], proximity_status);
	TPD_PROXIMITY_DBG("buffer[2] = 0x%02X\n", buffer[2]);
	TPD_PROXIMITY_DBG("buffer[3] = 0x%02X\n", buffer[3]);
	
	//test = i2c_smbus_read_byte_data(g_pts->client, 0xB0);
	//	TPD_PROXIMITY_DBG("-->Enable CTP PS B0 == 0x%x \n",test);
   
	if (proximity_status == TPD_PROXIMITY_CLOSE_VALUE)
	{
		TPD_PROXIMITY_DBG("slaver report i am close...\n");
		ps_data = 0;
	}
	else if(proximity_status == TPD_PROXIMITY_FARAWAY_VALUE)
	{
		TPD_PROXIMITY_DBG("slaver report i am far away...\n");
		ps_data = 1;
	}
	mutex_unlock(&g_pts->mutex);
#endif
//END <tp> <DATE20130608> <tp proximity> zhangxiaofei	
...
//	tpd_dump_touchinfo( ts );
 ...
 {	 
    ...
    TPD_PROXIMITY_DBG("tpd_load_status = %d, ps_data_pre = %d, ps_data = %d", tpd_load_status, ps_data_pre, ps_data);
	if(tpd_load_status&&(ps_data_pre!=ps_data)){
		TPD_PROXIMITY_DBG("come here--->");
...
//BEIN <tp> <DATE20130514> <tp proximity> zhangxiaofei	
#ifdef TPD_PROXIMITY
    TPD_PROXIMITY_DBG("ps_mode =%d\n",ps_mode);
	if(ps_mode == 1){
		TPD_PROXIMITY_DBG("ps_mode =%d, simulate PS,do not suspend!\n",ps_mode);
		 return ;//TPD_OK;
	}
	else{
		TPD_PROXIMITY_DBG("ps_mode =%d, suspend!\n",ps_mode);
	}
#endif
//END <tp> <DATE20130514> <tp proximity> zhangxiaofei
	if ( g_pts ){
...
static void ft6x06_ctp_convert(char mode,bool force)
{
    ...
    int ret = 0;
    char buffer;
    TPD_PROXIMITY_DBG("current_CTP_PS mode=%d\n", mode);
/*	if(force==false)
	{
	...
	{
		ps_mode = 0;
		TPD_PROXIMITY_DBG("DISABLE_CTP_PS mode=%d\n", mode);
		ret = i2c_smbus_write_byte_data(g_pts->client, TPD_PROXIMITY_ENABLE_REG, 0);
		buffer = i2c_smbus_read_byte_data(g_pts->client, TPD_PROXIMITY_ENABLE_REG);
		TPD_PROXIMITY_DBG("Disable CTP PS B0 == 0x%x \n",buffer);
		if(ret < 0)
			TPD_DEBUG("Disable CTP PS Error \n");
	}
	else if(ENABLE_CTP_PS == mode)
	{
		TPD_PROXIMITY_DBG("ENABLE_CTP_PS mode=%d\n", mode);
		ps_mode = 1;
		ret = i2c_smbus_write_byte_data(g_pts->client, TPD_PROXIMITY_ENABLE_REG, 1);
		buffer = i2c_smbus_read_byte_data(g_pts->client, TPD_PROXIMITY_ENABLE_REG);
		TPD_PROXIMITY_DBG("Enable CTP PS B0 == 0x%x \n",buffer);
		if(ret < 0)
			TPD_PROXIMITY_DBG("Enable CTP PS Error \n");
	}
}
...
//BEGIN <tp> <DATE20130608> <tp proximity> zhangxiaofei
#if defined TPD_PROXIMITY 
	.tpd_get_ps  =  ft6x06_get_ps,
	.tpd_convert =  ft6x06_ctp_convert,
#endif
