...
bool tpd_get_ft6x06_IC();
static bool g_Isft6x06_IC = false;
#define TPD_OK 0
...
{
 	printk("==========tpd_proximity_detect%d============",tpd_proximity_detect);
 ...
 char *fts_get_vendor_name(int vendor_id)
{
	switch(vendor_id){
		...
		case FTS_CTP_VENDOR_DIJING:	return "DIJING";	 break; //LINE<TP><ADD TP DIJING><20130926>YOLO
 ...
 bool tpd_get_ft6x06_IC()
 {
	printk("===========tpd_get_ft6x06_IC %d========",g_Isft6x06_IC);
	return g_Isft6x06_IC;
 }
  EXPORT_SYMBOL(tpd_get_ft6x06_IC);
 static int __devinit tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
 {	 
	int retval = TPD_OK;
	int panel_version = 0;
	...
	while (iRetry) {
		...
		if ( panel_version < 0 || panel_vendor<0 || ret<0||panel_version==0xff ){
...
extern char tpd_desc[50];
char* get_TpName( void )
{
  char tpd_desc_info[50];
  sprintf(tpd_desc_info, "%s-S3520AP-FT6x06-%X",get_fw_name(), get_fw_version_ext());
...
void ft6x06_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
 ...
 static struct tpd_driver_t tpd_device_driver = {
	...
	.tpd_have_button = 0,
#endif	
	.tpd_get_ps =  tpd_get_ps_value,
	.tpd_convert = tpd_enable_ps,
        .tpd_TpName = get_TpName,
