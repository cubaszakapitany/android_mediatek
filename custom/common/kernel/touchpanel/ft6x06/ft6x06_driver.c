...
#include "S4031_DJ_Ver0x16_20131225_app.h"
...
static u8 tpd_sleep_flag 			    = 0;
static u8 tpd_PS_sleep_flag 			= 0;
...
static void ft6x06_ctp_convert(int mode,bool force)
{
	if((mode==1)&&(tpd_sleep_flag==1))
		tpd_PS_sleep_flag = 1;
	else
		tpd_enable_ps(mode);
}
...
static char *fts_get_vendor_name(int vendor_id)
{
	switch(vendor_id){
		...
		case FTS_CTP_VENDOR_DIJING_OLD: return "DIJING";break;
