...
extern int gTGesture;
extern bool bTGesture;
static int tpd_flag = 0; 
int tpd_halt = 0;
static struct task_struct *thread = NULL;
static DECLARE_WAIT_QUEUE_HEAD(waiter);

static struct input_dev *GTXX_key_dev;
#define KEY_GTXX_SENSOR 238
...
#define TOUCH_IOC_MAGIC 'B'

#define TPD_GET_VELOCITY_CUSTOM_X _IO(TOUCH_IOC_MAGIC,0)
#define TPD_GET_VELOCITY_CUSTOM_Y _IO(TOUCH_IOC_MAGIC,1)
#define TPD_GET_TP_TYPE           _IO(TOUCH_IOC_MAGIC,2)
...
static long tpd_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    {
  //add by yaohua.li
	case TPD_GET_TP_TYPE:   
		{
			int rc;
			uint8_t vendor = 0;
			uint8_t version = 0;
			int flag =0;
			printk("Try to get TPD TPYE\n!");
			...
			flag = 1;
			if(copy_to_user(data,&flag, sizeof(int))!=0)
			{
	...
	break;
      ...
      //add by yaohua.li
        default:
            printk("asdasdasdasd tpd: unknown IOCTL: 0x%08x\n", cmd);
...
//add by yaohua.li
       tpd_fw_version =tpd_info.vid;
       sprintf(tpd_desc, "ShenYue-S4750AE-GT970-%d", tpd_fw_version);
    return ret;
}
...
//******************************************************************************
//----------------------------------------------------------------------------
static ssize_t GT970_show_state(struct device_driver *ddri, char *buf)
{
	printk(" gt970_state = %s\n",  gt970_state);
	return snprintf(buf, PAGE_SIZE, "%s\n",  gt970_state);
}
//----------------------------------------------------------------------------
static ssize_t GT970_show_key(struct device_driver *ddri, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", enable_input_key);
}
//----------------------------------------------------------------------------
static ssize_t GT970_store_key(struct device_driver *ddri, const char *buf, size_t count)
{
    int enable, res;
    u8 databuf[1];
	...
	{
		printk("invalid enable content: '%s', length = %d\n", buf, count);
	}
	return count;    
}
//---------------------------------------------------------------------------------------
static DRIVER_ATTR( GT970_state,     S_IWUSR | S_IRUGO, GT970_show_state, NULL);
static DRIVER_ATTR(input_key_enable,    0666, GT970_show_key, GT970_store_key);
//----------------------------------------------------------------------------
static struct driver_attribute *GT970_attr_list[] = {
    &driver_attr_GT970_state,
    &driver_attr_input_key_enable,
};

//----------------------------------------------------------------------------
static int GT970_create_attr(struct device_driver *driver) 
{
	int idx, err = 0;
	int num = (int)(sizeof(GT970_attr_list)/sizeof(GT970_attr_list[0]));
	if (driver == NULL)
	{
		return -EINVAL;
	}
	...
	{
		if((err = driver_create_file(driver, GT970_attr_list[idx])))
		{            
			printk("driver_create_file (%s) = %d\n", GT970_attr_list[idx]->attr.name, err);
			break;
		}
	}    
	return err;
...
//----------------------------------------------------------------------------
	static int GT970_delete_attr(struct device_driver *driver)
	{
	int idx ,err = 0;
	int num = (int)(sizeof(GT970_attr_list)/sizeof(GT970_attr_list[0]));
	...
	for (idx = 0; idx < num; idx++) 
	{
		driver_remove_file(driver, GT970_attr_list[idx]);
	}
	
	return err;
}
//----------------------------------------------------------------------------
/*static int GT970_probe(struct platform_device *pdev);
static int GT970_remove(struct platform_device *pdev);
static struct platform_driver GT970_tpd_driver = {
	.probe      = GT970_probe,
	.remove     = GT970_remove,    
	.driver     = {
		.name  = "gt970",
...
struct platform_device sensor_gt970 = {
	.name	       = "gt970",
	.id            = -1,
};
static int GT970_probe(struct platform_device *pdev) 
{
 int err;
printk("==================================GT970_probe================\n");
	if((err = GT970_create_attr(&GT970_tpd_driver.driver)))
	{
...
{	
	int err;		
	if((err =  GT970_delete_attr(&GT970_tpd_driver.driver)))
	{
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //add by yaohua.li
	 strcpy(Tg_buf,"msc");
	// printk("===LGC===",Tg_buf);
         GTXX_key_dev= input_allocate_device();
	if (! GTXX_key_dev) 
	{
		printk("[GTP]GTXX_key_dev : fail!\n");
	}
	__set_bit(EV_KEY,  GTXX_key_dev->evbit);
	__set_bit(KEY_GTXX_SENSOR,  GTXX_key_dev->keybit);
	GTXX_key_dev->id.bustype = BUS_HOST;
	GTXX_key_dev->name = "GT970";
	if(input_register_device(GTXX_key_dev))
	{
		printk("[GTP]GTXX_key_dev register : fail!\n");
	}else
	{
		printk("[GTP]GTXX_key_dev register : success!!\n");
	} 
...
static int tpd_i2c_remove(struct i2c_client *client)
{
	//add by yaohua.li
	int err;		
	input_unregister_device(GTXX_key_dev);
	//add by yaohua.li
#if GTP_CREATE_WR_NODE
    uninit_wr_node();
#endif
	  ...
	  if(bEnTGesture)
{
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            printk("0x814B = 0x%02X", doze_buf[2]);
            if (ret > 0)
            {               
		  ...
		  if ((doze_buf[2] == 'm') || (doze_buf[2] == 's') || (doze_buf[2] == 'c')||(doze_buf[2] == 0x5E))		
                {
                    gTGesture =doze_buf[2];
                    if (doze_buf[2] != 0x5E)
                    {
                      //  printk("===LGC Wakeup by gesture(	%c), light up the screen!", doze_buf[2]);
                    	input_report_key(GTXX_key_dev, KEY_GTXX_SENSOR, 1);
			input_report_key(GTXX_key_dev, KEY_GTXX_SENSOR, 0);
			input_sync(GTXX_key_dev);
                    }
                    else
                    {
			input_report_key(GTXX_key_dev, KEY_GTXX_SENSOR, 1);
			input_report_key(GTXX_key_dev, KEY_GTXX_SENSOR, 0);
			input_sync(GTXX_key_dev);
                        //printk("===LGG Wakeup by gesture(^), light up the screen!");
                    }
...
if(bEnTGesture)
{
   ...
   // printk("1====LGC=bEnTGesture%d===\n",bEnTGesture);
}
 else
{
// printk("2====LGC=bEnTGesture%d===\n",bEnTGesture);
 mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
...
{
         doze_status = DOZE_DISABLED;
	 //printk("====LGC==1==bEnTGesture%d",bEnTGesture);
}
    else
{
         //  printk("====LGC==2==bEnTGesture%d",bEnTGesture);
	 mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
}
#else 
    mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
...
{
    GTP_INFO("MediaTek gt91xx touch panel driver init --tpd_driver_init\n");
 /*   
    if( platform_device_register(&sensor_gt970))
	{
		printk("failed to register driver");
		return 0;
	}
    if(platform_driver_register(&GT970_tpd_driver))
	{
		printk("failed to register driver");
		return 0;
	}*/
