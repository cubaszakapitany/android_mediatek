...
#endif
//add yaohua.li
static struct input_dev *S3202_key_dev;
#define KEY_S3202_SENSOR 238
...
const u16 synaptics_touch_key_array[] = { KEY_MENU, KEY_HOMEPAGE,KEY_BACK };
#define MAX_KEY_NUM ( sizeof(  synaptics_touch_key_array )/sizeof(  synaptics_touch_key_array[0] ) )
#endif
...
#endif
#if 0
#define STP_INFO(fmt,arg...)           printk("<<-STP-INFO->> "fmt"\n",##arg)
#else
#define STP_INFO(fmt,arg...)           do {} while (0)
#endif
static struct point {
	int x;
	int raw_x;
...
extern atomic_t tp__write_flag;
...
int tpd_clear_interrupt(struct i2c_client *client);
extern int synaptics_fw_updater(unsigned char *fw_data);
#if defined(TINNO_ANDROID_S9121)||defined(TINNO_ANDROID_S9320AE)||defined(TINNO_ANDROID_S4750AE)
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retval != PAGE_SELECT_LEN) {
		STP_INFO("tpd_set_page fail, retval = %d\n", retval);
		retval = -EIO;
		goto exit;
	}
 #ifdef MT6577
	...
	while (left_len > 0) {
		...
		#endif	
			if (retval < 0) {
				STP_INFO(KERN_ERR"xxxx focal reads data error!! xxxx\n");
			}
			
			if (retval > 0) {
				break;
			} else {
				STP_INFO("%s: I2C retry %d\n", __func__, retry + 1);
				msleep(20);
			}
		}
#ifdef  SYNAPTICS_DEBUG_IF
...
#ifdef  SYNAPTICS_DEBUG_IF
	STP_INFO("tpd_i2c_read_data ret=%d length=%d \n",retval,length);
	if(length<retval)
	{
		retval=length;
	}
...

{
	...
	while (left_len > 0) {	
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval < 0) {
				STP_INFO(KERN_ERR"xxxxfocal write data error!! xxxx\n");
			}
			if (retval > 0) {
				break;
			} else {
...
exit:
	mutex_unlock(&(ts->io_ctrl_mutex));
#ifdef  SYNAPTICS_DEBUG_IF
	STP_INFO("tpd_i2c_write_data ret=%d length=%d \n",retval,length);
	if(length<retval)
	{
		retval=length;
	}
...
EXPORT_SYMBOL(synaptics_rmi4_proximity_enables);
 ...
 int tpd_rmi4_read_pdt(struct tpd_data *ts)
{
	int retval;
	unsigned char ii;
...
{	u16 temp=0;
	...
	int point_count;
	int ret=0;
    STP_INFO("tpd_process read data to clear interrupt!!\n");

	//clear interrupt bit
	retval = tpd_i2c_read_data(ts->client, ts->f01.data_base + 1, &status, 1);
	if (retval < 0)
	{
		STP_INFO("tpd_i2c_read_data retval error\n");
		ret=0;
		goto exit;
	}
		
         STP_INFO("tpd_process status =%d,ts->fn1a_mask =%d, ts->fn11_mask =%d ts->f1a.data_base =%d \n",status,ts->fn1a_mask,  ts->fn11_mask,ts->f1a.data_base);
       if(status==0)
       {		
       	ret=0;
		goto exit;
	...
	if (status & ts->fn1a_mask) { /* key */
		...
		STP_INFO("======>hx read button!!!! 0x%x \n",button);
		if (button)
		{
			/*for (i = 0; i < MAX_KEY_NUM; i++)
			{
				input_report_key(tpd->dev,  synaptics_touch_key_array[i], button & (0x01 << i));
		...
		//	  #endif
			...
			//  #endif
		    	...
		    	//input_report_key(tpd->dev,  synaptics_touch_key_array[id], 0);
			//#ifdef MULTI_TOUCH_PROTOCOL_TYPE_B	
			 if (FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
			  {
			  	input_report_key(tpd->dev,  synaptics_touch_key_array[id], 0);
    ...
    STP_INFO("======LGC==status:%d!! ts->fn1a_mask=0x%x =====ts->fn11_mask%d=======\n",status,ts->fn1a_mask,ts->fn11_mask);
//	if (status & 0x04) { /* point */
	if (status & ts->fn11_mask) {	
        //yaohua.li
		 ...
		 int icount =0;
	        int retval = tpd_i2c_read_data(ts->client, 0x04b, &status, 1);
		STP_INFO("====LGC===tpd_process status 1=%d\n",status);
                 unsigned char charstatus1[8] = {0};
	         retval = tpd_i2c_read_data(ts->client, 0x04c, &charstatus1, 8);
                STP_INFO("====LGC===tpd_process status i=%d=2=%d\n",icount,charstatus1[6]);
               if(status==0x01)
               {
                #ifdef CUSTOM_KERNEL_TGESTURE_DK
                STP_INFO("=====LGC========status==0x40=\n");
                       gTGesture ='U';
                       input_report_key(S3202_key_dev,KEY_S3202_SENSOR, 1);
                input_report_key(S3202_key_dev,KEY_S3202_SENSOR , 0);
                input_sync(S3202_key_dev);
                #endif
               ...
               {
		   STP_INFO("=====LGC========status==0x40=\n");
                 unsigned char charstatus[8] = {0};
	         retval = tpd_i2c_read_data(ts->client, 0x04c, &charstatus, 8);
		/*for(icount =0;icount < 8;icount++)
		STP_INFO("====LGC===tpd_process status i=%d=2=%d\n",icount,charstatus[icount]);
		STP_INFO("\n");*/
                 switch (charstatus[6])
		{
		case 0x63:
...
static int tpd_sw_power(struct i2c_client *client, int on)
{
	...
	STP_INFO("Read OK.\n");
	if (retval < 0) {
		STP_INFO("Error sensor can not wake up\n");
		goto out;
	}
	...
	if (on)
	{
        ...
        STP_INFO("Write OK.\n");
		if (retval < 0) {
			STP_INFO("Error touch can not leave very-low power state\n");
			goto out;
		}
        ...
        STP_INFO("Write OK.\n");
		if (retval < 0) {
			STP_INFO("Error touch can not enter very-low power state\n");
			goto out;
		}      
	}

out:
    STP_INFO("tpd_sw_power end\n");
	return retval;	
}
...
static int __devexit tpd_remove(struct i2c_client *client)
{
	...
	input_unregister_device(S3202_key_dev);
	return 0;
}
...
void Config_Touch_GPIO(bool  work_or_stop)
{
    ...
    if(work_or_stop)//work
    {
     ...
     STP_INFO("Config_Touch_GPIO end\n");
...
// printk("===LGC===",Tg_buf);
    ...
    if (!i2c_check_functionality(client->adapter,
		...
		STP_INFO("SMBus byte data not supported\n");
		return -EIO;
	}

	ts = kzalloc(sizeof(struct tpd_data), GFP_KERNEL);
	if (!ts) {
		STP_INFO("Failed to alloc mem for tpd_data\n");
		return -ENOMEM;
	}

	td = kzalloc(sizeof(struct tpd_debug), GFP_KERNEL);
	if (!td) {
		STP_INFO("Failed to alloc mem for tpd_debug\n");
	}
	...
	if (retval < 0) {
		STP_INFO("Failed to query device\n");
...
//{
	...
	{ 
		...
		STP_INFO(TPD_DEVICE " failed to create kernel thread: %d\n", retval);
	}
//}	
#endif
	...
	if (!mtk_tpd_wq)
	{
		STP_INFO("Error Could not create work queue mtk_tpd_wq: no memory");
		retval = -ENOMEM;
...
*/	
	    ...
	    //add by yaohua.li
      S3202_key_dev= input_allocate_device();
	if (! S3202_key_dev) 
	{
		STP_INFO("[STP]S3202_key_dev: fail!\n");
	}
        __set_bit(EV_KEY,  S3202_key_dev->evbit);
	__set_bit(KEY_S3202_SENSOR,  S3202_key_dev->keybit);
	S3202_key_dev->id.bustype = BUS_HOST;
	S3202_key_dev->name = "GT970";
	if(input_register_device(S3202_key_dev))
	{
		STP_INFO("[STP]S3202_key_dev register : fail!\n");
	}else
	{
		STP_INFO("[STP]S3202_key_dev register : success!!\n");
	} 
	//end by yaohua.li
	    for(i=0;i<MAX_KEY_NUM;i++)
	   __set_bit( synaptics_touch_key_array[i], tpd->dev->keybit);
#endif	
#endif
...
//	synaptics_fw_updater(synaImage);
	...
	STP_INFO("tpd_probe   TP_Max_X=:%d!!TP_Max_Y=%d\n",TP_Max_X,TP_Max_Y);
	tp_x_for_lcd = LCD_X;
...
//     tp_y_for_lcd = 2120;
#if 0
	STP_INFO("before tp_x_for_lcd = %d, tp_y_for_lcd = %d\n", tp_x_for_lcd, tp_y_for_lcd);
	...
	tpd_i2c_read_data(client, ts->f11.ctrl_base+6, &TP_Max_X,2);
    	tpd_i2c_read_data(client, ts->f11.ctrl_base+8,&TP_Max_Y,2);
#endif
	STP_INFO("after TP_Max_X = %d, TP_Max_Y = %d\n", TP_Max_X, TP_Max_Y);
	...
	tpd_load_status = 1;
	STP_INFO("%s: TouchPanel Device Probe %s\n", __func__, (retval < 0) ? "FAIL" : "PASS");
    return 0;
...
{
	STP_INFO("Synaptics I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
 
	if(i2c_add_driver(&tpd_i2c_driver)!=0)
	{
		STP_INFO("Error unable to add i2c driver.\n");
		return -1;
	}
...
{
 unsigned char buf = 0; 
tpd_i2c_read_data(ts->client, 0X0014, &buf,1); 
 STP_INFO("=====TPD wake up====0x14====buf=====%d\n",buf);
}
...
STP_INFO("=====suspend_reset==XXX============\n");
synaptics_chip_reset();
mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
atomic_set(&suspend_flag,0);
}
mutex_unlock(&tp_mutex);
STP_INFO("=====TPD enter slresume=bEnTGesture1  ==%d============\n",bEnTGesture);
STP_INFO("======TPD wake up============\n");
unsigned char buf = 0; 
tpd_i2c_read_data(ts->client, 0X57, &buf,1); 
 STP_INFO("=====TPD wake up========buf=====%d\n",buf);
buf = (buf>>3)<<3; 
tpd_i2c_write_data(ts->client, 0X57, &buf,1); 
 STP_INFO("=====TPD wake up========buf=====%d\n",buf);

STP_INFO("=====TPD key open============\n");
buf = 0; 
tpd_i2c_read_data(ts->client, 0X202, &buf,1); 
 STP_INFO("=====TPD key open=======buf=====%d\n",buf);
buf = ((buf>>3)<<3)|0x0007; 
tpd_i2c_write_data(ts->client, 0X0202, &buf,1);
STP_INFO("=====TPD enter sleep========buf=====%d\n",buf);
atomic_set(&tp_write_flag,1);
return ;
}
atomic_set(&tp_write_flag,1);
}
STP_INFO("=====TPD enter resume=bEnTGesture2  ==============\n");
    if(atomic_read(&suspend_flag))
    {
...
{
STP_INFO("========tpd_suspend=============\n");
if(atomic_read(&tp_write_flag))
{
atomic_set(&tp_write_flag,0);
STP_INFO("===============tpd_suspend=====bEnTGesture==================\n");
bTPGesturetemp =bEnTGesture;
if(bTPGesturetemp)
{
STP_INFO("=====TPD enter sleep=bEnTGesture1  ==%d============\n",bEnTGesture);
unsigned char  buf = 0; 
...
buf = ((buf>>3)<<3)|0x0004; 
tpd_i2c_write_data(ts->client, 0X57, &buf,1);
STP_INFO("=====TPD enter sleep=============\n");
buf = 0; 
tempbuf = 0; 
tpd_clear_interrupt(ts->client);
tpd_i2c_read_data(ts->client, 0X0202, &buf,1); 
 STP_INFO("=====TPD key close========buf=====%d\n",buf);
buf = ((buf>>3)<<3); 
tpd_i2c_write_data(ts->client, 0X0202, &buf,1);
STP_INFO("=====TPD key close========buf=====%d\n",buf);
 atomic_set(&tp_write_flag,1);
...
atomic_set(&tp_write_flag,1);
   ...
   STP_INFO("=====TPD enter sleep=bEnTGesture2  =============\n");
   if(atomic_read(&Firmware_Update_Flag))
    {
        STP_INFO("Firmware_Update_Flag no need suspend \n");
        return;
    }
    mutex_lock(&tp_mutex);
...
static int __init tpd_driver_init(void)
{
	...
	if(!tpDMABuf_va){
		STP_INFO(KERN_ERR"xxxx Allocate DMA I2C Buffer failed!xxxx\n");
		return -1;
	}
#endif	
	
	STP_INFO("synaptics touch panel driver init\n");
	i2c_register_board_info(0, &i2c_tpd, 1);
	if(tpd_driver_add(&tpd_device_driver) < 0)
		STP_INFO("Error Add synaptics driver failed\n");
	return 0;
}

static void __exit tpd_driver_exit(void)
{
	STP_INFO("synaptics touch panel driver exit\n");
