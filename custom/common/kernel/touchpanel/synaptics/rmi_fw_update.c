...
#if 0
#define STP_INFO_UP(fmt,arg...)           printk("<<-STP-INFO->> "fmt"\n",##arg)
#else
#define STP_INFO_UP(fmt,arg...)           do {} while (0)
#endif
//Tinno:CJ for 6592
#ifdef MT6592
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
		...
		if (rmi_fd.fn_number) {
			...
			case SYNAPTICS_RMI4_F34:
				STP_INFO_UP("mtk-tpd addr=0x%x VALUE=0x%x \n",addr, rmi_fd.ctrl_base_addr);
				f34found = true;
				fwu->f34_fd = rmi_fd;
		#if defined(TINNO_ANDROID_S9320AE)||defined(TINNO_ANDROID_S4750AE) /*force reg addr =0x4c !!!*/	
				fwu->f34_fd.ctrl_base_addr=0x4d;
		#endif
				fwu->intr_mask = 0;
...
extern  int tpd_rmi4_read_pdt(struct tpd_data *ts);
extern  int tpd_clear_interrupt(struct i2c_client *client);
extern void Config_Touch_GPIO(bool  work_or_stop);
extern void tpd_new_probe();
...
tpd_new_probe();
//retval = tpd_rmi4_read_pdt(fwu->rmi4_data);
//tpd_clear_interrupt(fwu->rmi4_data->i2c_client);
...
int synaptics_fw_version_updater()
{
    ...
    STP_INFO_UP("fwu->f34_fd.ctrl_base_addr=0x%x \n",fwu->f34_fd.ctrl_base_addr);
 STP_INFO_UP("config_id =0x%x  0x%x 0x%x 0x%x \n",config_id[0],config_id[1],config_id[2],config_id[3]);
    if(retval<0)
    {
	tpd_fw_version =0;
	...
	sprintf(tpd_desc, "JieMian-S4750AE-S3202-%d", tpd_fw_version);
     	}
	////////////////////////////////////////////////////////////////////////////
	return retval;
}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	fwu->data_pos += count;
      ...
      STP_INFO_UP("fwu_sysfs_store_image count=%d  fwu->data_pos =%d \n",count,fwu->data_pos);
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
	   STP_INFO_UP("err _lock Firmware_Update_Flag \n");
          goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fwu->data_pos = 0;
       ...
       STP_INFO_UP("fwu_sysfs_image_size_store fwu->image_size = %d \n",fwu->image_size);
