...
static int s4BU6424AF_WriteReg(u16 a_u2Data)
{
   ...
   BU6424AFDB("[BU6424AF] g_sr %d, write %d ,p1=0x%x,p2=0x%x\n", g_sr, a_u2Data,((a_u2Data >> 8)&0x03)|0xc4,(a_u2Data & 0x00FF));
...
static int BU6424AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    // FM50 SRC Initial Setting -- Add by truly 2014.05.20
    BU6424AFDB("[BU6424AF] FM50 SRC Init Start!! \n");
    ...
    BU6424AFDB("[BU6424AF] FM50 SRC Init End!! \n");
...
static int BU6424AF_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	msleep(10);   
	s4BU6424AF_WriteReg(150);
	msleep(10);
	s4BU6424AF_WriteReg(100);
	msleep(10);
	s4BU6424AF_WriteReg(50);
	msleep(10);
