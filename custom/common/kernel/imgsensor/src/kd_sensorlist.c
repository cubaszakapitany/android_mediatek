...
//Append by Jieve Liu
//Begin{
static char g_currSensorNames[2][32] = {KDIMGSENSOR_NOSENSOR,KDIMGSENSOR_NOSENSOR};
static char *sensorname_main =& g_currSensorNames[1];
static char *sensorname_sub = &g_currSensorNames[0];
static int sensorinum = 0;
...
********************************************************************************/
#define KD_MULTI_FUNCTION_ENTRY()   PK_XLOG_INFO("[%s]:Enter \n",__FUNCTION__)
#define KD_MULTI_FUNCTION_EXIT()    PK_XLOG_INFO("[%s]:eXit \n",__FUNCTION__)
//
MUINT32
...
********************************************************************************/
#if defined(TINNO_PIP_SUPPORT)
int kdSetDriver(void* pBuf)
#else
int kdSetDriver(unsigned int* pDrvIndex)
#endif
{
#if defined(TINNO_PIP_SUPPORT)
    SENSOR_DRIVER_INDEX_STRUCT *pDrvIndexPara = (SENSOR_DRIVER_INDEX_STRUCT*)pBuf;
    unsigned int *pDrvIndex = pDrvIndexPara->drvIndex;
    g_i4PipMode = pDrvIndexPara->i4PipMode;
#endif
...
#if defined(TINNO_PIP_SUPPORT)
    printk("kdSetDriver: pDrvIndex:0x%08x/0x%08x PIP(%d)\n",
...
#if 0/*Not suport to use old open function to check.-- Jieve*/
		            PK_DBG(" Not implement!!, use old open function to check\n");
		            err = g_pSensorFunc->SensorOpen();
#else
		            PK_DBG(" Not implement!!\n");
...
//Begin{
				...
				printk("<Jieve> Sensor found ID = 0x%x\n", sensorID);
				sensorinum++;
				if(DUAL_CAMERA_MAIN_SENSOR == g_invokeSocketIdx[i]){
				        err = g_pSensorFunc->SensorFeatureControl(
							g_invokeSocketIdx[i], 
							SENSOR_FEATURE_GET_SENSOR_NAME, 
							(MUINT8*)&g_currSensorNames[1], 
							&retLen);
				        if(ERROR_NONE != err)
				        {
						strcpy(&g_currSensorNames[1],&g_invokeSensorNameStr[0]);
				        }
				}
				else{
				        err = g_pSensorFunc->SensorFeatureControl(
							g_invokeSocketIdx[i], 
							SENSOR_FEATURE_GET_SENSOR_NAME, 
							(MUINT8*)&g_currSensorNames[0], 
							&retLen);
				        if(ERROR_NONE != err)
				        {
						strcpy(&g_currSensorNames[0],&g_invokeSensorNameStr[0]);
				        }
				}
		        }
//}Append by Jieve Liu
...
inline static int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	PK_DBG("pScenarioId[0]=%d, pScenarioId[1]=%d\n", pScenarioId[0], pScenarioId[1]);
	...
	for(i=0; i<2; i++) {
	    // SenorInfo
	    if(copy_to_user((void __user *)(pSensorGetInfo->pInfo[i]), 
			(void*)pInfo[i] , 
	    ...
	    // SensorConfig
	    if(copy_to_user((void __user *) (pSensorGetInfo->pConfig[i]) , 
			(void*)pConfig[i] , 
...
#if defined(TINNO_PIP_SUPPORT)
u32 gSearchSendor = -1;
int kdSearchSendor( u32 status)
{
    PK_DBG("[kdSearchSendor]:  status=%d\n", status);
    spin_lock(&kdsensor_drv_lock);
    gSearchSendor = status;
    spin_unlock(&kdsensor_drv_lock);
    return 0;
}
#endif
...
)
{
    ...
    switch(a_u4Command) {
        ...
        case KDIMGSENSORIOC_X_RELEASE_I2C_TRIGGER_LOCK:
            ...
            break;
#if defined(TINNO_PIP_SUPPORT)
        case KDIMGSENSORIOC_X_SEARCH_SENSOR:
            i4RetValue = kdSearchSendor(*pIdx);
            break;
#endif
    	default :
...
//Append by Jieve Liu
//Begin{
module_param(sensorname_main,charp,0644);
module_param(sensorname_sub,charp,0644);
module_param(sensorinum,int,0644);
//}Append by Jieve Liu
//End
