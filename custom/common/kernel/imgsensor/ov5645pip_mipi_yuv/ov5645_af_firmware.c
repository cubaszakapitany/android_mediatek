#include <linux/videodev2.h>
#include <linux/i2c.h>
#include <linux/platform_device.h>
#include <linux/delay.h>
#include <linux/cdev.h>
#include <linux/uaccess.h>
#include <linux/fs.h>
#include <asm/atomic.h>
#include <asm/io.h>
#include <asm/system.h>	 
#include "kd_camera_hw.h"
#include "kd_imgsensor.h"
#include "kd_imgsensor_define.h"
#include "kd_imgsensor_errcode.h"
#include "kd_camera_feature.h"
#include "ov5645mipiyuv_Sensor_m.h"

extern int iBurstWriteReg(u8 *pData, u32 bytes, u16 i2cId) ;
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);
#define OV5645_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para ,1,OV5645_WRITE_ID)

//=====================For iBurstWriteReg AF regs==========================//
static const kal_uint8 addr_data_pair1[254] =
{
	,
};
static const kal_uint8 addr_data_pair2[255] =
{	
	, 
};
static const kal_uint8 addr_data_pair3[255] =
{
	,
};
static const kal_uint8 addr_data_pair4[255] =
{
	,
};
static const kal_uint8 addr_data_pair5[255] =
{
	,

};
static const kal_uint8 addr_data_pair6[255] =
{	
	,
};
static const kal_uint8 addr_data_pair7[255] =
{
	,
};
static const kal_uint8 addr_data_pair8[255] =
{	
	,
};
static const kal_uint8 addr_data_pair9[255] =
{
	,
};
static const kal_uint8 addr_data_pair10[255] =
{
	,

};
static const kal_uint8 addr_data_pair11[255] =
{
	,
};
static const kal_uint8 addr_data_pair12[255] =
{
	,
};
static const kal_uint8 addr_data_pair13[255] =
{
	,
};
static const kal_uint8 addr_data_pair14[255] =
{
	,
};
static const kal_uint8 addr_data_pair15[255] =
{
	,
};
static const kal_uint8 addr_data_pair16[255] =
{	
	,
};
static const kal_uint8 addr_data_pair17[255] =
{
	,
};
static const kal_uint8 addr_data_pair18[255] =
{	
	,
};
static const kal_uint8 addr_data_pair19[255] =
{
	,
};
static const kal_uint8 addr_data_pair20[255] =
{
	,
};
static const kal_uint8 addr_data_pair21[255] =
{	
	,
};
static const kal_uint8 addr_data_pair22[181] =
{
	,
};

/*************************************************************************
* FUNCTION
*   OV5645_FOCUS_OVT_AFC_Init
* DESCRIPTION
*   This function is to load micro code for AF function
* PARAMETERS
*   None
* RETURNS
*   None
* GLOBALS AFFECTED
*************************************************************************/
void OV5645_Burst_FOCUS_OVT_AFC_Init(void)
{
	OV5645_write_cmos_sensor(0x3000,0x20);
	iBurstWriteReg(addr_data_pair1  , 254, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair2  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair3  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair4  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair5  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair6  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair7  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair8  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair9  , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair10 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair11 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair12 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair13 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair14 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair15 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair16 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair17 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair18 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair19 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair20 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair21 , 255, OV5645_WRITE_ID);
	iBurstWriteReg(addr_data_pair22 , 181, OV5645_WRITE_ID);
	OV5645_write_cmos_sensor(0x3022,0x00);
	OV5645_write_cmos_sensor(0x3023,0x00);
	OV5645_write_cmos_sensor(0x3024,0x00);
	OV5645_write_cmos_sensor(0x3025,0x00);
	OV5645_write_cmos_sensor(0x3026,0x00);
	OV5645_write_cmos_sensor(0x3027,0x00);
	OV5645_write_cmos_sensor(0x3028,0x00);
	OV5645_write_cmos_sensor(0x3029,0x7F);
	OV5645_write_cmos_sensor(0x3000,0x00);
	OV5645_write_cmos_sensor(0x3000,0x00);
}

