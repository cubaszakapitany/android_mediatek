...
void GC0329GammaSelect(kal_uint32 GammaLvl)
{
	switch(GammaLvl)
	{
		...
		case GC0329_RGB_Gamma_night:									//Gamma for night mode
			...
			break;
		default:
			GC0329_write_cmos_sensor(0xBF, 0x0E);//m4 bruce
			GC0329_write_cmos_sensor(0xc0, 0x1C);
			GC0329_write_cmos_sensor(0xc1, 0x34);
			GC0329_write_cmos_sensor(0xc2, 0x48);
			GC0329_write_cmos_sensor(0xc3, 0x5A);
...
void GC0329NightMode(kal_bool bEnable)
{
	...
	else 
	{
		...
		if(GC0329_MPEG4_encode_mode == KAL_TRUE)
			...
			GC0329_write_cmos_sensor(0x33, 0x20);
		GC0329_write_cmos_sensor(0xfe, 0x00);
		GC0329GammaSelect(GC0329_RGB_Gamma_m4);//bruce 3
...
void GC0329_Sensor_Init(void)
{
	...
	GC0329_write_cmos_sensor(0x33 , 0x1c); //blk ratio bruce
	...
	GC0329_write_cmos_sensor(0x82 , 0x90); //DN auto DNDD DEC DNDD //0e //55 jambo  90 betty bruce
	GC0329_write_cmos_sensor(0x83 , 0x04);//05 betty bruce
	...
	GC0329_write_cmos_sensor(0x95 , 0x78); //45 betty bruce
	...
	GC0329_write_cmos_sensor(0xfe , 0x00);
	GC0329_write_cmos_sensor(0x9c , 0x0a);//[3:0] dn slope
...
#else //sub
	GC0329_write_cmos_sensor(0xfe , 0x00);
	GC0329_write_cmos_sensor(0xb3 , 0x3d);//40  42 betty bruce
	GC0329_write_cmos_sensor(0xb4 , 0xfd);//00  ff betty
	GC0329_write_cmos_sensor(0xb5 , 0x04);//06  06 betty bruce 0x02 
	...
	GC0329_write_cmos_sensor(0xd3 , 0x48);//bruce 40 
	GC0329_write_cmos_sensor(0xd5 , 0xfb);//00 betty

	GC0329_write_cmos_sensor(0xdd , 0x14); 
	GC0329_write_cmos_sensor(0xde , 0x32);//bruce 


	GC0329_write_cmos_sensor(0x7a , 0x7d); //80 betty
	GC0329_write_cmos_sensor(0x7b , 0x80); 
	GC0329_write_cmos_sensor(0x7c , 0x78); //80 betty bruce
	...
	GC0329_write_cmos_sensor(0x12 , 0x47);	// center weight *2 13bruce
	GC0329_write_cmos_sensor(0x13 , 0x62); //4 //4); // AE Target bruce
	GC0329_write_cmos_sensor(0x15 , 0xc2);//bruce
	...
	GC0329_write_cmos_sensor(0x21 , 0xe8);//c0 betty
	GC0329_write_cmos_sensor(0x22 , 0x60); 
	GC0329_write_cmos_sensor(0x3c , 0xe8);//th bruce e0
	GC0329_write_cmos_sensor(0x3d , 0x84); //slope bruce 98
	GC0329_write_cmos_sensor(0x3e , 0x30); //read 3f for status  45 betty
	...
	GC0329_write_cmos_sensor(0x5d , 0x7a);//bruce
	GC0329_write_cmos_sensor(0x5e , 0x11);
	GC0329_write_cmos_sensor(0x5f , 0x40);
	GC0329_write_cmos_sensor(0x60 , 0x40);
        ...
        MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->PreviewDelayFrame = 6;//00 betty 4
...
BOOL GC0329_set_param_exposure(UINT16 para)
{
	...
	value_luma = (GC0329_NIGHT_MODE?0x2b:0xfb);//00 betty
	value_Y = (GC0329_NIGHT_MODE?0x68:0x50);
	//printk("jacky:GC0329_set_param_exposure para=%d",para);

	switch (para)
	{
		case AE_EV_COMP_n20:
		//case AE_EV_COMP_n13:
			GC0329_write_cmos_sensor(0xd5, value_luma - 0x20);//yixuhong 20140704 modify 0x48
			GC0329_write_cmos_sensor(0xfe, 0x01);
		...
		case AE_EV_COMP_n10:
			GC0329_write_cmos_sensor(0xd5, value_luma - 0x10);//yixuhong 20140704 modify 0x30
			GC0329_write_cmos_sensor(0xfe, 0x01);
