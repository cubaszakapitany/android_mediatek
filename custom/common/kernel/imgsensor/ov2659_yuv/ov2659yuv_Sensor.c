...
static void OV2659InitialSetting(void)
{
	...
	OV2659_write_cmos_sensor(0x500c,0x03); 
	OV2659_write_cmos_sensor(0x500d,0x00); 
	OV2659_write_cmos_sensor(0x500e,0x02); 
	OV2659_write_cmos_sensor(0x500f,0x70);         
	OV2659_write_cmos_sensor(0x5010,0x3f); //5a 56 7-30 0x49
	OV2659_write_cmos_sensor(0x5011,0x00); 
	OV2659_write_cmos_sensor(0x5012,0x66); 
	OV2659_write_cmos_sensor(0x5013,0x03); 
	OV2659_write_cmos_sensor(0x5014,0x00); 
	OV2659_write_cmos_sensor(0x5015,0x02); 
	OV2659_write_cmos_sensor(0x5016,0x70); 
	OV2659_write_cmos_sensor(0x5017,0x3b); //58 4c 7-30
	OV2659_write_cmos_sensor(0x5018,0x00); 
	OV2659_write_cmos_sensor(0x5019,0x66); 
	OV2659_write_cmos_sensor(0x501a,0x02); 
	OV2659_write_cmos_sensor(0x501b,0xd0); //18
	OV2659_write_cmos_sensor(0x501c,0x02); 
	OV2659_write_cmos_sensor(0x501d,0x66); //50
	OV2659_write_cmos_sensor(0x501e,0x34); //52 43 7-30
	...
	OV2659_write_cmos_sensor(0x5061,0x7d);   //be
	OV2659_write_cmos_sensor(0x5062,0x7d);   //be
...
static void OV2659PreviewSetting(void)
{
	...
	//VTS 616
	OV2659_write_cmos_sensor(0x380e, 0x02);
	OV2659_write_cmos_sensor(0x380f, 0xde);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	OV2659SetHVMirror(IMAGE_NORMAL);
	//OV2659_night_mode(1);
	
	mDELAY(100); //LINE <delay the preview screen for 100ms> <20130726> <> panzaoyan
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	pSensorInfo->SensorDrivingCurrent = ISP_DRIVING_2MA;   //LINE <ISP_DRIVING_8MA> <20130726> <decend the driver current from 8mA to 2mA to solve the green line distrub on part meachines>	panzaoyan	
...
UINT32 OV2659YUVSetVideoMode(UINT16 u2FrameRate)
{
	...
	if (line_length >= 0x3000)
	{
		...
		OV2659_write_cmos_sensor(0x3006,0x0f);/////01 101  2X 3X
		spin_lock(&ov2659_drv_lock);
