...
/*
	...
	*/
	...
	if(enable_input_key){
		if((value_hall1_rev != value_hall1)&&(0!=gBackLightLevel))
			//|| ((value_hall2_rev != value_hall2)&&(0!=gBackLightLevel)))
                {
/*	if((value_hall1_rev != value_hall1) || (value_hall2_rev != value_hall2)  ){*/
		...
		input_sync(ak8789_key_dev);
              //  input_report_key(ak8789_key_dev,KEY_POWER, 1);
              //  input_report_key(ak8789_key_dev,KEY_POWER, 0);
		input_sync(ak8789_key_dev);
		}
                else if(value_hall1&&(0==gBackLightLevel))//||(value_hall2&&(0==gBackLightLevel)))
               {
	        input_report_key(ak8789_key_dev, KEY_SENSOR, 1);
                 input_sync(ak8789_key_dev);
		input_report_key(ak8789_key_dev, KEY_SENSOR, 0);
...
static int ak8789_probe(struct platform_device *pdev) 
{
	...
	__set_bit(EV_KEY, ak8789_key_dev->evbit);
        ...
        input_set_capability(ak8789_key_dev, EV_KEY, KEY_POWER);
        input_set_capability(ak8789_key_dev, EV_KEY, KEY_SENSOR);
