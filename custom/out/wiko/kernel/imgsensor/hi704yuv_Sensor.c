...
#define SP2528_OP_CODE_END		0x03		/* End of initial setting. */
	...
	static u8 SP2528_Initialize_from_T_Flash()
	{
		...
		printk("hi704_sd_Initialize_from_T_Flash Open File Success\n");
		...
		for (j=0; j<i; j++)
		{
			...
			{
				HI704_write_cmos_sensor((kal_uint8)SP2528_Init_Reg[j].init_reg, (kal_uint8)SP2528_Init_Reg[j].init_val);
			}
			else
			{
				printk("REG ERROR!\n");
...
UINT32 HI704Open(void)
{
    ...
    //判断手机对应目录下是否有名为hi704_sd 的文件,没有默认参数
...
static kal_uint32 HI704_GetSensorID(kal_uint32 *sensorID)
{
    ...
    *sensorID = HI704_power_on() ;
	...
	//Jieve Liu
	spin_lock(&hi704_yuv_drv_lock);
	strcpy(HI704_sensor.vendor, "CAMERAKING");
	spin_unlock(&hi704_yuv_drv_lock);
...
BOOL HI704_set_param_exposure(UINT16 para)
{
    ...
    case AE_EV_COMP_00:  // +0 EV
        HI704_write_cmos_sensor(0x40,0x84);
        break;    
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_SENSOR_NAME:
		{/*MAX LEN is 31 char*/
			*pFeaturePara = '\0';
			strcat(pFeaturePara, SENSOR_DRVNAME_HI704_YUV );
			strcat(pFeaturePara, "_" );
			strcat(pFeaturePara, HI704_sensor.vendor );
