...
#define OV5645MIPISENSORDB(fmt, arg...)    printk("[OV5645MIPI]%s: " fmt, __FUNCTION__ ,##arg)
#else
...
static void OV5645MIPIYUV_read_otp(void)
{
	int i;
...
static void OV5645MIPIinitalvariable()
{
	...
	if ( is_initialized ){
		OV5645MIPISENSORDB("initialized\n");
		return;
	}
	is_initialized = 1;
...
#if defined(TINNO_PIP_SUPPORT)
extern kal_bool OV5645_Is_Support_AF(void);
extern void OV5645_Get_Modules( kal_uint32 *pModule_id, kal_uint32 *pLens_id);
#else
static kal_bool OV5645_Is_Support_AF(void)
{
	return OV5645MIPISensor.is_support_af;
}

static void OV5645_Get_Modules( kal_uint32 *pModule_id, kal_uint32 *pLens_id)
{
...
static void OV5645PIP_otp_handle(void)
{
	...
	if ( iOTP_Handled ){
		OV5645MIPISENSORDB("iOTP_Handled=%d\n", iOTP_Handled);
		return;
	}
	iOTP_Handled = 1;

	OV5645MIPI_write_cmos_sensor(0x3000, 0x00);
	OV5645MIPI_write_cmos_sensor(0x3004, 0xdf);

	for (addr = 0x3D00; addr <= 0x3D1F; addr++)
		OV5645MIPI_write_cmos_sensor(addr, 0x00);

	OV5645MIPI_write_cmos_sensor(0x3D21, 0x01);
	mdelay(10);
	module_id = OV5645MIPIYUV_read_cmos_sensor(0x3d05);
	lens_id = OV5645MIPIYUV_read_cmos_sensor(0x3d06);
	...
	OV5645MIPISENSORDB("mid1 = 0x%x:\n", OV5645MIPISensor.module_id);
	OV5645MIPISENSORDB("lens_id = 0x%x:\n", OV5645MIPISensor.lens_id);
...
static kal_uint32 OV5645MIPI_GetSensorID(kal_uint32 *sensorID)
{
	...
	*sensorID =0xffffffff;
	...
	for(i=0;i<3;i++)
	{
		...
		OV5645MIPISENSORDB("OV5645MIPI READ ID: %x\n",sensor_id);
...
//static 
void OV5645MIPIInitialSetting_Common(void)
{
...
*************************************************************************/
//static 
void OV5645MIPIInitialSetting_truly_af(void)
{
	OV5645MIPIInitialSetting_Common();
	...
	OV5645MIPI_write_cmos_sensor(0x5583, 0x38);//0x40
	...
	OV5645MIPI_write_cmos_sensor(0x5800, 0x23);//dnp lsc
	...
	OV5645MIPI_write_cmos_sensor(0x5818, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x5819, 0x0c);//
	...
	OV5645MIPI_write_cmos_sensor(0x5823, 0x26);//
	OV5645MIPI_write_cmos_sensor(0x5824, 0x0a);//
...
/*************************************************************************
* FUNCTION
*    OV5645MIPIInitialSetting_truly_ff
*
* DESCRIPTION
*    This function initialize the registers of CMOS sensor.
*
...
*************************************************************************/
//static 
void OV5645MIPIInitialSetting_truly_ff(void)
{
	OV5645MIPIInitialSetting_Common();
...
/*************************************************************************
* FUNCTION
*    OV5645MIPIInitialSetting_truly_af
*
* DESCRIPTION
*    This function initialize the registers of CMOS sensor.
*
...
*************************************************************************/
//static 
void OV5645MIPIInitialSetting_darling_af(void)
{
	OV5645MIPIInitialSetting_Common();
	...
	//From OV Kobe DNP.
	OV5645MIPI_write_cmos_sensor(0x5800, 0x23);//lsc
	...
	OV5645MIPI_write_cmos_sensor(0x5806, 0x0c);//
	OV5645MIPI_write_cmos_sensor(0x5807, 0x08);//
	OV5645MIPI_write_cmos_sensor(0x5808, 0x05);//
	OV5645MIPI_write_cmos_sensor(0x5809, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x580a, 0x08);//
	...
	OV5645MIPI_write_cmos_sensor(0x5838, 0x44);//
	OV5645MIPI_write_cmos_sensor(0x5839, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x583a, 0x2A);//
	OV5645MIPI_write_cmos_sensor(0x583b, 0x48);//
	OV5645MIPI_write_cmos_sensor(0x583c, 0x42);//
	OV5645MIPI_write_cmos_sensor(0x583d, 0xCE);//
...
*************************************************************************/
//static 
void OV5645MIPIInitialSetting_darling_ff(void)
{
	OV5645MIPIInitialSetting_Common();
	...
	OV5645MIPI_write_cmos_sensor(0x5184, 0x25);//
	OV5645MIPI_write_cmos_sensor(0x5185, 0x24);//
	OV5645MIPI_write_cmos_sensor(0x5186, 0x0e);//
	OV5645MIPI_write_cmos_sensor(0x5187, 0x16);//
	OV5645MIPI_write_cmos_sensor(0x5188, 0x0d);//
	OV5645MIPI_write_cmos_sensor(0x5189, 0x74);//0x75
	OV5645MIPI_write_cmos_sensor(0x518a, 0x54);//
	OV5645MIPI_write_cmos_sensor(0x518b, 0xd8);//0xcc
	OV5645MIPI_write_cmos_sensor(0x518c, 0x84);//
	OV5645MIPI_write_cmos_sensor(0x518d, 0x3b);//
	OV5645MIPI_write_cmos_sensor(0x518e, 0x31);//
	OV5645MIPI_write_cmos_sensor(0x518f, 0x52);//
	...
	OV5645MIPI_write_cmos_sensor(0x5199, 0x18);//
	OV5645MIPI_write_cmos_sensor(0x519a, 0x04);//
	OV5645MIPI_write_cmos_sensor(0x519b, 0x00);//
	OV5645MIPI_write_cmos_sensor(0x519c, 0x07);//
	OV5645MIPI_write_cmos_sensor(0x519d, 0x33);//
	OV5645MIPI_write_cmos_sensor(0x519e, 0x38);//
	...
	OV5645MIPI_write_cmos_sensor(0x5304, 0x08);//	; DNS th1
	OV5645MIPI_write_cmos_sensor(0x5305, 0x30);//	; DNS th2
	OV5645MIPI_write_cmos_sensor(0x5306, 0x10);//	; DNS off1 //0x08 0x10
	OV5645MIPI_write_cmos_sensor(0x5307, 0x16);//	; DNS off2 //0x16 0x20
	OV5645MIPI_write_cmos_sensor(0x5309, 0x08);//	; sharpen TH th1
	OV5645MIPI_write_cmos_sensor(0x530a, 0x30);//	; sharpen TH th2
	OV5645MIPI_write_cmos_sensor(0x530b, 0x04);//	; sharpen TH th2
	OV5645MIPI_write_cmos_sensor(0x530c, 0x06);//	; sharpen TH off2
...
void OV5645MIPIInitialSetting(void)
{
	...
	OV5645_Get_Modules(&module_id, &lens_id);
	if (OV5645_MID_TRULY == module_id) {
		switch (lens_id) {
	...
	} else if (OV5645_MID_DARLING == module_id) {
		switch (lens_id) {
		case OV5645_LENS_ID_EP_D420F00:
			OV5645MIPIInitialSetting_darling_af();
			break;
		case OV5645_LENS_ID_UNKNOW_FOR_DARLING_AF:
			OV5645MIPIInitialSetting_darling_af();
			break;
		default:
			OV5645MIPIInitialSetting_darling_ff();
			break;
		}
	} else {
		OV5645MIPIInitialSetting_truly_af();
...
* GLOBALS AFFECTED
*************************************************************************/
static void OV5645MIPI_FOCUS_OVT_AFC_Init(void)
{
	int i, length, address, AF_status;
	OV5645MIPI_write_cmos_sensor(0x3000,0x20);
/*
	length = sizeof(OV5645MIPI_firmware)/sizeof(int);
	address = 0x8000;
	for(i=0;i<length;i++) {
		OV5645MIPI_write_cmos_sensor(address, OV5645MIPI_firmware[i]);
		address++;
	}
*/
	iBurstWriteReg(OV5645AF_addr_data_pair1  , 254, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair2  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair3  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair4  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair5  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair6  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair7  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair8  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair9  , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair10 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair11 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair12 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair13 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair14 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair15 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair16 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair17 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair18 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair19 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair20 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair21 , 255, OV5645MIPI_WRITE_ID);
	iBurstWriteReg(OV5645AF_addr_data_pair22 , 181, OV5645MIPI_WRITE_ID);
...
static void OV5645_FOCUS_OVT_AFC_Get_AF_Status(UINT32 *pFeatureReturnPara32)
{
	...
	*pFeatureReturnPara32 = SENSOR_AF_IDLE;
	...
	mDELAY(1);
	OV5645MIPISENSORDB("enter function:state_3028=0x%x,state_3029=0x%x\n",state_3028,state_3029);
	if (state_3028==0){
	...
	//OV5645MIPISENSORDB("exit function:state_3028=%d,state_3029=%d\n",state_3028,state_3029);
}
...
*
*************************************************************************/
extern int ov5645_mipi_yuv_af_poweron(BOOL On);
...
//	OV5645MIPIinitalvariable(); /*Move to earlier*/
	OV5645MIPIInitialSetting();
	if (OV5645_Is_Support_AF()) {
		OV5645MIPI_FOCUS_OVT_AFC_Init();
		if (false == AF_Power){
			...
			OV5645MIPISENSORDB(" AF Power has already on.\n");
		}
	}
...
UINT32 OV5645MIPIClose(void)
{
	...
	if (OV5645_Is_Support_AF()) {
		...
		{
			OV5645MIPISENSORDB(" AF Power is already off.\n");
		}
	}
	return ERROR_NONE;
}	/* OV5645MIPIClose() */
       ...
       case AE_EV_COMP_20:	                   
		...
		case AE_EV_COMP_00:
			...
			OV5645MIPI_write_cmos_sensor(0x3a1e, 0x18);//	; control zone H
        	 ...
        	 break;
      	case AE_EV_COMP_n20:  // -2 EV
			OV5645MIPI_write_cmos_sensor(0x3a0f, 0x16);//	; AEC in H
			OV5645MIPI_write_cmos_sensor(0x3a10, 0x10);//	; AEC in L
			OV5645MIPI_write_cmos_sensor(0x3a11, 0x3c);//	; AEC out H
			OV5645MIPI_write_cmos_sensor(0x3a1b, 0x16);//	; AEC out L
...
#define FLASH_BV_THRESHOLD 0x19 
...
UINT32 OV5645MIPIFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId,UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_GET_SENSOR_NAME:
			{/*MAX LEN is 31 char*/
				*pFeaturePara = '\0';
				strcat(pFeaturePara, SENSOR_DRVNAME_OV5645_MIPI_YUV );
				if ( OV5645_MID_DARLING == OV5645MIPISensor.module_id ){
