...
//dengyanjun2013 12 05 test
//#define OV5645MIPIYUV_LOAD_FROM_T_FLASH  
#ifdef OV5645MIPIYUV_LOAD_FROM_T_FLASH

kal_uint8 fromsd = 0;
kal_uint16 OV5645MIPI_write_cmos_sensor(kal_uint8 addr, kal_uint8 para);
...
#define SP2528_OP_CODE_END		0x03		/* End of initial setting. */
	...
	u8 SP2528_Initialize_from_T_Flash()
	{
		...
		fp = filp_open("/storage/sdcard0/ov5645mipiyuv_sd.dat", O_RDONLY , 0); 
		if (IS_ERR(fp)) { 
		...
		for (j=0; j<i; j++)
		{
			...
			{
				OV5645MIPI_write_cmos_sensor((kal_uint8)SP2528_Init_Reg[j].init_reg, (kal_uint8)SP2528_Init_Reg[j].init_val);
			}
			else
			{
				printk("REG ERROR!\n");
...
void OV5645MIPIInitialSetting(void)
{
	...
	OV5645MIPI_write_cmos_sensor(0x5181, 0x54);//
	...
	OV5645MIPI_write_cmos_sensor(0x5186, 0x10);// 
	OV5645MIPI_write_cmos_sensor(0x5187, 0x16);//
	OV5645MIPI_write_cmos_sensor(0x5188, 0x10);// 
	OV5645MIPI_write_cmos_sensor(0x5189, 0x73);//0x72 60 0x68
	OV5645MIPI_write_cmos_sensor(0x518a, 0x5a);//0x68 50 0x55
	OV5645MIPI_write_cmos_sensor(0x518b, 0xe0);//
	OV5645MIPI_write_cmos_sensor(0x518c, 0x96);//
	OV5645MIPI_write_cmos_sensor(0x518d, 0x30);//
	OV5645MIPI_write_cmos_sensor(0x518e, 0x2c);//
	OV5645MIPI_write_cmos_sensor(0x518f, 0x55);//
	OV5645MIPI_write_cmos_sensor(0x5190, 0x44);//
	...
	OV5645MIPI_write_cmos_sensor(0x5199, 0xa1);//
...
static void OV5645MIPIPreviewSetting_SVGA(void)
{
	...
	OV5645MIPISetHVMirror(IMAGE_V_MIRROR, SENSOR_MODE_PREVIEW);
...
static void OV5645MIPIFullSizeCaptureSetting(void)
{
	...
	//OV5645MIPI_write_cmos_sensor(0x5306, 0x08);//	; DNS off1
	//OV5645MIPI_write_cmos_sensor(0x5307, 0x10);//	; DNS off2
	//ov5645_mipi_yuv_truly remove capture red by glimon
	OV5645MIPISetHVMirror(IMAGE_V_MIRROR, SENSOR_MODE_CAPTURE);
	if(run_test_potten)
	{
		run_test_potten=0;
		OV5645SetTestPatternMode(1);
...
/*************************************************************************
//沤脣潞炉脢媒卤盲脕驴脦沤露拧脪氓,脟毛啪霉鸥脻卤脿脪毛露拧脪氓脢媒鸥脻卤盲脕驴.
//prview 1280*960 
//16碌脛脮没脢媒卤露 ; n*16*80/1280
//16碌脛脮没脢媒卤露 ; n*16*60/960
//touch_x  脦陋露脭脫艩preview碌脛潞谩脳酶卤锚[0-1280]
//touch_y  脦陋露脭脫艩preview碌脛脳脻脳酶卤锚[0-960]
...
UINT32 OV5645MIPIOpen(void)
{
...
#ifdef OV5645MIPIYUV_LOAD_FROM_T_FLASH  //gepeiwei   120903
    ...
    printk("ov5645mipiyuvOpen File Start\n");
    fp = filp_open("/storage/sdcard0/ov5645mipiyuv_sd.dat", O_RDONLY , 0); 
    if (IS_ERR(fp)) 
    {   
        fromsd = 0;   
