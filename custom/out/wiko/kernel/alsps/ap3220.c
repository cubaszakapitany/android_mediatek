...
struct wake_lock ps_suspend_lock;
...
#if defined(TINNO_ANDROID_S7811A) && !defined(BUILD_UBOOT) && !defined(BUILD_LK)
{
	extern int mtkfb_set_ps_state(int ps_status);
	APS_LOG("ap3220_enable_ps %d!\n", enable);
	mtkfb_set_ps_state(enable);
}
#endif
...
{
	ssize_t res;
	u8 reg_PS_THDL_L, reg_PS_THDL_H;
	u8 reg_PS_THDH_L, reg_PS_THDH_H;
	u8 reg_PS_CALI_L, reg_PS_CALI_H;
	u8 reg_PS_DATA_L, reg_PS_DATA_H;
	u8 reg_PS_CONF;
	u8 reg_PS_LED;
	int ps_thl_low, ps_thl_high;
	int ps_cali;
	int ps_data;
	...
	ap3220_i2c_read_reg(AP3220_REG_PS_THDL_L, &reg_PS_THDL_L);
	ap3220_i2c_read_reg(AP3220_REG_PS_THDL_H, &reg_PS_THDL_H);
    
	ap3220_i2c_read_reg(AP3220_REG_PS_THDH_L, &reg_PS_THDH_L);
	ap3220_i2c_read_reg(AP3220_REG_PS_THDH_H, &reg_PS_THDH_H);

	ap3220_i2c_read_reg(AP3220_REG_PS_CAL_L, &reg_PS_CALI_L);
	ap3220_i2c_read_reg(AP3220_REG_PS_CAL_H, &reg_PS_CALI_H);
	
	ap3220_i2c_read_reg(AP3220_REG_SYS_PS_DATA_LOW, &reg_PS_DATA_L);
	ap3220_i2c_read_reg(AP3220_REG_SYS_PS_DATA_HIGH, &reg_PS_DATA_H);
	
	ap3220_i2c_read_reg(AP3220_REG_PS_CONF, &reg_PS_CONF);

	ap3220_i2c_read_reg(AP3220_REG_PS_LED, &reg_PS_LED);

     	ps_thl_low = (reg_PS_THDL_H<<2) | (reg_PS_THDL_L&0x03);
	ps_thl_high= (reg_PS_THDH_H<<2) | (reg_PS_THDH_L&0x03);
       ps_cali = (reg_PS_CALI_H<<1) | (reg_PS_CALI_L&0x01);
	ps_data = ((reg_PS_DATA_H & 0x3f) << 4 |(reg_PS_DATA_L & 0x0f));

	res = sprintf(buf, "ps_thl_low: %x, ps_thl_high: %x \n ps_cali: %x ps_data: %x \n PS_THDL_L: %x, PS_THDL_H: %x, \n PS_THDH_L: %x, PS_THDH_H: %x \n PS_CAL_L: %x, PS_CAL_H: %x \n PS_DATA_L: %x, PS_DATA_H: %x  \n PS_CONF: %x  \n PS_LED:%x", 
		ps_thl_low, ps_thl_high, ps_cali, ps_data, 
		reg_PS_THDL_L, reg_PS_THDL_H, 
		reg_PS_THDH_L, reg_PS_THDH_H, 
		reg_PS_CALI_L, reg_PS_CALI_H, 
		reg_PS_DATA_L, reg_PS_DATA_H,
		reg_PS_CONF,reg_PS_LED);
	return res;
}
/*----------------------------------------------------------------------------*/
...
static ssize_t ap3220_write_reg(struct device_driver *ddri, const char *buf,size_t count)
{
	u8 addr,val;
	if(!ap3220_obj)
	{
	...
	if(2 == sscanf(buf,"%x %x",&addr,&val))
	{
		ap3220_i2c_write_reg(addr,val);
		printk("ap3220_write_reg addr:%x,val :%x\n",addr,val);	
	}
	else{
		APS_ERR("invalid content:'%s',length = %d\n",buf,count);
	}
...
static DRIVER_ATTR(reg,     S_IWUSR | S_IRUGO, ap3220_show_reg, ap3220_write_reg);
/*----------------------------------------------------------------------------*/
...
//Tinno:CJ
	if(intr_flag)
		wake_lock_timeout(&ps_suspend_lock, HZ * 5);
...
static void ap3220_ps_cali_set_threshold(void)
{
	...
	Irled_data=(g_ps_base_value&0x3000)>>12;
    value_high= (g_ps_base_value&0x03ff) + hw->ps_threshold_high;
    value_low= (g_ps_base_value&0x03ff) + hw->ps_threshold_low;

    if( value_high > 0x300)
    {
        value_high= (g_ps_base_value&0x03ff) + 0x170;
        value_low= (g_ps_base_value&0x03ff) + 0x100;
	APS_LOG("value_high > 0x300:value_high=%x,value_low=%x! \n",value_high,value_low);
    }
	...
	if(Irled_data==0)                                    //for adjusting the distance when seting the Irled to 16.7%
	{
		value_high= value_high-0x70;
		value_low= value_low-0x50;
	}
	...
	atomic_set(&ap3220_obj->ps_thd_val_low,  value_low);
			...
			//PS LED Control
			data = (AP3220_PS_SETTING_LED_PULSE_2 << AP3220_PS_LED_PULSE_SHIFT) & AP3220_PS_LED_PULSE_MASK;
			data2 = (Irled_data << AP3220_PS_LED_RATIO_SHIFT) & AP3220_PS_LED_RATIO_MASK;
			data |= data2;
			res = ap3220_i2c_write_reg(AP3220_REG_PS_LED,data);
			mdelay(30); 
APS_LOG("ap3220_ps_cali_set_threshold:led-set:Irled_data=%x,data=%x! \n",Irled_data,data);
...
static void ap3220_ps_cali_start(void)
{
    ...
    */
	...
	vl_ps_sun=0;
	vl_ps_count=0;
	APS_LOG("ap3220_ps_cali_start:Enter-Irled_flag=%x \n",Irled_flag);
	    for(vl_index = 0; vl_index < 4; vl_index++)
	    {
	        ...
	        APS_LOG("vl_index=%x, vl_read_ps = %x \n",vl_index, vl_read_ps);
		...
		APS_LOG("vl_ps_sun=%x, vl_ps_count = %x \n",vl_ps_sun, vl_ps_count);

	    g_ps_base_value = (vl_ps_sun/vl_ps_count);
	    g_ps_cali_flag = 1;

		if(g_ps_base_value<0x300)
		{	
			Irled_flag=0;
		}
		else
		{
			//PS LED Control
			data = (AP3220_PS_SETTING_LED_PULSE_2 << AP3220_PS_LED_PULSE_SHIFT) & AP3220_PS_LED_PULSE_MASK;
			data2 = ((AP3220_PS_SETTING_LED_RATIO_66-Irled_count++) << AP3220_PS_LED_RATIO_SHIFT) & AP3220_PS_LED_RATIO_MASK;
			data |= data2;
			res = ap3220_i2c_write_reg(AP3220_REG_PS_LED,data);
			mdelay(40);   
			if(Irled_count==4)
		    	...
		    	APS_LOG("ap3220_ps_cali_start:g_ps_base_value=%x,Irled_count=%x,Irled_flag=%x \n",g_ps_base_value,Irled_count,Irled_flag);
		}

	}   
	
	APS_LOG("ap3220_ps_cali_start:exit-Irled_flag=%x \n",Irled_flag);
	APS_LOG("ap3220_ps_cali_start:Irled_count=%x \n",Irled_count);
	APS_LOG("ap3220_ps_cali_start_original:g_ps_base_value=%x \n",g_ps_base_value);
	
	g_ps_base_value=g_ps_base_value|((AP3220_PS_SETTING_LED_RATIO_66 -Irled_count+1)<<12);

	 APS_LOG("ap3220_ps_cali_start_adjust:g_ps_base_value=%x \n",g_ps_base_value);
