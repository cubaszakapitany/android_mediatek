...
/* CHUER. DATE20121220 BEGIN */

static struct proc_dir_entry *proc_entry;

ssize_t proc_flash_ctl( struct file *filp, const char __user *buff,unsigned long len, void *data)
{
	int index = 1;//KD_CONSTANT_FLASHLIGHT_ID;
	int enable=0;
	int level=0;
	char  flash_ctl_data[256];
	char n_0='0';
	printk("proc_flash_ctl 1\n");
	if (copy_from_user( &flash_ctl_data, buff, len)) {
		printk("proc_flash_ctl 2\n");
		return -EFAULT;
	}
	enable=flash_ctl_data[0]-(int)n_0;
	level=((int)flash_ctl_data[1]-(int)n_0)*10+(int)flash_ctl_data[2]-(int)n_0;
	printk("proc_flash_ctl 3 %d %d\n",enable,level);
	kdSetFlashlightDrv(&index);
	if(g_pFlashlightFunc != NULL)
	{
		printk("proc_flash_ctl 4\n");
		g_pFlashlightFunc->flashlight_ioctl(FLASHLIGHTIOC_T_LEVEL,level);
		g_pFlashlightFunc->flashlight_ioctl(FLASHLIGHTIOC_T_ENABLE,enable);
	}else{
		printk("proc_flash_ctl g_pFlashlightFunc is NULL \n");
	}
return len;
}

int init_proc_flash_ctl(void)
{
    int ret=0;
    proc_entry = create_proc_entry( "flash_ctl", 0666, NULL );
    
    if (proc_entry == NULL) {
        ret = -ENOMEM;
    } else {
        proc_entry->write_proc = proc_flash_ctl ;
    }
    return ret;
}
