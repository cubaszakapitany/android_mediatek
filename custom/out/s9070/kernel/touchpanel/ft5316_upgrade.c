...
void add_focaltech(const char * temp_focal, int len)
{
	if(focal_count == 5){
...
char get_fts_ctpm_ver(char* CTPM_FW,int length)
{
	unsigned int ui_sz;
 ...
 char get_fts_ctpm_vendor(char* CTPM_FW,int length)
{
	unsigned int ui_sz;
	ui_sz = length;//sizeof(CTPM_FW);
	printk("CTPM_FW length == 0x%x\n",ui_sz);
