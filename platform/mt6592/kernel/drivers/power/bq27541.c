...
/**********************************************************
  ...
  *
  *********************************************************/  
#if defined (TINNO_SUPPORT_GPIO_I2C)
...
#endif  /* TINNO_SUPPORT_GPIO_I2C */




int bq27541_get_temp(void)
{
	int ret = 0;
	int data = -255;
...
EXPORT_SYMBOL(bq27541_get_temp);

int bq27541_read_soc(void)
{
	int ret = 0;
	int data = -255;
	if(bq27541_state==BQ27541_DRIVER_NOT_PROBE)
	...
	if(bq27541_state==BQ27541_DEVICE_HAS_FOUND)
	{
		...
		printk("[BQ27541]StateOfCharge = %d%%\n", data);
	}
	return data;
}

EXPORT_SYMBOL(bq27541_read_soc);

int bq27541_check_state(void)
{
    int ret = 0;
    
    printk("[bq27541_check_state] %d\n", bq27541_state);

    if (BQ27541_DEVICE_HAS_FOUND == bq27541_state)
        ret = 1;
	return ret;
}

EXPORT_SYMBOL(bq27541_check_state);

int bq27541_get_batvol(void)
{
	int ret = 0;
	int data = -255;
	if(bq27541_state==BQ27541_DRIVER_NOT_PROBE)
	...
	if(bq27541_state==BQ27541_DEVICE_HAS_FOUND)
	{
		...
		printk("[BQ27541]battery vol = %d mV\n", data);
	}
	return data;
}

EXPORT_SYMBOL(bq27541_get_batvol);

short int bq27541_get_current(void)
{
	int ret = 0;
	int data = -255;
	if(bq27541_state==BQ27541_DRIVER_NOT_PROBE)
	...
	if(bq27541_state==BQ27541_DEVICE_HAS_FOUND)
	{
		...
		printk("[BQ27541]battery current = %d mA\n", (short int)data);
	}
	return (short int)data;
}

EXPORT_SYMBOL(bq27541_get_current);
...
void bq27541_debug_interface(void)
{
    int ret = 0;
    int data = 0;
    int count = 3;
    ...
    ret = bq27541_set_cmd_write(0x00, 0x0000);
    ret = bq27541_set_cmd_read(0x00, &data);
    printk("[BQ27541]CONTROL_STATUS = %X\n", data); 

    // Check seal
    while ((data&0x2000) && (data&0x4000) && count) // Seal ?
    {
        // Unseal
        printk("[BQ27541]Unseal %d\n", count); 
        ret = bq27541_set_cmd_write(0x00, 0x0414);
        ret = bq27541_set_cmd_write(0x00, 0x3672);
        ret = bq27541_set_cmd_write(0x00, 0xFFFF);
        ret = bq27541_set_cmd_write(0x00, 0xFFFF);
    ...
    // Get shutdown voltage
    ret = bq27541_set_cmd_write(0x3e, 0x50);
    ret = bq27541_set_cmd_write(0x3f, 0x01);
    ret = bq27541_set_cmd_read(0x52, &data);
    printk("[BQ27541]REG[52] = %X\n", data);
...
/**********************************************************
  ...
  *********************************************************/
#if defined (TINNO_SUPPORT_GPIO_I2C)
#define BQ27541_BUSNUM 3
#else
#define BQ27541_BUSNUM 2
#endif  /* TINNO_SUPPORT_GPIO_I2C */
