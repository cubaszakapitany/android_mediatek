...
#ifdef  TINNO_SUPPORT_GPIO_I2C
#include <mach/mt_gpio.h>			//add mtk gpio_i2c
...
u32 g_battery_init_voltage;
u32 g_battery_init_current;
u32 g_battery_init_charger;
EXPORT_SYMBOL(g_battery_init_voltage);
EXPORT_SYMBOL(g_battery_init_current);
EXPORT_SYMBOL(g_battery_init_charger);
...
static void gpio_i2c_config_gpio(struct i2c_gpio_platform_data *data,int on)		//add mtk gpio_i2c
{	
	if(on)	
	{	    
		mt_set_gpio_mode(data->scl_pin, GPIO_MODE_00);		//GPIO mode	    
		mt_set_gpio_dir(data->scl_pin,GPIO_DIR_OUT);	    
		mt_set_gpio_out(data->scl_pin,GPIO_OUT_ONE);		//output high	    
		mt_set_gpio_mode(data->sda_pin, GPIO_MODE_00);		//GPIO mode	    
		mt_set_gpio_dir(data->sda_pin,GPIO_DIR_OUT);	    
		mt_set_gpio_out(data->sda_pin,GPIO_OUT_ONE);		//output high	

		mt_set_gpio_pull_enable(data->sda_pin, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(data->sda_pin, GPIO_PULL_UP);
		mt_set_gpio_pull_enable(data->scl_pin, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(data->scl_pin, GPIO_PULL_UP);
	}	
	else	{	    
		mt_set_gpio_out(data->scl_pin,GPIO_OUT_ONE);		//output high	    
		mt_set_gpio_dir(data->scl_pin,GPIO_DIR_IN);	    
		mt_set_gpio_out(data->sda_pin,GPIO_OUT_ONE);		//output high	    
		mt_set_gpio_dir(data->sda_pin,GPIO_DIR_IN);	
	}
}

static struct i2c_gpio_platform_data resources_gpio_i2c[] = {	
	{		
		.sda_pin=(118 | 0x80000000),				//add mtk gpio_i2c
		.scl_pin=(119 | 0x80000000),		
		.udelay=2,		
		.sda_is_open_drain=1,		
		.scl_is_open_drain=1,		
		.is_sccb = 0,		
		.is_common = 1,		
		.gpio_i2c_config_gpio = gpio_i2c_config_gpio,	
	},
};


struct platform_device gpio_device_i2c = {	
	.name		= "i2c-gpio",	
	.id		= 3,				//add mtk gpio_i2c
	.dev = {       	.platform_data = &resources_gpio_i2c,	
	},
};
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    for (; tags->hdr.size; tags = tag_next(tags)) {
        ...
        #if defined (TINNO_BATTERY_FEATURE)
        else if(tags->hdr.tag == ATAG_BATTERY){
            g_battery_init_voltage = tags->u.battery.bat_voltage;
            g_battery_init_current = tags->u.battery.bat_current;
            g_battery_init_charger = tags->u.battery.bat_charger;
            printk("LK battery vol=%dmV current=%dmA charger=%d\n", 
                g_battery_init_voltage, g_battery_init_current, g_battery_init_charger);
        }
        #endif  /* TINNO_BATTERY_FEATURE */
    }
...
*/		
		...
		printk("[mt_board_init] get_boot_mode return %d \n",boot_mode);
		if((NORMAL_BOOT != boot_mode) && (FACTORY_BOOT != boot_mode) && (ADVMETA_BOOT != boot_mode) && (ATE_FACTORY_BOOT != boot_mode))
		{
...
#endif
#ifdef  TINNO_SUPPORT_GPIO_I2C
	retval = platform_device_register(&gpio_device_i2c);	
		if (retval != 0){		
			printk("gpio_device_i2c platform_device_register error!!\n");		 
			return retval;	
	}
#endif
