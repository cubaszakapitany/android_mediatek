...
void DSI_set_cmdq_V2(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	...
	if (0 != DSI_REG->DSI_MODE_CTRL.MODE){//not in cmd mode
		...
		if (cmd < 0xB0 ||lcm_params->dsi.force_dcs_packet)//JSTINNO_SRC xiaoyan.yu,  modify for forcing use 39h command for long packet ,DATE20140714-01 LINE
		{
			if (count > 1)
			{
				vm_cmdq.LONG_PKT = 1;
...
//       DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", " DSI_RX_STA : 0x%x \n", DSI_REG->DSI_RX_STA);
	...
	if(packet_type == 0x1A || packet_type == 0x1C)
    {
       ...
       {
             // [JSTINNO_SRC xiaoyan.yu,  NOTE, DATE20140319-01 START
             recv_data_cnt = read_data0.byte0 & 0x0F; 
             // JSTINNO_SRC xiaoyan.yu, DATE20140319-01 END]
