...
#ifdef TINNO_ANDROID_S4750AE//TINNO_ANDROID_S9320AE
void boot_mode_menu_select()
{
	...
	mt65xx_backlight_on();
	while(mtk_detect_key(MT65XX_RECOVERY_KEY)||mtk_detect_key(MT65XX_MENU_OK_KEY))//VOL_UP
	{
		printf("%s Volume up or Volume down is pressed\n",MODULE_NAME);
	}
	...
	while(1)
	{
               /* if(mtk_detect_key(MT65XX_FACTORY_KEY))
                   {
                          video_printf("MT65XX_FACTORY_KEY :XXXXX !\n");
                          //return;
                   }
                   if(mtk_detect_key(MT65XX_RECOVERY_KEY))
                   {
                          video_printf("MT65XX_RECOVERY_KEY :?FFFFFFFFFFFFFFFFFFFF!\n");
                          //return;
                   }*/
		if(mtk_detect_key(MT65XX_RECOVERY_KEY))//VOL_UP
		{
			select=0;
			break;
		}
