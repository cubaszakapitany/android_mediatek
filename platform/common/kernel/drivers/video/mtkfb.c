...
//<version & devices info><20140329><guomingyi add start>
char lcd_desc[100]= {"Hello world !"};
static ssize_t lcd_desc_show(struct device *dev, struct device_attribute *attr, char *buf) {
	//get name from: disp_drv.c -BOOL DISP_SelectDeviceBoot(const char* lcm_name)
	printk("gmy:DEVICE_ATTR write to lcm_info :%s\n", lcd_desc);
	return sprintf(buf, "%s\n", lcd_desc);
}
static DEVICE_ATTR(lcm_info, 0444, lcd_desc_show, NULL);
//<version & devices info><20140329><guomingyi add end>
...
//<version & devices info><20140329><guomingyi add start>
	if(device_create_file(&pdev->dev, &dev_attr_lcm_info)) {
	    printk("gmy:create lcd_info file error!\n");
	}
	else {
		printk("gmy:create lcd_info file ok: %s\n", lcd_desc);
	}
//<version & devices info><20140329><guomingyi add end>
