...
#define DISABLE_CLEAR_INTR

#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && (CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
static struct task_struct *disp_irq_rdma_task = NULL;
static wait_queue_head_t disp_irq_rdma_wq;

static struct task_struct *disp_irq_wdma_task = NULL;
static wait_queue_head_t disp_irq_wdma_wq;
#ifdef DISABLE_SECURE_INTR
static struct task_struct *disp_irq_ovl_task = NULL;
static wait_queue_head_t disp_irq_ovl_wq;
#endif
extern unsigned int init_tplay_handle(void);
extern unsigned int is_secure_port;
extern int clear_rdma_intr_secure(unsigned int reg_val);
extern int clear_wdma_intr_secure(unsigned int reg_val);
extern int clear_ovl_intr_secure(unsigned int reg_val);
#endif
...
volatile static unsigned int gRegVal = 0;
volatile static unsigned int gWdmaRegVal = 0;
volatile static unsigned int gOvlRegVal = 0;
volatile static unsigned int gRdmaSecureIrq = 0;
volatile static unsigned int gWdmaSecureIrq = 0;
#endif

#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
static int disp_irq_rdma_kthread_func(void *data)
{
    while(1)
    {
        wait_event_interruptible(disp_irq_rdma_wq, (gRegVal != 0));
        {
            // clear intr and enable
            //   in secure world, it will use ~value to clear register
            DISP_DBG("[SVP] clear intr RDMA with value=0x%x \n", gRegVal);
            clear_rdma_intr_secure(gRegVal);
            gRegVal = 0;

            enable_irq(MT6582_DISP_RDMA_IRQ_ID);
        }
    }
...
static int disp_irq_wdma_kthread_func(void *data)
{
    while(1)
    {
        wait_event_interruptible(disp_irq_wdma_wq, (gWdmaRegVal != 0));
        {
            // same with RDMA
            DISP_DBG("[SVP] clear intr WDMA with value=0x%x \n", gWdmaRegVal);
            clear_wdma_intr_secure(gWdmaRegVal);
            gWdmaRegVal = 0;

            enable_irq(MT6582_DISP_WDMA_IRQ_ID);
        }
    }
...
#ifdef DISABLE_SECURE_INTR
static int disp_irq_ovl_kthread_func(void *data)
{
    while(1)
    {
        wait_event_interruptible(disp_irq_ovl_wq, (gOvlRegVal != 0));
        {
            // same with RDMA
            DISP_DBG("[SVP] clear intr OVL with value=0x%x \n", gOvlRegVal);
            clear_ovl_intr_secure(gOvlRegVal);
            gOvlRegVal = 0;

            enable_irq(MT6582_DISP_OVL_IRQ_ID);
        }
    }
...
static /*__tcmfunc*/ irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
    ...
    switch(irq)
    {
...
#ifndef DISABLE_CLEAR_INTR
#ifdef DISABLE_SECURE_INTR
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
                // for secure mode, do not clear intr immediately, instead, trigger a thread
                // to clear intr status in secure world
                if (1 == is_secure_port && 0 != reg_val)
                {
                    gOvlRegVal = reg_val;
                    disable_irq_nosync(MT6582_DISP_OVL_IRQ_ID); // disable irq first

                    // wake up the kthread to clear intr in secure world
                    wake_up_interruptible(&disp_irq_ovl_wq);
                }
                else
                {
#endif
        ...
        case MT6582_DISP_WDMA_IRQ_ID:
                ...
                {
                    gWdmaRegVal = reg_val;
                    disable_irq_nosync(MT6582_DISP_WDMA_IRQ_ID); // disable irq first

                    // wake up the kthread to clear intr in secure world
                    wake_up_interruptible(&disp_irq_wdma_wq);
                }
                else
                {
#endif
...
//	                      }
                ...
                {
                    gRegVal = reg_val;
                    disable_irq_nosync(MT6582_DISP_RDMA_IRQ_ID); // disable irq first

                    // wake up the kthread to clear intr in secure world
                    wake_up_interruptible(&disp_irq_rdma_wq);
                }
                else
                {
#endif
...
//            disp_set_needupdate(DISP_MODULE_BLS , 0);
        ...
        case DISP_IOCTL_SET_TPLAY_HANDLE:
            ...
            disp_path_notify_tplay_handle(value);
            break;
#endif
        default :
...
#ifndef DISABLE_CLEAR_INTR
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
    init_waitqueue_head(&disp_irq_rdma_wq);
    disp_irq_rdma_task = kthread_create(disp_irq_rdma_kthread_func, NULL, "disp_irq_rdma_kthread");
    if (IS_ERR(disp_irq_rdma_task)) 
    {
        DISP_ERR("DISP_InitVSYNC(): Cannot create disp_irq_rdma_task kthread\n");
    }
    wake_up_process(disp_irq_rdma_task);

    init_waitqueue_head(&disp_irq_wdma_wq);
    disp_irq_wdma_task = kthread_create(disp_irq_wdma_kthread_func, NULL, "disp_irq_wdma_kthread");
    if (IS_ERR(disp_irq_wdma_task)) 
    {
        DISP_ERR("DISP_InitVSYNC(): Cannot create disp_irq_wdma_task kthread\n");
    }
    wake_up_process(disp_irq_wdma_task);
#ifdef DISABLE_SECURE_INTR
    init_waitqueue_head(&disp_irq_ovl_wq);
    disp_irq_ovl_task = kthread_create(disp_irq_ovl_kthread_func, NULL, "disp_irq_ovl_kthread");
    if (IS_ERR(disp_irq_ovl_task)) 
    {
        DISP_ERR("DISP_InitVSYNC(): Cannot create disp_irq_ovl_task kthread\n");
    }
    wake_up_process(disp_irq_ovl_task);
#endif
#endif
#endif
