...
extern unsigned int get_tplay_handle(void);
extern int alloc_decouple_buffer(void);
extern int map_decouple_mva(void);
extern int prepare_decouple_buffer_handle(unsigned int w, unsigned int h, unsigned int bpp,
                                   unsigned int *pHandle, unsigned int cnt);
extern unsigned int decouple_buffer_handles[3];
extern int debug_start_isrh(void);
extern int debug_stop_isrh(void);
...
static void process_dbg_opt(const char *opt)
{
    ...
    {
        int ret = disp_path_notify_tplay_handle(0);
        DISP_MSG("debug notify_tplay_handle, ret=%d \n", ret);
    }
    else if (0 == strncmp(opt, "decouple_buffer", 15))
    {
        int ret = alloc_decouple_buffer();
        DISP_MSG("debug alloc_decouple_buffer, ret=%d \n", ret);
    }
    else if (0 == strncmp(opt, "map_buffer", 10))
    {
        int ret = map_decouple_mva();
        DISP_MSG("debug map_decouple_mva ret=%d \n", ret);
    }
    else if (0 == strncmp(opt, "prepare_decouple_buffer", 23))
    {
        int ret = prepare_decouple_buffer_handle(540,960,3,decouple_buffer_handles,3);
        DISP_MSG("debug prepare_decouple_buffer_handle ret=%d\n", ret);
    }
    else if (0 == strncmp(opt, "start_isrh", 10))
    {
        int ret = debug_start_isrh();
    }
    else if (0 == strncmp(opt, "stop_isrh", 9))
    {
        int ret = debug_stop_isrh();
    }
    else if (0 == strncmp(opt, "prepare_display_secure", 22))
    {
        int ret = prepare_display_secure();
    }
    else if (0 == strncmp(opt, "release_display_secure", 22))
    {
        int ret = release_display_secure();
    }
#endif
    else
    {
