...
static atomic_t gCmdqEnableProfile = ATOMIC_INIT(0);
...
static int32_t cmdq_read_profile_enable_proc(char    *pPage,
                                         char    **ppStart,
                                         off_t   offset,
                                         int32_t count,
                                         int32_t *pEOF,
                                         void    *pData)
{
    ...
    length = sprintf(&pPage[offset], "enable profile: %d\n", atomic_read(&gCmdqEnableProfile));
    *pEOF = 1;
        
    return length;
}

static int cmdq_write_profile_enable_proc(struct file* file, 
                                     const char *buf, 
                                     unsigned long count, 
                                     void *data)
{
    ...
    cmdq_core_set_profile_enabled(value);
...
void cmdq_init_procfs(void)
{
    ...
    if (NULL != gCmdqProcEntry)
    {
        ...
        pEntry = create_proc_entry("profile_enable", 0660, gCmdqProcEntry);
        if (NULL != pEntry)
        {
            pEntry->read_proc = cmdq_read_profile_enable_proc;
            pEntry->write_proc = cmdq_write_profile_enable_proc; 
        }
...
void cmdq_deinit_procfs(void)
{
    if (NULL != gCmdqProcEntry)
    {
        ...
        remove_proc_entry("profile_enable", gCmdqProcEntry);
...
{
    return (atomic_read(&gCmdqEnableProfile));
}

void cmdq_core_set_profile_enabled(int32_t enable)
{
    CMDQ_LOG("profile flag: 0x%08x\n", enable);
    
    atomic_set(&gCmdqEnableProfile, enable);
}
