...
extern unsigned int is_decouple_secure_port_wdma;
extern unsigned int is_decouple_secure_port_rdma;
extern unsigned int need_release_rdma_secure_port;
extern unsigned int need_release_wdma_secure_port;

extern int config_ovl_layer_secure(void* config, uint32_t len, uint32_t secure);
extern int config_ovl_roi_secure(unsigned int w, unsigned int h);
...
extern int prepare_display_secure(void);
extern int release_display_secure(void);
extern int config_rdma_secure(void *config, uint32_t len);
extern int config_wdma_secure(void* config, uint32_t len);
extern int notify_decouple_mva(unsigned int mva);
extern int switch_wdma_secure_port(unsigned int secure);
extern int switch_rdma_secure_port(unsigned int secure);
extern int switch_rdma_apc(unsigned int secure);
extern int switch_wdma_apc(unsigned int secure);
extern int switch_ovl_apc(unsigned int secure);
#endif
...
int disp_path_config_rdma (RDMA_CONFIG_STRUCT* pRdmaConfig) {
    ...
    if (1 == is_secure_port) {
        switch_rdma_apc(1);
        config_rdma_secure((void *)pRdmaConfig, sizeof(RDMA_CONFIG_STRUCT));
        switch_rdma_secure_port(1);
    } else {
#endif
...
int disp_path_wait_frame_done(void) {
    ...
    //ret = wait_event_interruptible_timeout(mem_out_wq, mem_out_done, disp_ms2jiffies(200) );
    //ret = wait_event_interruptible_timeout(mem_out_wq, mem_out_done, disp_ms2jiffies(300) );
    if(ret==0)  {
...
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
    if (DISP_SECURE_BUFFER == is_ovl_secured())
    {
        // 0. necessary preparation before ovl layer config secure
        if (is_secure_port == 0)
        {
            prepare_display_secure();
        }

        switch_ovl_apc(1);
        // 1. config secure
        config_ovl_layer_secure(pOvlConfig, sizeof(OVL_CONFIG_STRUCT), pOvlConfig->security);
        // 2. switch port
        switch_ovl_secure_port(1);
    }
    else if (DISP_PROTECT_BUFFER == is_ovl_secured())
    {
        ;
    }
    else
    {
        // when the port is still secured, we need to do secure configuration
        if (is_secure_port == 1)
        {
            config_ovl_layer_secure(pOvlConfig, sizeof(OVL_CONFIG_STRUCT), pOvlConfig->security);
        }
