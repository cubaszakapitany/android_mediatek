...
static int m4u_print_active_port_tee(unsigned int m4u_index)
{
	int  ret;    
    mutex_lock(&m4u_dci_mutex);
	if(NULL == m4u_dci_msg) 
...
static unsigned int m4u_alloc_sec_pt_for_debug(void)
{
    ...
    sec_va = (unsigned int)dma_alloc_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int), &sec_pa, GFP_KERNEL);
    if(!sec_va)
    {
    ...
    if((sec_pa&M4U_PAGE_TABLE_ALIGN)!=0)
    {
        ...
        dma_free_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int), (void *)sec_va, sec_pa);
        ...
        sec_va = (tmp+M4U_PAGE_TABLE_ALIGN)&(~M4U_PAGE_TABLE_ALIGN);
...
static int m4u_systrace_cmd(unsigned int event, unsigned long buf_pa, 
					size_t size, unsigned int *head)
{
    int ret;
    mutex_lock(&m4u_dci_mutex);
	...
	m4u_dci_msg->cmd = CMD_M4U_SYSTRACE;
	m4u_dci_msg->systrace_param.event = event;
	m4u_dci_msg->systrace_param.buf = buf_pa;
	...
	if(head)
		*head = m4u_dci_msg->systrace_param.head;
...
static int m4u_enable_systrace(void *buf, size_t size)
{
	return m4u_systrace_cmd(M4U_SEC_SYSTRACE_START, __pa(buf), size, NULL);
}
static int m4u_disable_systrace(void)
{
	return m4u_systrace_cmd(M4U_SEC_SYSTRACE_STOP, 0, 0, NULL);
}
static int m4u_pause_systrace(unsigned int *head)
{
	return m4u_systrace_cmd(M4U_SEC_SYSTRACE_PAUSE, 0, 0, head);
}
static int m4u_resume_systrace(void)
{
	return m4u_systrace_cmd(M4U_SEC_SYSTRACE_RESUME, 0, 0, NULL);
}



static struct secure_callback_funcs m4u_systrace_ops = {
	.enable_systrace = m4u_enable_systrace,
	.disable_systrace = m4u_disable_systrace,
	.pause_systrace = m4u_pause_systrace,
	.resume_systrace = m4u_resume_systrace,
};



static int m4u_systrace_init()
{
	unsigned int va, pa;
	int ret;

	ret = m4u_sec_systrace_init(SEC_SYSTRACE_MODULE_M4U, "m4u", "m4u", 
		1024*16, &m4u_systrace_ops);

	if(ret)
		M4UMSG("error: init systrace fail %d\n", ret);
	return ret;;
}
...
/*
static int m4u_sec_deinit(void)
{
    enum mc_result mcRet;
    //Close session
...
static int smi_reg_backup_sec(void) {return 0;}
static int smi_reg_restore_sec(void) {return 0;}
...
int m4u_do_debug_command(unsigned int command)
{
    ...
    switch(command)
    {
        ...
        case 22:
            m4u_dump_sec_pgtable(0, (unsigned int)2*1024*1024*1024);
            break;
