...
void disp_bls_init(unsigned int srcWidth, unsigned int srcHeight)
{
    ...
    clock_is_on(MT_CG_MDP_BLS_26M_SW_CG),//MT_CG_MDP_BLS_26M_SW_CG
    clock_is_on(MT_CG_PWM_MM_SW_CG));//MT_CG_PWM_MM_SW_CG
...
//            enable_clock(MT_CG_DISP0_SMI_LARB0, "DDP");
        ...
        DISP_MSG("####-----------> DISP_REG_SET(DISP_REG_BLS_DEBUG, 0x3)2222\n");
#endif
    
            DISP_REG_SET(DISP_REG_BLS_PWM_DUTY, DISP_REG_GET(DISP_REG_BLS_PWM_DUTY));
            DISP_REG_SET(DISP_REG_BLS_PWM_CON, 0x0);
            DISP_MSG("####-----------> DISP_REG_SET(DISP_REG_BLS_PWM_CON, 0x0)1111--------\n");
...
printk("tcp_test:disp_bls_set_backlight start=%d\n",level);
    mutex_lock(&backlight_mutex);
...
//	    }
    mutex_unlock(&backlight_mutex);
printk("tcp_test:disp_bls_set_backlight end\n");
    return 0;    
...
DISP_MSG("####------------>disp_bls_set_backlight:2222\n");
    mutex_lock(&backlight_mutex);
    disp_aal_lock();
