...
//#define TN_DISPLAY 1	//ProjectConfig.mk
//TN801031 added 2013.06.17

//TN801031 modified 2013.05.21
#if TN_DISPLAY
...
//TN801031 modified 2013.06.21
#if TN_DISPLAY
bool dsi_noncont_clk_enabled = false;
unsigned int dsi_noncont_clk_period = 1;
#else
...
//TN801031 modified 2013.06.21
#if TN_DISPLAY
volatile bool dsiStartTransfer = false;
volatile bool dsiTeEnable = true;
#else
...
volatile bool dsiTeExtEnable = false;
...
//TN801031 modified 2013.06.03
DSI_STATUS DSI_WaitForEngineNotBusy(void)
//DSI_STATUS DSI_WaitForEngineNotBusy2(void)
//TN801031 modified 2013.06.03
{
   int timeOut;
#if ENABLE_DSI_INTERRUPT
...
//TN801031 added 2013.05.27
DSI_STATUS DSI_StartTransfer2(bool isMutexLocked);
#if TN_DISPLAY
extern DSI_STATUS DSI_StartTransfer3(bool isMutexLocked);
#endif

DSI_STATUS DSI_StartTransfer(bool isMutexLocked)
{
#if TN_DISPLAY
	return(DSI_StartTransfer3(isMutexLocked));
#else
	return(DSI_StartTransfer2(isMutexLocked));
#endif
}
//TN801031 added 2013.05.27

//TN801031 modified 2013.06.15
DSI_STATUS DSI_StartTransfer2(bool isMutexLocked)
//DSI_STATUS DSI_StartTransfer(bool isMutexLocked)
//TN801031 modified 2013.06.15
{
    ...
    DSI_WaitForEngineNotBusy();
//tinnohd mask
    // //add by MTK haitao for send Hsync package
#if TN_DISPLAY
#else
        while(DSI_REG->DSI_INTSTA.BUSY);  
        OUTREG32(&DSI_REG->DSI_INTSTA, 0x0);
...
void DSI_set_cmdq_V2(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	...
	else
	{
		...
		{
			t2.CONFG = 2;
			t2.Data_ID = DSI_DCS_LONG_PACKET_ID; //yixuhong 20130701 DSI_GERNERIC_LONG_PACKET_ID;
			t2.WC16 = count+1;
...
#else
   tmp_reg.HSTX_CKLP_EN = 1;//need customization??? by TXD IC FAE
#endif
   //tmp_reg.HSTX_CKLP_EN = 1;//need customization??? by TXD IC FAE//tinnohd modify for ili9806h
   OUTREG32(&DSI_REG->DSI_TXRX_CTRL, AS_UINT32(&tmp_reg));
   
   return DSI_STATUS_OK;
}

//TN801031 added 2013.05.29
DSI_STATUS DSI_PS_Control2(unsigned int ps_type, unsigned int vact_line, unsigned int ps_wc);
#if TN_DISPLAY
extern DSI_STATUS DSI_PS_Control3(unsigned int ps_type, unsigned int vact_line, unsigned int ps_wc);
#endif

DSI_STATUS DSI_PS_Control(unsigned int ps_type, unsigned int vact_line, unsigned int ps_wc)
{
#if TN_DISPLAY
    return DSI_PS_Control3(ps_type, vact_line, ps_wc);
#else
    return DSI_PS_Control2(ps_type, vact_line, ps_wc);
#endif

}
//disp_drv_dpi.c
extern UINT32 dsiTmpBufBpp;// = 0;
...
//zx++
void DSI_switch_to_cmd_mode(void)
{

         DSI_MODE_CTRL_REG mode_ctl;
...
#if TN_DISPLAY
#if S4030AP_PRJ
unsigned char gv_OtpCount = 0;

#define OTPCOUNT    11

unsigned char gv_ser_num[OTPCOUNT][16]=
{
	{
		//6041
		0x71,0x79,0x73,0xae,0x1d,0x7b,0x22,0x48,0x65,0xc6,
		0xf7,0x0e,0xf4,0x18,0xb7,0x62
	},
	{
		0x71,0x79,0x73,0xae,0x1d,0x7b,0x22,0x48,0x65,0xc6,
		0xf7,0x0e,0xf4,0x18,0xb7,0x62
	...
	{
		//6041
		//0x5a,0x35,0x36,0x35,0x34,0x35,0x34,0x44,0x43,0x41,
		//0x4a,0x56,0x4f,0x5a,0x4e,0x5a
		//id-10,lv_pChar=13,0,1c,ae,78,1,53,4d,6b,b0
		//id-20,lv_pChar=88,b,eb,0,ac,0
		0x13,0x00,0x1c,0xae,0x78,0x1,0x53,0x4d,0x6b,0xb0,
		0x88,0x0b,0xeb,0x00,0xac,0x00
	},
	{
                0x1d,0x01,0x64,0xd5,0x04,0x79,0x30,0x42,0x11,0xb9,
                0x8d,0x17,0xe6,0x0f,0xda,0x12
	},
	{
                0x05,0x77,0x66,0xd9,0x19,0x67,0x22,0x44,0x62,0xc6,
                0x93,0x16,0xef,0x14,0xab,0x18
	},
	{
                0x7d,0x66,0x13,0xd3,0x00,0x7e,0x52,0x5d,0x71,0xa5,
                0xf5,0x0e,0xf3,0x11,0xb8,0x16
	},
	{
                0x03,0x7f,0x79,0xd8,0x03,0x76,0x32,0x5a,0x62,0xb7,
                0x92,0x0b,0xee,0x6d,0xb5,0x1d
	},
	{
                0x13,0x66,0x7e,0xd8,0x07,0x62,0x33,0x4a,0x61,0xa3,
                0x91,0x0e,0x9c,0x6f,0xda,0x16
	},
	{
                0x7d,0x61,0x6d,0xad,0x1c,0x62,0x36,0x5e,0x7d,0xb2,
                0x81,0x6b,0x93,0x6f,0xb5,0x09
	},
	{
                0x05,0x6f,0x7f,0xd8,0x08,0x79,0x35,0x5a,0x78,0xa7,
                0x8e,0xb,0x9c,0x0a,0xaa,0x0b
	},
	{
                0x00,0x67,0x65,0xd9,0x15,0x67,0x33,0x46,0x1f,0xa5,
                0x95,0x69,0xe9,0x10,0xb8,0x6f
	}
};


int mt_otp_access(unsigned int f_Type, unsigned int f_Offset, void * f_BufferPtr, unsigned int f_Length, unsigned int * f_Status)
{
	if(f_BufferPtr == 0)
	{
		*f_Status = 1;
	...
	if(gv_OtpCount > OTPCOUNT-1)
	{
		*f_Status = 1;
		return 0;
	}

	memcpy(f_BufferPtr,gv_ser_num[gv_OtpCount],f_Length);
	gv_OtpCount++;
