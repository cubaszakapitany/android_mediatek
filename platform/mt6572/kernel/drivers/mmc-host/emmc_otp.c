...
//songyutao remove static
#if TN_DISPLAY
int mt_otp_access(unsigned int access_type, unsigned int offset, void *buff_ptr, unsigned int length, unsigned int *status)
#else
...
void tinno_emmc_otp_test(void)
{
	...
	#if 0
	{
		...
		otpctl.Length = 36;
		if (!(pbuf = kmalloc(sizeof(char)*otpctl.Length, GFP_KERNEL))){
			printk("OTP:kmalloc failed! \n");
		        return;
		}		
		printk("OTP IOCTL: EMMC_OTP_WRITE Offset(0x%x), Length(0x%x) \n", otpctl.Offset, otpctl.Length);
	        memset(pbuf, 0x00, sizeof(char)*otpctl.Length);		
		//memcpy(pbuf,(void*)"123456789012345678901234567890123456789000000",36);
		for(i = 0;i<36;i++)
		{
			pbuf[i]=i;
		}
	        mt_otp_access(FS_EMMC_OTP_WRITE, otpctl.Offset , pbuf, otpctl.Length, &otpctl.status);
		printk("FS_EMMC_OTP_WRITE otpctl.status=%d\n",otpctl.status);		
		kfree(pbuf);
	}
	#endif
	//read
	{
		...
		printk("FS_EMMC_OTP_READ otpctl.status=%d\n",otpctl.status);
		printk("OTP read data start------------------------------\n");
		for(i = 0;i<36;i++)
		{
			printk("--%d	--\n",pbuf[i]);
		}
		printk("OTP read data end-------------------------------\n");
		kfree(pbuf);
	}
