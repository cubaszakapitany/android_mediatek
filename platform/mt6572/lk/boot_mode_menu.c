/*This file implements MTK boot mode.*/

#include <sys/types.h>
#include <debug.h>
#include <err.h>
#include <reg.h>

#include <platform/mt_typedefs.h>
#include <platform/boot_mode.h>
#include <platform/mt_reg_base.h>
#include <platform/mtk_key.h>

#include <target/cust_key.h>

#define MODULE_NAME "[BOOT_MENU]"
extern char g_CMDLINE [];
bool g_boot_menu = false;

#if defined(MEM_PRESERVED_MODE_ENABLE)
extern void platform_mem_preserved_config(unsigned int enable);
#endif //#if defined(MEM_PRESERVED_MODE_ENABLE)

void boot_mode_menu_select()
{
          int select = 0;  // 0=recovery mode, 1=fastboot.  2=normal boot 3=normal boot + ftrace
          const char* title_msg = "Select Boot Mode:\n[VOLUME_UP to select.  VOLUME_DOWN is OK.]\n\n";
          video_clean_screen();
          video_set_cursor(video_get_rows()/2, 0);
          video_printf(title_msg);
          video_printf("[Recovery    Mode]         <<==\n");
#ifdef MTK_FASTBOOT_SUPPORT
          video_printf("[Fastboot    Mode]             \n");
#endif
          video_printf("[Normal      Boot]             \n");
#ifndef USER_BUILD
          video_printf("[Normal      Boot +ftrace]     \n");
#if defined(MEM_PRESERVED_MODE_ENABLE)
          video_printf("[Normal      No mem-pre]       \n");
#endif //#if defined(MEM_PRESERVED_MODE_ENABLE)
#endif
          while(1)
          {
             if(mtk_detect_key(MT65XX_MENU_SELECT_KEY))//VOL_UP
             {
                    g_boot_menu = true;
                    switch(select) {

                        case 0:
#ifdef MTK_FASTBOOT_SUPPORT
                            select = 1;

                            video_set_cursor(video_get_rows()/2, 0);
                            video_printf(title_msg);
                            video_printf("[Recovery    Mode]             \n");
                            video_printf("[Fastboot    Mode]         <<==\n");
                            video_printf("[Normal      Boot]             \n");
#ifndef USER_BUILD
                            video_printf("[Normal      Boot +ftrace]     \n");
#if defined(MEM_PRESERVED_MODE_ENABLE)
                            video_printf("[Normal      No mem-pre]       \n");
#endif //#if defined(MEM_PRESERVED_MODE_ENABLE)
#endif
                        break;
#endif
                        case 1:
                            select = 2;

                            video_set_cursor(video_get_rows()/2, 0);
                            video_printf(title_msg);
                            video_printf("[Recovery    Mode]             \n");
#ifdef MTK_FASTBOOT_SUPPORT
                            video_printf("[Fastboot    Mode]             \n");
#endif
                            video_printf("[Normal      Boot]         <<==\n");
#ifndef USER_BUILD
                            video_printf("[Normal      Boot +ftrace]     \n");
#if defined(MEM_PRESERVED_MODE_ENABLE)
                            video_printf("[Normal      No mem-pre]       \n");
#endif //#if defined(MEM_PRESERVED_MODE_ENABLE)
#endif
                        break;

                        case 2:
#ifdef USER_BUILD
                            select = 0;
#else
                            select = 3;
#endif

                            video_set_cursor(video_get_rows()/2, 0);
                            video_printf(title_msg);
#ifdef USER_BUILD
                            video_printf("[Recovery    Mode]         <<==\n");
#ifdef MTK_FASTBOOT_SUPPORT
                            video_printf("[Fastboot    Mode]             \n");
#endif
                            video_printf("[Normal      Boot]             \n");
#else
                            video_printf("[Recovery    Mode]             \n");
#ifdef MTK_FASTBOOT_SUPPORT
                            video_printf("[Fastboot    Mode]             \n");
#endif
                            video_printf("[Normal      Boot]             \n");
                            video_printf("[Normal      Boot +ftrace] <<==\n");
#if defined(MEM_PRESERVED_MODE_ENABLE)
                            video_printf("[Normal      No mem-pre]       \n");
#endif //#if defined(MEM_PRESERVED_MODE_ENABLE)
#endif
                        break;

#ifndef USER_BUILD
                        case 3:
#if defined(MEM_PRESERVED_MODE_ENABLE)
                            select = 4;
#else
                            select = 0;
#endif
                            video_set_cursor(video_get_rows()/2, 0);
                            video_printf(title_msg);
#if defined(MEM_PRESERVED_MODE_ENABLE)
                            video_printf("[Recovery    Mode]             \n");
#else
                            video_printf("[Recovery    Mode]         <<==\n");
#endif
#ifdef MTK_FASTBOOT_SUPPORT
                            video_printf("[Fastboot    Mode]             \n");
#endif
                            video_printf("[Normal      Boot]             \n");
                            video_printf("[Normal      Boot +ftrace]     \n");
#if defined(MEM_PRESERVED_MODE_ENABLE)
                            video_printf("[Normal      No mem-pre]   <<==\n");
#endif //#if defined(MEM_PRESERVED_MODE_ENABLE)
                        break;

#if defined(MEM_PRESERVED_MODE_ENABLE)
                        case 4:
                            select = 0;
                            video_set_cursor(video_get_rows()/2, 0);
                            video_printf(title_msg);
                            video_printf("[Recovery    Mode]         <<==\n");
#ifdef MTK_FASTBOOT_SUPPORT
                            video_printf("[Fastboot    Mode]             \n");
#endif
                            video_printf("[Normal      Boot]             \n");
                            video_printf("[Normal      Boot +ftrace]     \n");
                            video_printf("[Normal      No mem-pre]       \n");
                        break;
#endif  //#if defined(MEM_PRESERVED_MODE_ENABLE)
#endif  //#ifndef USER_BUILD
                        default:
                        break;
                    }
                    dprintf(0,  "[VOL_UP]Key Detect, current select:%d\n", select);
                    mdelay(300);
             }
             else if(mtk_detect_key(MT65XX_MENU_OK_KEY))//VOL_DOWN,
             {
                 //use for OK
                 break;
             }
             else
             {
                //pass
             }
          }
          if(select == 0)
          {
               g_boot_mode = RECOVERY_BOOT;
          }
          else if(select == 1)
          {
                g_boot_mode = FASTBOOT;
          }
          else if(select == 2)
          {
                g_boot_mode = NORMAL_BOOT;
          }
          else if(select == 3)
          {
                sprintf(g_CMDLINE, "%s trace_buf_size=11m boot_time_ftrace", g_CMDLINE);
                g_boot_mode = NORMAL_BOOT;
          }
#if defined(MEM_PRESERVED_MODE_ENABLE)
          else if(select == 4)
          {
                dprintf(0, "No memory preserved mode, current select:%d\n", select);
                //enable mem-preserved mode.
                platform_mem_preserved_config(0);
                g_boot_mode = NORMAL_BOOT;
          }
#endif //#if defined(MEM_PRESERVED_MODE_ENABLE)
          else{
                //pass
          }

          video_set_cursor(video_get_rows()/2 +8, 0);
          return;
}

//BEGIN <ftm> <DATE2013118> <ftm ker V+ + V-> mickal.ma
void boot_mode_menu_select2()
{
          int select = 0;  // 0=recovery mode, 1=fastboot.  2=normal boot
          const char* title_msg = "Select Boot Mode:\n[VOLUME_UP to Recovery Mode.  VOLUME_DOWN to factory Mode.]\n\n";
          video_clean_screen();

          #if 0
          video_set_cursor(2, 0);
          video_printf(title_msg);
          video_printf("[Recovery    Mode]             \n");
          video_printf("[Factory    Mode]             \n");
          #else
          video_set_cursor(2,0);
          video_printf("Recovery Mode: Volume Up\r\n");
          video_set_cursor(4,0);
          video_printf("Factory Mode: Volume Down\r\n");
          #endif
          
	   mt65xx_backlight_on();
	   ulong begin = get_timer(0);

	   while(mtk_detect_key(MT65XX_MENU_SELECT_KEY) || mtk_detect_key(MT65XX_MENU_OK_KEY))
	   {

	   }
	   
          while(1)
          {
		if(mtk_detect_key(MT65XX_MENU_SELECT_KEY)&&!mtk_detect_key(MT65XX_MENU_OK_KEY))//VOL_UP
		{
			select = 0;
			break;
		}
		else if(mtk_detect_key(MT65XX_MENU_OK_KEY)&&!mtk_detect_key(MT65XX_MENU_SELECT_KEY))//VOL_DOWN,
		{
			select = 1;
			break;
		}
		else
		{
		//pass
		}
        
	    if (get_timer(begin) < 20000)
		mtk_wdt_restart();
          }
          if(select == 0)
          {
               g_boot_mode = RECOVERY_BOOT;
          }
          else if(select == 1)
          {
                g_boot_mode = FACTORY_BOOT;
          }
          else
          {
                //pass
          }

          video_set_cursor(2 +8, 0);
          return;
//END <ftm> <DATE2013118> <ftm ker V+ + V-> mickal.ma
}

//#define USES_DAYO_DMAT		//songyutao test

#ifdef USES_DAYO_DMAT
void boot_mode_menu_selectdayo()
{
          int select = 0;  // 0=recovery mode, 1=fastboot.  2=normal boot
          video_clean_screen();

          video_set_cursor(2,0);
          video_printf("DAYO mode select \r\n");
          
	   mt65xx_backlight_on();
	   ulong begin = get_timer(0);
	   
          while(1)
          {
		if(mtk_detect_key(MT65XX_MENU_SELECT_KEY)&&!mtk_detect_key(MT65XX_MENU_OK_KEY))//VOL_UP
		{
			select = 0;
			break;
		}
		else if(mtk_detect_key(MT65XX_MENU_OK_KEY)&&!mtk_detect_key(MT65XX_MENU_SELECT_KEY))//VOL_DOWN,
		{
			select = 1;
			break;
		}
		else
		{
		//pass
		}
        
	    if (get_timer(begin) < 20000)
		mtk_wdt_restart();
          }
          if(select == 0)
          {
               g_boot_mode = RECOVERY_BOOT;
          }
          else if(select == 1)
          {
                g_boot_mode = FACTORY_BOOT;
          }
          else
          {
                //pass
          }

          video_set_cursor(2 +8, 0);
          return;
}

#endif /*USES_DAYO_DMAT*/

BOOL boot_menu_key_trigger(void)
{
#if 1
	//wait
    ulong begin = get_timer(0);
    printf("\n%s Check  boot menu\n",MODULE_NAME);
    printf("%s Wait 50ms for special keys\n",MODULE_NAME);

    //let some case of recovery mode pass.
    if(unshield_recovery_detection())
    {
        return TRUE;
    }

//BEGIN  <ftm ker: factory mode "power + V-" and recovery "power + V+"> dayo 
#ifdef USES_DAYO_DMAT
    while(get_timer(begin)<50)
    {
		if(mtk_detect_key(MT65XX_BOOT_MENU_KEY) || mtk_detect_key(MT65XX_MENU_OK_KEY))
		{
                           mtk_wdt_disable();
                           boot_mode_menu_selectdayo();
                           mtk_wdt_init();
                           return TRUE;
		}
    }
    begin = get_timer(0);
    return FALSE;
#endif /*USES_DAYO_DMAT*/
//END  <ftm ker: factory mode "power + V-" and recovery "power + V+"> dayo

//BEGIN <ftm> <DATE2013118> <ftm ker V+ + V-> mickal.ma
#if 1
    while(get_timer(begin)<50)
    {
		if(mtk_detect_key(MT65XX_BOOT_MENU_KEY)&&mtk_detect_key(MT65XX_MENU_OK_KEY))
		{
                           mtk_wdt_disable();
                           boot_mode_menu_select2();
                           mtk_wdt_init();
                           return TRUE;
		}
    }
    begin = get_timer(0);
    return FALSE;
#endif
//END <ftm> <DATE2013118> <ftm ker V+ + V-> mickal.ma

    while(get_timer(begin)<50)
    {
		if(mtk_detect_key(MT65XX_BOOT_MENU_KEY))
		{
                           mtk_wdt_disable();
                           boot_mode_menu_select();
                           mtk_wdt_init();
                           return TRUE;
		}
    }
#endif
     return FALSE;
}

BOOL boot_menu_detection(void)
{
    return boot_menu_key_trigger();
}


int unshield_recovery_detection(void)
{
    //because recovery_check_command_trigger's BOOL is different from the BOOL in this file.
    //so use code like this type.
    return recovery_check_command_trigger()? TRUE:FALSE;
}

