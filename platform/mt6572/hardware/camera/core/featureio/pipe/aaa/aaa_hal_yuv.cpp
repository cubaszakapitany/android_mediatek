    ...
    else if (eCmd == ECmd_PrecaptureStart)
    {
        if (m_pStrobeDrvObj)
        {
            ...
            if (fgFlashOn)
            {
                ...
                if(m_rParam.u4ShotMode == CAPTURE_MODE_BURST_SHOT)
                {
                    if (m_pStrobeDrvObj->setDuty(6) == MHAL_NO_ERROR)
                    {
                        MY_LOG("Burst shot mode, setLevel:6\n");
                    }
                }
                else
                {
    ...
    if (m_pStrobeDrvObj)
    {
        ...
        else
        {
            ...
            if (mode == FLASHLIGHT_TORCH)
            {
                ...
                m_pStrobeDrvObj->setDuty(NSCamCustom::custom_GetYuvTorchDuty());
    ...
    if (m_pStrobeDrvObj && m_strobeWidth >0 && m_bFlashActive == TRUE)
    {
        ...
        if (m_rParam.u4ShotMode == CAPTURE_MODE_BURST_SHOT)
        {
            if (m_pStrobeDrvObj->setTimeOutTime(0) == MHAL_NO_ERROR)
            {
                ...
                MY_LOG("Burst shot mode, setTimeOutTime: 0 ms\n");
            }
            if (m_pStrobeDrvObj->setDuty(6) == MHAL_NO_ERROR)
            {
                MY_LOG("Burst shot mode, setLevel: 6\n");
